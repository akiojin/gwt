...
- **develop ブランチへの直接コミットは禁止。** develop は PR マージ専用ブランチであり、エージェント・人間を問わず直接コミットしてはならない。SPEC 策定や壁打ちで develop 上で作業する場合も、コミットは feature ブランチで行うこと。pre-commit hook がこのルールを強制する。

...
The repo root `docker-compose.yml` resolves host config mounts with `HOME`
first and falls back to `USERPROFILE` when `HOME` is unset. This keeps Windows
PowerShell / cmd launches working without manually exporting `HOME`.
...
### Windows で `HOME must be set` が発生する場合

repo root の `docker-compose.yml` は `HOME` を優先し、未設定時だけ
`USERPROFILE` にフォールバックします。通常の Windows セッションでは追加設定なしで
起動できる想定です。

それでも `HOME or USERPROFILE must be set` で失敗する場合は、起動元プロセスが
両方の環境変数を失っているため、PowerShell / cmd / Docker Desktop を
通常のユーザー環境から起動し直してください。

...
function getSessionFilePath(repositoryRoot: string): string {
  ...
  const repoHash = Buffer.from(repositoryRoot).toString('base64').replace(/[/+=]/g, '_');
...
export async function saveSession(sessionData: SessionData): Promise<void> {
  try {
    ...
    if (process.env.DEBUG_SESSION) {
      console.error('Failed to save session:', error instanceof Error ? error.message : String(error));
    }
  }
}

export async function loadSession(repositoryRoot: string): Promise<SessionData | null> {
  try {

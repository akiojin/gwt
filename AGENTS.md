...
| gwt-spec-brainstorm | `/gwt:gwt-spec-brainstorm` | Cross-agent pre-SPEC intake for rough ideas. Interview the user one question at a time, search existing Issues and SPECs first, then route automatically to an existing SPEC, a new SPEC, or a plain Issue. Use when user says "brainstorm this before writing a spec", starts from a title-level request, or asks whether something should become a SPEC. |

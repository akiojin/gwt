...
## 2026-04-07 — fix: Codex は mouse mode を明示しなくても PTY-owned scroll として扱う

### 事象

Claude Code では改善したのに、Codex ではまだ local snapshot scrollback に落ち、
スクロール結果と scrollbar が破綻した。

### 原因

- Codex pane は terminal viewport を自前で持っていても、
  gwt が期待していた `?1000h` / `?1006h` の明示的な mouse-mode 宣言を必ずしも出さない。
- そのため「mouse mode 宣言がある agent だけ PTY-owned scroll にする」という判定では、
  Codex だけ誤って local snapshot scrollback 側へ残った。

### 再発防止策

1. PTY-owned scroll の capability 判定に agent-specific rule を持たせ、Codex は明示的に remote-scroll agent として扱う。
2. remote-scroll agent では local scrollbar overlay も抑止し、指標の責務分裂を起こさない。
3. Codex pane が explicit mouse reporting なしでも wheel input を PTY へ受け取る回帰テストを固定する。

...
Type: workflow
Context: SPEC-2077 T-266 initially failed in SS.mov because the recorded Codex window used hooks generated by an older /Applications/GWT.app gwtd fallback and lacked enabled trust entries in ~/.codex/config.toml; the dev serve URL had also rotated.
...
Future Action: Before manual Codex hook/status verification, rebuild target/debug/gwt and target/debug/gwtd, restart only the dev gwt serve process, share the new URL, and inspect ~/.codex/config.toml for the tested worktree or relaunch the Codex agent from the fresh server.
...
## 2026-05-30 — gwt headed-browser E2E harness: serve 廃止後の隔離起動 + Playwright bridge

Type: lesson
Context: #2937 (Shift+Cmd+Arrow 画面崩れ) を別 worktree の fix build (ca2fe7144) で独立 E2E 検証。headless-browser-check skill の 'gwt serve' recipe は SPEC #2920 で serve が削除済みのため v9.49.0+ build で使えなかった。
Learning: (1) v9.49.0+ で 'gwt serve' 廃止。隔離 E2E は 'HOME=<isolated> GWT_BROWSER_URL_FILE=<f> <bin> --no-tray --no-open' で起動し URL を file から読む(singleton は HOME 単位なので隔離 HOME で衝突回避)。(2) fresh HOME は chroma venv を初回再構築(遅く exit 144 で落ちることあり)。venv cache 後に再起動 + GWT_INDEX_FAKE_EMBEDDING=1 で安定。(3) Playwright test bridge は page.addInitScript(()=>window.__gwtPlaywrightTestBridge=true) を goto 前に仕込み、window.dispatchEvent(new CustomEvent('__gwt_test_send',{detail:{kind,...}})) で FrontendEvent 送信。reopen_recent_project{path} は任意 path を project として開く。create_window{preset:'shell'|'claude'}。(4) agent preset window は create_window では 'not_started'(launch wizard 必須)なので、rendering 崩れ test は shell pane で実 'claude' を起動するのが早い(alternate-screen TUI を再現)。(5) plain git repo を開くと migration modal(Migrate ボタンのみ)が pointer を遮る。(6) #2937 の崩れは preset 非依存(cycle_focus_events が全 pane の PTY を resize)。
Future Action: gwt GUI/terminal を headed E2E する時はこの手順を使う。別 agent の fix branch を検証する時は worktree の git status を必ず確認(fix が working tree で revert されていることがある=baseline 取得の名残)。rebuild せず committed binary を使う。

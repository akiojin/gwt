...
## 2026-05-29 — GUI 視覚検証は本番 GWT.app を止めず 隔離 HOME + reopen_recent_project で行う

Type: lesson
Context: Opus 4.8 ラベル変更(PR #2935)の PR 前視覚検証で、本番 GWT.app が per-user tray lock(~/.gwt/, main.rs:6242 cli::tray::lock::acquire)を保持しており、ビルド済み target/debug/gwt --no-tray --no-open が tray-resident 検出で exit 0 し、既存インスタンスの URL(同一ポート)を流用してしまう問題に遭遇。さらに gwt serve/--headless は SPEC #2920(v10)で廃止され後継は gwt --no-tray --no-open。headless Playwright は Open Project の server-side native picker を操作できない。
Learning: HOME を一時dirへ隔離し $TMPHOME/.gwt/runtime と cache を実 ~/.gwt のものへ symlink した上で HOME=$TMPHOME gwt --no-tray --no-open を起動すると、別 tray-lock・別ポートで自分のビルド済みバイナリを serve できる(本番 GWT.app は停止不要)。プロジェクトは native picker を介さず ws://host/ws へ {kind:'reopen_recent_project', path:'<worktree>'} を送れば開け(サーバ側 project state は global)、reload 後 Start Work → Configure and start でモデルドロップダウンを確認できる。
Future Action: production GWT.app を停止せずに gwt GUI を視覚検証する必要がある時は、隔離 HOME 起動 + reopen_recent_project WS で native picker と per-user tray lock を回避する。AGENTS.md の skip 禁止ルールを守りつつ視覚確認動線を再現できる。

...
## 2026-06-08 — Branch Cleanup reconnect must not synthesize failed results

Type: lesson
Context: SPEC-2009 Phase 6: Branch Cleanup could finish on the backend after WebSocket loss, while frontend converted running cleanup to failed rows with 'Connection lost while cleaning up branches'. Backend worker sent final result to the old client_id, so reconnecting clients missed it.
Learning: Long-running GUI operations need a frontend operation_id and backend operation snapshot/broadcast/replay path. WebSocket close should mark the operation interrupted/reconnecting, not synthesize per-item failure rows.
Future Action: For any long-running WebSocket operation, add protocol operation_id, stale-event guard, sync/clear commands, and focused reconnect tests before relying on client_id-scoped replies.

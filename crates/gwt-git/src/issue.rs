...
impl IssueCache {
    ...
    pub fn new() -> Result<Self> {
        let home = dirs::home_dir().ok_or_else(|| {
            GwtError::Internal("Cannot determine home directory".into())
        })?;
        let cache_dir = home.join(".gwt").join("cache").join("issues");
        fs::create_dir_all(&cache_dir).map_err(|e| GwtError::Internal(e.to_string()))?;
        Ok(Self { cache_dir })
    }
    ...
    pub fn read(&self, owner: &str, repo: &str) -> Result<Option<Vec<Issue>>> {
        ...
        let data = fs::read_to_string(&path).map_err(|e| GwtError::Internal(e.to_string()))?;
        let issues: Vec<Issue> =
            serde_json::from_str(&data).map_err(|e| GwtError::Internal(e.to_string()))?;
        Ok(Some(issues))
    }
    ...
    pub fn write(&self, owner: &str, repo: &str, issues: &[Issue]) -> Result<()> {
        let path = self.cache_path(owner, repo);
        let data =
            serde_json::to_string_pretty(issues).map_err(|e| GwtError::Internal(e.to_string()))?;
        fs::write(&path, data).map_err(|e| GwtError::Internal(e.to_string()))?;
        Ok(())
    }
...
pub fn fetch_issues(owner: &str, repo: &str) -> Result<Vec<Issue>> {
    ...
    let output = gwt_core::git::gh_cli::gh_command()
        .args([
            "issue",
            "list",
            "--repo",
...
pub fn parse_gh_issues_json(json: &str) -> Result<Vec<Issue>> {
    let raw: Vec<serde_json::Value> =
        serde_json::from_str(json).map_err(|e| GwtError::Internal(e.to_string()))?;

...
//!
//! Shows error messages in a centered modal overlay with severity-based styling,
//! optional details/suggestions, and FIFO queue support for multiple errors.
...
impl ErrorState {
    ...
    pub fn from_gwt_error(err: &gwt_core::error::GwtError) -> Self {
        ...
        let category = match err {
            gwt_core::error::GwtError::Io(_) => "IO",
            gwt_core::error::GwtError::Git(_) => "Git",
            gwt_core::error::GwtError::Config(_) => "Config",
            gwt_core::error::GwtError::Agent(_) => "Agent",
            gwt_core::error::GwtError::Terminal(_) => "Terminal",
            gwt_core::error::GwtError::Docker(_) => "Docker",
            gwt_core::error::GwtError::Ai(_) => "AI",
            gwt_core::error::GwtError::Notification(_) => "Notification",
            gwt_core::error::GwtError::Voice(_) => "Voice",
            gwt_core::error::GwtError::Clipboard(_) => "Clipboard",
            gwt_core::error::GwtError::Skill(_) => "Skill",
            gwt_core::error::GwtError::Other(_) => "Error",
        };

        Self {
            title: format!("{category} Error"),
            message,
            code: None,
            details: Vec::new(),
            suggestions: Vec::new(),
...
pub fn render_error_with_queue(queue: &ErrorQueue, buf: &mut Buffer, area: Rect) {
    if let Some(state) = queue.current() {
        render_error_internal(state, queue.position_string(), buf, area);
    }
}
...
fn render_error_internal(
    ...
    let mut constraints = vec![Constraint::Length(3)]; // Message
    ...
    let message = Paragraph::new(state.message.clone())
        ...
        .wrap(Wrap { trim: true });
    message.render(chunks[chunk_idx], buf);
    chunk_idx += 1;
    ...
    if !state.details.is_empty() {
        ...
        details_block.render(chunks[chunk_idx], buf);
        ...
        details.render(details_inner, buf);
        chunk_idx += 1;
    }
    ...
    if !state.suggestions.is_empty() {
        ...
        suggestions_block.render(chunks[chunk_idx], buf);
        ...
        suggestions_para.render(suggestions_inner, buf);
        chunk_idx += 1;
    }
    ...
    footer.render(chunks[chunk_idx], buf);
}
...
mod tests {
    ...
    #[test]
    fn test_render_error_no_panic() {
        let mut queue = ErrorQueue::new();
        queue.push(
            ErrorState::from_error("Test error")
                .with_code("E1001")
        ...
        render_error_with_queue(&queue, &mut buf, area);
    }
}

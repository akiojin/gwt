use super::{
    app_js, index_html, root_js_module_assets, root_js_module_response, static_asset_response,
    static_asset_text, static_assets, AssetBody,
};

fn root_js_module_source(path: &str) -> &'static str {
    root_js_module_assets()
        .iter()
        .find(|asset| asset.path == path)
        .unwrap_or_else(|| panic!("root JS module manifest has no entry for {path}"))
        .source
}
...
fn xterm_js() -> &'static str {
    static_asset_text("/assets/xterm/xterm.mjs")
}

fn xterm_fit_js() -> &'static str {
    static_asset_text("/assets/xterm/addon-fit.mjs")
}
...
fn embedded_web_terminal_clipboard_fallback_restores_terminal_focus() {
    ...
    assert!(
        html.contains("writeClipboardText(selection") && html.contains("runtime.terminal.focus()"),
...
fn embedded_web_terminal_writes_refresh_viewport_after_xterm_parse() {
    ...
    let snapshot_activation = regex::Regex::new(
        ...
        .expect("valid regex");
    let refresh_call =
        regex::Regex::new(r"runtime\.terminal\.refresh\(0,\s*runtime\.terminal\.rows\s*-\s*1\);")
            .expect("valid regex");
...
/// SPEC-3016: full snapshot of the static asset manifest. Pins every
/// served route with its content type and cache-control policy, asserts
/// non-empty embedded content, and checks the response headers built
/// from each manifest entry. Route registration iterates this manifest
/// (see embedded_server.rs), so this table is the serving contract.
#[test]
fn embedded_web_static_asset_manifest_is_complete() {
    use axum::http::header;

    const JS: &str = "text/javascript; charset=utf-8";
    const CSS: &str = "text/css; charset=utf-8";
    const FONT: &str = "font/woff2";
    const MUTABLE: Option<&str> = Some("no-store, max-age=0");
    const IMMUTABLE: Option<&str> = Some("public, max-age=31536000, immutable");
    let expected: &[(&str, &str, Option<&str>)] = &[
        ...
        ("/assets/highlight/highlight.min.js", JS, None),
        ("/assets/highlight/github-dark.min.css", CSS, None),
        ("/styles/tokens.css", CSS, MUTABLE),
        ("/styles/typography.css", CSS, MUTABLE),
        ("/styles/components.css", CSS, MUTABLE),
        ("/styles/app.css", CSS, MUTABLE),
        ("/assets/fonts/MonaSans.woff2", FONT, IMMUTABLE),
        ("/assets/fonts/HubotSans-Regular.woff2", FONT, IMMUTABLE),
        ("/assets/fonts/HubotSans-Bold.woff2", FONT, IMMUTABLE),
        (
            "/assets/fonts/HubotSansCondensed-Bold.woff2",
            FONT,
            IMMUTABLE,
        ),
        ("/assets/fonts/JetBrainsMono.woff2", FONT, IMMUTABLE),
    ];

    let manifest: Vec<(&str, &str, Option<&str>)> = static_assets()
        .iter()
        .map(|asset| (asset.route, asset.content_type, asset.cache_control))
        .collect();
    assert_eq!(
        manifest, expected,
        "static asset manifest changed; URLs / content types / cache \
             policies are a serving contract — update deliberately",
    );

    for asset in static_assets() {
        let non_empty = match asset.body {
            AssetBody::Text(text) => !text.is_empty(),
            AssetBody::Bytes(bytes) => !bytes.is_empty(),
        };
        assert!(
            non_empty,
            "expected {} to have embedded content",
            asset.route,
        );
        ...
        match asset.cache_control {
            Some(cache_control) => assert_eq!(
            ...
            None => assert!(
                ...
                "expected {} to omit cache-control",
                asset.route,
            ),
        }
    }
...
/// Root JS modules are first-party sources: always JavaScript content
/// type and never served stale from the WebView cache.
#[test]
fn embedded_web_root_js_modules_use_js_content_type_and_mutable_cache() {
    use axum::{http::header, response::IntoResponse};

    for asset in root_js_module_assets() {
        let response = root_js_module_response(*asset).into_response();
        assert_eq!(
            response.headers().get(header::CONTENT_TYPE).unwrap(),
...
fn embedded_web_socket_open_replays_frontend_ready_before_flushing_pending_messages() {
    ...
    let open_flow = regex::Regex::new(
            r#"function handleSocketOpen\(\)\s*\{[\s\S]*?setConnectionState\(true\);\s*send\(\{\s*kind:\s*"frontend_ready"\s*\}\);\s*recoveryCenterController\?\.reconnect\(\);\s*while\s*\(\s*pendingMessages\.length\s*>\s*0\s*\)\s*\{\s*socket\.send\(JSON\.stringify\(pendingMessages\.shift\(\)\)\);\s*\}\s*\}"#,
        )
        .expect("valid regex");
...
fn embedded_web_serves_every_root_module_import() {
    ...
    assert!(
        embedded_server_source.contains("static_assets()"),
        "expected embedded server static asset routes to be manifest-driven (SPEC-3016)",
    );
    ...
    for asset in root_js_module_assets() {
        ...
        // SPEC-3016: the root_js_modules! manifest derives the route and
        // the include_str! path from one file-name literal, so each
        // registered module must appear as exactly that literal.
        let manifest_entry = format!("\"{}\"", module_path.trim_start_matches('/'));
        assert!(
            embedded_web_source.contains(&manifest_entry),
            "expected embedded web manifest entry {manifest_entry} for {module_path}",
        );
    }
}
...
fn embedded_web_workspace_layout_primitives_define_shared_contracts() {
    ...
    for (selector, expected_props) in primitives {
        ...
        let end = block
            .find('}')
            .unwrap_or_else(|| panic!("expected layout primitive `{selector}` to close with `}}`"));
        let body = &block[..end];
...
fn embedded_web_modal_frame_primitives_define_shared_contracts() {
    ...
    for (selector, expected_props) in primitives {
        let start = html
            .find(selector)
            .unwrap_or_else(|| panic!("expected modal frame primitive `{selector}` to be defined"));
        let block = &html[start..];

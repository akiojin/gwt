...
fn discover_repair_authority_expectation(
    ...
    let mut hints = Vec::new();
    for contents in [&projection, &pointer].into_iter().flatten() {
    ...
    let owner = match hints.as_slice() {
        [owner] => *owner,
        [] => {
            return Err(invalid_generation_data(
                "execution_repair_owner_unknown: corrupt authority does not expose an owner and the current Session has no linked owner",
...
struct ExecutionRepairPendingCommit {
    ...
    authority_paths: [PathBuf; 3],
    audit_dir: PathBuf,
    previous_audit_bytes: Option<Vec<u8>>,
    previous_projection_mirror: Option<Vec<u8>>,
    previous_pointer_mirror: Option<Vec<u8>>,
}

fn restore_file_snapshot(path: &Path, previous: Option<&[u8]>) -> io::Result<()> {
    match previous {
        Some(bytes) => {
...
fn rollback_pending_execution_repair(
    ...
    let restored = repair_error_after_audit_and_authority_restore(
        error,
        &authority_paths,
        &pending.outcome.quarantined,
        &pending.audit_dir,
        pending.previous_audit_bytes.as_deref(),
    );
    let projection_restore = restore_file_snapshot(
        &state_path(worktree),
        pending.previous_projection_mirror.as_deref(),
    );
    let pointer_restore = restore_file_snapshot(
        &generation_pointer_path(worktree),
        pending.previous_pointer_mirror.as_deref(),
    );
    match (projection_restore, pointer_restore) {
        (Ok(()), Ok(())) => restored,
        (projection, pointer) => io::Error::new(
            ErrorKind::InvalidData,
            format!(
                "execution_repair_rollback_failed: {restored}; projection_mirror_restore={}; pointer_mirror_restore={}",
                projection
                    .err()
                    .map_or_else(|| "ok".to_string(), |failure| failure.to_string()),
                pointer
...
fn repair_corrupt_execution_impl(
    ...
    with_generation_activation_leases(worktree, owner, |context| {
        let repair = || {
            ...
            // authority.
            validate_repair_authority_expectation(context, &authority_expectation)?;
            let previous_projection_mirror = read_optional_authority_bytes(&state_path(worktree))?;
            let previous_pointer_mirror =
                read_optional_authority_bytes(&generation_pointer_path(worktree))?;
            ...
            Ok(ExecutionRepairPendingCommit {
                ...
                binding,
                authority_paths: [ecr_path, pointer_path, ledger_path],
                audit_dir,
                previous_audit_bytes,
                previous_projection_mirror,
                previous_pointer_mirror,
            })
        };
        ...
        let binding_repair = match expected_session {
            ...
            None => repair_session_binding(worktree, session_id, owner, pending.binding.clone()),
        };
        match (expected_session, binding_repair) {
            ...
            (Some(_), Err(error)) => {
                return Err(rollback_pending_execution_repair(
                    worktree, &pending, error,
                ));
            }

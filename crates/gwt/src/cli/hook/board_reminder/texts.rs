...
\n\
毎周回、`pr.list` の `unlanded_branches` も読んで棚卸しします。各行は commit 済みなのに PR が無い作業です。owner Issue を再起動するか archive を裁定してください。未対応のまま次周回へ持ち越した行は escalation であり、「変化なし」ではありません。";

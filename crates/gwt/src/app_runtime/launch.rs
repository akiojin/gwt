...
impl FinalizedAgentCapabilityLaunch<'_> {
    ...
    fn install_with_prepared_claim(
        ...
        if let Some(current_binding_identity) = current_binding.as_ref() {
            ...
            let current_status = ledger.current_effective_status();
            // Issue #3472: a Completed generation already returned its
            // authority, so no holder can reopen it in place and refusing here
            // strands the owner on every later launch. It takes the same
            // terminal successor route as Blocked, through the Completed
            // predecessor transaction the manual Launch Agent already uses.
            let completed_predecessor = current_status
                == Some(gwt::cli::execution_state::ExecutionControlStatus::Completed);
            // Issue #3457: a non-Blocked generation only legitimately blocks a
            // fresh launch while its holder can still return. Once the holder's
            // Host is gone it can never settle, so the launch supersedes it
            // through the exact-terminal successor transaction instead of
            // refusing forever.
            let unreachable_holder = if completed_predecessor
                || current_status
                    == Some(gwt::cli::execution_state::ExecutionControlStatus::Blocked)
            {
                None
            } else {
                let holder = gwt::cli::execution_state::unreachable_current_generation_holder(
                    sessions_dir,
                    worktree,
                    owner,
                )
            ...
            let request = gwt::cli::execution_state::SuccessorRequest {
                ...
                work_id: None,
                source: if completed_predecessor {
                    gwt::cli::execution_state::MANUAL_COMPLETED_OWNER_LAUNCH_SOURCE.to_string()
                } else {
                    gwt::cli::execution_state::FRESH_LINKED_OWNER_LAUNCH_SOURCE.to_string()
                },
            ...
            let prepared = match unreachable_holder.as_ref() {
                Some(holder) => gwt::cli::execution_state::prepare_exact_terminal_active_successor(
                    worktree,
                    owner,
                    &request,
                    sessions_dir,
                ...
                None if completed_predecessor => {
                    gwt::cli::execution_state::prepare_exact_manual_launch_successor(
                        ...
                        gwt::cli::execution_state::ExactManualLaunchPredecessor {
                            ...
                            session: None,
                            runtime: None,
                            binding: current_binding_identity,
                            status:
                                gwt::cli::execution_state::SuccessorPredecessorStatus::Completed,
                            terminal_reason:
                ...
                None => gwt::cli::execution_state::prepare_fresh_linked_owner_launch_successor(

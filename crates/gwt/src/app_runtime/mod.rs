...
pub struct AppRuntime {
    ...
    /// Issue #4143 (AC-3): windows spawned by automatic restore whose PTY has
    /// not started yet, mapped to the restored Session id (`None` for a
    /// non-agent process window). A restore that dies before its PTY must not
    /// leave a persistent Error window behind: that window would be restored
    /// again on the next start and the failures would pile up across
    /// generations. Consumed by [`AppRuntime::launch_error_events`] and dropped
    /// once the PTY is live.

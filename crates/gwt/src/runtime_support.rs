...
/// Whether a worktree-relative status `entry` currently holds only content
/// generated by gwt during launch and is therefore safe to reap with a
/// disposable Intake worktree.
///
/// This covers merged provider hook configs and the lane registry file. An
/// extra user key, a non-managed hook, a tracked deletion, or any unrelated
/// `.gwt` file remains real local work. This bridges `gwt-git`'s teardown probe
/// (which cannot depend on `gwt-skills`) to managed-asset semantics.
pub fn intake_hook_config_is_disposable(worktree: &Path, entry: &str) -> bool {
    let entry = entry.trim().trim_matches('"');
    let entry = entry.strip_prefix("./").unwrap_or(entry);
    if entry.trim_end_matches('/') == ".gwt" {
        return intake_gwt_directory_has_only_generated_content(worktree);
    }
    if entry == gwt_skills::LANE_FILE_RELATIVE {
        return intake_lane_file_has_only_generated_content(&gwt_skills::lane_file_path(worktree));
    }
...
fn intake_gwt_directory_has_only_generated_content(worktree: &Path) -> bool {
    let gwt_dir = worktree.join(".gwt");
    let lane_file = gwt_skills::lane_file_path(worktree);
    let mut pending = vec![gwt_dir];
    let mut saw_lane_file = false;
    ...
    while let Some(dir) = pending.pop() {
        ...
        for entry in entries {
            ...
            if file_type.is_dir() {
                pending.push(path);
            } else if file_type.is_file() && path == lane_file {
                if !intake_lane_file_has_only_generated_content(&path) {
                    return false;
                }
                saw_lane_file = true;
            } else {
                // Unknown files and symlinks may contain user-authored state.
                return false;
            }
        }
    }
...
fn intake_lane_file_has_only_generated_content(path: &Path) -> bool {
    let Ok(bytes) = std::fs::read(path) else {
        // A status entry for a missing path is a tracked deletion, not a
        // launch-generated file that can be discarded.
        return false;
    };
    ...
    let Some(lane) = fields.get("lane").and_then(serde_json::Value::as_str) else {
        return false;
    };
    let known_lane = gwt_skills::LaneRegistry::resolve(lane);
    known_lane.id == lane
        && fields
            .get("profile_version")
            .and_then(serde_json::Value::as_u64)
            == Some(u64::from(gwt_skills::LANE_FILE_VERSION))
}
...
pub enum FrontDoorRoute {
    ...
    RepoBackedCli,
    DetachedCli,
    /// SPEC #2920: tray-resident process route. Phase 4 will wire `main()`
    /// to dispatch this variant to `gwt::cli::tray::run`. Until then it is
    /// reachable only from tests and reserves the routing slot so the
    /// public enum API stays stable across Phase 2〜4 commits.
    #[allow(dead_code)]
    Tray,
    /// SPEC #2920 Q9: `gwt serve` / `gwt --headless` verbs were removed in
    /// v10.0.0 (supersede of SPEC-1942 US-14). `main()` matches this
    /// variant first, writes the usage hint, and exits with code 2 — no
    /// embedded server is bootstrapped on this route anymore.
    LegacyServeUsageHint,
}

pub fn front_door_route(argv: &[String]) -> FrontDoorRoute {
    match argv.get(1).map(String::as_str) {
        Some("issue" | "pr" | "actions") => FrontDoorRoute::RepoBackedCli,
        Some("serve" | "--headless") => FrontDoorRoute::LegacyServeUsageHint,
...
/// SPEC #2920 Q9: canonical stderr message emitted when a caller still
/// invokes the removed `gwt serve` / `gwt --headless` verbs. Exposed so
/// regression tests and `main()` share the exact same string.
pub const LEGACY_SERVE_USAGE_HINT: &str =
    "gwt: `gwt serve` / `gwt --headless` have been removed in v10.0.0 (SPEC #2920). Use `gwt --no-tray --no-open` instead.";
...
pub fn run_cli(argv: &[String]) -> io::Result<()> {
    match front_door_route(argv) {
        ...
        // Gui, Tray, and LegacyServeUsageHint are all handled by `main()`
        // after `run_cli` returns; `run_cli` itself only dispatches verbs
        // that exit the process.
        FrontDoorRoute::Gui | FrontDoorRoute::Tray | FrontDoorRoute::LegacyServeUsageHint => Ok(()),
    }
}
...
mod tests {
    ...
    #[test]
    fn intake_lane_file_is_disposable_only_with_generated_shape() {
        let dir = tempfile::tempdir().unwrap();
        let worktree = dir.path();

        gwt_skills::write_lane_file(worktree, &gwt_skills::INTAKE_PROFILE)
            .expect("write generated lane file");
        assert!(intake_hook_config_is_disposable(
            worktree,
            gwt_skills::LANE_FILE_RELATIVE
        ));
        assert!(
            intake_hook_config_is_disposable(worktree, ".gwt/"),
            "git status collapses ignored lane metadata to the .gwt directory"
        );

        std::fs::write(
            gwt_skills::lane_file_path(worktree),
            r#"{"lane":"intake","profile_version":1,"user_note":"keep"}"#,
        )
        .expect("write user-extended lane file");
        assert!(
            !intake_hook_config_is_disposable(worktree, gwt_skills::LANE_FILE_RELATIVE),
            "extra user content in a managed file must keep the worktree"
        );
        assert!(!intake_hook_config_is_disposable(worktree, ".gwt/"));
        assert!(!intake_hook_config_is_disposable(
            worktree,
            ".gwt/user-notes.md"
        ));
    ...
    #[test]
    fn front_door_route_routes_serve_subcommand_to_legacy_usage_hint() {
        for args in [
            argv(&["gwt", "serve"]),
            ...
            assert_eq!(
                front_door_route(&args),
                FrontDoorRoute::LegacyServeUsageHint
            );
            assert!(
                !gwt::cli::should_dispatch_cli(&args),
                "removed serve must not fall through to CLI dispatch: {args:?}"
            );
        }
    }

    // SPEC #2920 Q9: `gwt --headless` is removed alongside `gwt serve`. Both
    // forms must resolve to the same usage-hint route so the canonical message
    // is shown regardless of which legacy spelling the caller used.
    #[test]
    fn front_door_route_routes_dash_dash_headless_flag_to_legacy_usage_hint() {
        for args in [
            argv(&["gwt", "--headless"]),
            ...
            assert!(
                !gwt::cli::should_dispatch_cli(&args),
                "removed --headless must not fall through to CLI dispatch: {args:?}"
            );
        }
    }

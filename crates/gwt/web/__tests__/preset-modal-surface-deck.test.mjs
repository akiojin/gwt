...
test("Surface Deck — the unified Issue entry leaves 11 visible presets", () => {
...
// centers modeling the weighted deck (SURFACES 2-col x 3-row, KNOWLEDGE 2-col
...
test("Surface Deck behavioral — geometry roving picks the nearest tile in the pressed direction", () => {
  ...
  // center coords keyed by DOM order: SURFACES(0-5), KNOWLEDGE(6-8), CONFIG(9-10)
  const centers = [
    ...
    { x: 250, y: 260 }, // 5 improvement (SURFACES col2 row3)
    { x: 450, y: 100 }, // 6 issue       (KNOWLEDGE col1 row1)
    { x: 580, y: 100 }, // 7 pr          (KNOWLEDGE col2 row1)
    { x: 450, y: 180 }, // 8 index       (KNOWLEDGE col1 row2)
  ...
  assert.equal(move(3, "ArrowDown"), 5, "Board → Improvement Inbox");
  assert.equal(move(4, "ArrowRight"), 5, "Workspace → Improvement Inbox");
  assert.equal(move(4, "ArrowDown"), 4, "bottom of SURFACES clamps");
  // Cross-column to the next category at the same row.
  assert.equal(move(1, "ArrowRight"), 6, "Logs → Issue (jump to KNOWLEDGE)");
  assert.equal(move(7, "ArrowRight"), 9, "PR → Settings (jump to CONFIG)");

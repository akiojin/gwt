...
use cli::{Cli, Commands, ConfigAction, HookAction, OutputFormat};
...
fn handle_command(cmd: Commands, repo_root: &PathBuf, settings: &Settings) -> Result<(), GwtError> {
    match cmd {
        ...
        Commands::Config { action } => cmd_config(action, repo_root),
    }
}
...
/// Handle config subcommands (SPEC-a3f4c9df FR-011/FR-012)
fn cmd_config(action: ConfigAction, repo_root: &Path) -> Result<(), GwtError> {
    use gwt_core::ai::AgentHistoryStore;
    use gwt_core::config::{
        get_cleanup_candidates, BareProjectConfig, ProfilesConfig, Settings, ToolsConfig,
    };

    match action {
        ConfigAction::Cleanup { dry_run } => {
            let candidates = get_cleanup_candidates();

            // Add local candidates
            let mut local_candidates = Vec::new();
            let local_tools_json = repo_root.join(".gwt").join("tools.json");
            let local_tools_toml = repo_root.join(".gwt").join("tools.toml");
            if local_tools_json.exists() && local_tools_toml.exists() {
                local_candidates.push((local_tools_json, local_tools_toml, "JSON → TOML"));
            }

            let local_project_json = repo_root.join(".gwt").join("project.json");
            let local_project_toml = repo_root.join(".gwt").join("project.toml");
            if local_project_json.exists() && local_project_toml.exists() {
                local_candidates.push((local_project_json, local_project_toml, "JSON → TOML"));
            }

            if candidates.is_empty() && local_candidates.is_empty() {
                println!("Nothing to clean up. All config files are already migrated.");
                return Ok(());
            }
            ...
            if dry_run {
                ...
                for candidate in &candidates {
                    println!(
                        ...
                        candidate.format_change
                    );
                }
                for (old, new, change) in &local_candidates {
                    println!(
                        "  {} (migrated to {}, {})",
                        old.display(),
                        new.display(),
                ...
                let mut deleted = 0;
                for candidate in candidates {
                    if let Err(e) = candidate.cleanup() {
                        eprintln!("Failed to delete {}: {}", candidate.old_path.display(), e);
                    } else {
                        println!("Deleted: {}", candidate.old_path.display());
                        deleted += 1;
                    }
                }
                for (old, _new, _change) in local_candidates {
                    if let Err(e) = std::fs::remove_file(&old) {
                        eprintln!("Failed to delete {}: {}", old.display(), e);
                    } else {
                        println!("Deleted: {}", old.display());
                        deleted += 1;
                    }
                }
                ...
                if deleted == 0 {
                    ...
                    println!("\nCleaned up {} old config file(s).", deleted);
                }
            }
            Ok(())
        }
        ...
        ConfigAction::Status => {
            println!("gwt Configuration Status\n");
            println!("Global config locations:");

            // Settings
            if let Some(new_path) = Settings::new_global_config_path() {
                let status = if new_path.exists() {
                    "exists"
                } else {
                    "not found"
                };
                println!("  Settings (new):    {} [{}]", new_path.display(), status);
            }
            if let Some(legacy_path) = Settings::legacy_global_config_path() {
                let status = if legacy_path.exists() {
                    "exists"
                } else {
                    "not found"
                };
            ...
            // Profiles
            let profiles_toml = ProfilesConfig::toml_path();
            let profiles_yaml = ProfilesConfig::yaml_path();
            let profiles_toml_status = if profiles_toml.exists() {
                "exists"
            } else {
                "not found"
            };
            let profiles_yaml_status = if profiles_yaml.exists() {
                "exists"
            } else {
                "not found"
            };
            println!(
                "  Profiles (TOML):   {} [{}]",
                profiles_toml.display(),
                profiles_toml_status
            );
            println!(
                "  Profiles (YAML):   {} [{}]",
                profiles_yaml.display(),
                profiles_yaml_status
            );

            // Tools (global)
            if let Some(tools_toml) = ToolsConfig::global_toml_path() {
                let status = if tools_toml.exists() {
                    "exists"
                } else {
                    "not found"
                };
                println!("  Tools (TOML):      {} [{}]", tools_toml.display(), status);
            }
            if let Some(tools_json) = ToolsConfig::global_json_path() {
                let status = if tools_json.exists() {
                    "exists"
                } else {
                    "not found"
                };
                println!("  Tools (JSON):      {} [{}]", tools_json.display(), status);
            }

            // Agent History
            if let Ok(toml_path) = AgentHistoryStore::get_toml_history_path() {
                let status = if toml_path.exists() {
                    "exists"
                } else {
                    "not found"
                };
                println!("  Agent History:     {} [{}]", toml_path.display(), status);
            }
            if let Ok(json_path) = AgentHistoryStore::get_json_history_path() {
            ...
            // Local Settings
            let local_gwt_toml = repo_root.join(".gwt.toml");
            let local_gwt_dir = repo_root.join(".gwt").join("config.toml");
            let local_toml_status = if local_gwt_toml.exists() {
                "exists"
            } else {
                "not found"
            };
            let local_dir_status = if local_gwt_dir.exists() {
                "exists"
            } else {
                "not found"
            };
            println!(
                "  .gwt.toml:         {} [{}]",
                local_gwt_toml.display(),
                local_toml_status
            );
            println!(
                "  .gwt/config.toml:  {} [{}]",
                local_gwt_dir.display(),
                local_dir_status
            );

            // Local Tools
            let local_tools_toml = ToolsConfig::local_toml_path(repo_root);
            let local_tools_json = ToolsConfig::local_json_path(repo_root);
            let local_tools_toml_status = if local_tools_toml.exists() {
                "exists"
            } else {
                "not found"
            };
            let local_tools_json_status = if local_tools_json.exists() {
                "exists"
            } else {
                "not found"
            };
            println!(
                "  Tools (TOML):      {} [{}]",
                local_tools_toml.display(),
                local_tools_toml_status
            );
            println!(
                "  Tools (JSON):      {} [{}]",
                local_tools_json.display(),
                local_tools_json_status
            );

            // Local Project (bare repo)
            let project_toml = BareProjectConfig::toml_config_path(repo_root);
            let project_json = BareProjectConfig::json_config_path(repo_root);
            let project_toml_status = if project_toml.exists() {
                "exists"
            } else {
                "not found"
            };
            let project_json_status = if project_json.exists() {
                "exists"
            } else {
                "not found"
            };
            ...
            if ProfilesConfig::needs_migration() {
                println!("  - Profiles: migration needed (YAML → TOML)");
                needs_migration = true;
            }
            if ToolsConfig::needs_global_migration() {
                println!("  - Tools (global): migration needed (JSON → TOML)");
                needs_migration = true;
            }
            if ToolsConfig::needs_local_migration(repo_root) {
                println!("  - Tools (local): migration needed (JSON → TOML)");
                needs_migration = true;
            }
            if BareProjectConfig::needs_migration(repo_root) {
                println!("  - Project: migration needed (JSON → TOML)");
                needs_migration = true;
            }
            if AgentHistoryStore::needs_migration() {
                println!("  - Agent History: migration needed (JSON → TOML)");
                needs_migration = true;
            }
            if Settings::needs_global_path_migration() {
            ...
            if !needs_migration {
                ...
                println!("\nRun 'gwt config migrate' to migrate config files.");
            }

            Ok(())
        }

        ConfigAction::Migrate { force: _ } => {
            println!("Migrating config files...\n");

            let mut migrated = 0;

            // Profiles
            match ProfilesConfig::migrate_if_needed() {
                Ok(true) => {
                    println!("  Migrated: profiles.yaml → profiles.toml");
                    migrated += 1;
                }
                Ok(false) => {}
                Err(e) => eprintln!("  Failed to migrate profiles: {}", e),
            }

            // Tools (global)
            match ToolsConfig::migrate_global_if_needed() {
                Ok(true) => {
                    println!("  Migrated: tools.json → tools.toml (global)");
                    migrated += 1;
                }
                Ok(false) => {}
                Err(e) => eprintln!("  Failed to migrate global tools: {}", e),
            }

            // Tools (local)
            match ToolsConfig::migrate_local_if_needed(repo_root) {
                Ok(true) => {
                    println!("  Migrated: tools.json → tools.toml (local)");
                    migrated += 1;
                }
                Ok(false) => {}
                Err(e) => eprintln!("  Failed to migrate local tools: {}", e),
            }

            // BareProject
            match BareProjectConfig::migrate_if_needed(repo_root) {
                Ok(true) => {
                    println!("  Migrated: project.json → project.toml");
                    migrated += 1;
                }
                Ok(false) => {}
                Err(e) => eprintln!("  Failed to migrate project: {}", e),
            }

            // Agent History
            match AgentHistoryStore::migrate_if_needed() {
                Ok(true) => {
                    println!("  Migrated: agent-history.json → agent-history.toml");
                    migrated += 1;
                }
                Ok(false) => {}
                Err(e) => eprintln!("  Failed to migrate agent history: {}", e),
            }

            // Settings path
            match Settings::migrate_global_path_if_needed() {
                Ok(true) => {
                    println!("  Migrated: ~/.config/gwt/config.toml → ~/.gwt/config.toml");
                    migrated += 1;
                }
                Ok(false) => {}
            ...
            if migrated == 0 {
                println!("No migration needed. All config files are up to date.");
            } else {
                println!("\nMigrated {} config file(s).", migrated);
                println!("Run 'gwt config cleanup' to remove old files.");
            }

...
pub(crate) fn resolve_project_root(selected: &Path) -> std::path::PathBuf {
    if git::is_git_repo(selected) {
        // If selected is a worktree, this resolves to the main repository's root directory.
...
pub(crate) fn resolve_repo_path_for_project_root(
    ...
    Err(format!("Not a git repository: {}", project_root.display()))
}
    ...
    for _attempt in 0..2 {
        ...
        match state.claim_project_for_window_with_identity(
            ...
            Ok(()) => {
                ...
                // Start file-system watcher for specs/ (replaces unconditional
                // index-on-open — re-indexing now happens only on change).
                {
                    let pr = project_root_str.clone();
                    if let Err(e) = crate::index_watcher::start_index_watcher(pr) {
                        tracing::warn!(
                            category = "project_open",
                            error = %e,
                            "Failed to start index watcher"
                        );
                    }
                }

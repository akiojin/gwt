...
mod store_consolidation {
    ...
    use gwt_core::workspace_projection::{
        save_workspace_work_items_projection_to_path, WorkEvent, WorkEventKind, WorkItemsProjection,
    };
    ...
    /// AC-8 writer competition: a held lease is a refusal, not a race.
    #[test]
    fn apply_refuses_while_another_writer_holds_the_lease() {
        let fixture = SplitStoreFixture::new("https://example.invalid/acme/consolidate-lease.git");
        let plan = plan_store_consolidation(&fixture.layout_root).expect("plan");
        let canonical_store = fixture.canonical_store();
        std::fs::create_dir_all(&canonical_store).expect("canonical store");
        let lease_path = canonical_store.join("store-consolidation.lock");
        let held = std::fs::OpenOptions::new()
            ...
            .open(&lease_path)
            .expect("hold lease");
        fs2::FileExt::lock_exclusive(&held).expect("lock lease");

        let error = apply_store_consolidation(&fixture.layout_root, &plan.manifest_hash)
            .expect_err("a held lease must fail closed");
        ...
        fs2::FileExt::unlock(&held).expect("release lease");
    }

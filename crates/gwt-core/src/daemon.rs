...
/// `bind` sentinel the GUI front door persisted for a project scope before
/// Issue #2338 stopped it from claiming the daemon endpoint slot at all.
///
/// The GUI serves no IPC transport, so a registration carrying this value was
/// never connectable. gwt homes upgraded from an older build still hold them
/// on disk, so readers must classify them structurally instead of inferring
/// staleness from the pid or protocol version — those describe a GUI process,
/// not a daemon that died or fell behind (Issue #3492).
...
impl DaemonEndpoint {
    ...
    /// True when this registration is a leftover GUI front-door sentinel
    /// rather than a daemon listening on an IPC transport.
    ///
    /// See [`FRONT_DOOR_BIND`]: carrying no transport is a permanent property
    /// of the registration, independent of whether the process that wrote it
    /// is still alive or speaks the current protocol version.

                ...
                match fs2::FileExt::try_lock_exclusive(&heavy_file) {
                    Ok(()) => {
                        ...
                        let _ = fs::remove_file(
                            pending_dir
                                .join(format!("{RESERVATION_PREFIX}{}.json", self.key.file_stem())),
                        );
...
fn append_lease_event(root: &Path, event: &LeaseEvent) {
    ...
    line.push(b'\n');
    // `read(true)` is what lets LockFileEx accept the handle on Windows: an
    // append-only handle carries neither GENERIC_READ nor GENERIC_WRITE, so
    // the lock — and with it every recorded event — silently failed there.

...
describe('config/index.ts - Session Management', () => {
  ...
  describe('getAllSessions (T303)', () => {
    it('should return all valid sessions', async () => {
      (readdir as any).mockResolvedValue(['repo1_hash1.json', 'repo2_hash2.json']);
    ...
    it('should return empty array when session directory does not exist', async () => {
      (readdir as any).mockRejectedValue(new Error('ENOENT: directory not found'));
    ...
    it('should skip non-JSON files', async () => {
      (readdir as any).mockResolvedValue(['session.json', 'readme.txt', 'data.xml']);
    ...
    it('should sort sessions by timestamp (newest first)', async () => {
      (readdir as any).mockResolvedValue(['repo1.json', 'repo2.json', 'repo3.json']);
      ...
      (readFile as any).mockImplementation((path: string) => {
        if (path.includes('repo1')) return Promise.resolve(JSON.stringify(sessions[0]));
        if (path.includes('repo2')) return Promise.resolve(JSON.stringify(sessions[1]));
        if (path.includes('repo3')) return Promise.resolve(JSON.stringify(sessions[2]));
        return Promise.reject(new Error('File not found'));
      });

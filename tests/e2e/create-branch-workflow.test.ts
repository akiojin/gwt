...
`,
      ...
      expect(execa).toHaveBeenCalledWith('git', ['checkout', '-b', branchName, baseBranch]);

      // Step 4: Generate worktree path
      const repoRoot = '/path/to/repo';
      const worktreePath = await worktree.generateWorktreePath(repoRoot, branchName);
  ...
  describe('Branch Type Workflows', () => {
    it('should create all branch types successfully', async () => {
      ...
      for (const { name, base } of branchTypes) {
        ...
        expect(execa).toHaveBeenCalledWith('git', ['checkout', '-b', name, base]);
      }
    });
  });

...
`,
      ...
      expect(execa).toHaveBeenCalledWith('git', ['checkout', '-b', 'feature/new-ui', 'develop']);

      // Step 3: Create worktree for the branch
      const worktreePath = await worktree.generateWorktreePath('/path/to/repo', 'feature/new-ui');
...
`,
      ...
      expect(execa).toHaveBeenCalledWith('git', ['checkout', '-b', 'hotfix/critical-bug', 'main']);

      // Create worktree
      const worktreePath = await worktree.generateWorktreePath('/path/to/repo', 'hotfix/critical-bug');
      await worktree.createWorktree({
  ...
  describe('Branch Type Validation', () => {
    it('should handle branch name patterns correctly', async () => {
      ...
      for (const { name, baseBranch } of testCases) {
        ...
        expect(execa).toHaveBeenCalledWith('git', ['checkout', '-b', name, baseBranch]);
      }
    });
  });

...
`,
      ...
      const worktreePath = await worktree.generateWorktreePath('/path/to/repo', 'feature/test');
...
`,
    ...
    it('should handle branch selection errors gracefully', async () => {
      ...
      await expect(git.getAllBranches()).rejects.toThrow('Failed to get local branches');
    });

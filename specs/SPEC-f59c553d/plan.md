...
npm postinstallのGitHub Releasesダウンロードを安定化する。`package.json` バージョンから `vX.Y.Z` のアセットURLを生成し、API取得に失敗した場合のフォールバックを用意する。HTTP 404/403/5xx等の一時的失敗は指数バックオフで再試行し（最大5回、初回0.5秒、倍率2、上限5秒）、失敗時は英語の復旧ガイダンスを表示する。指定バージョンのアセットが無い場合でも `latest` へはフォールバックしない。テストはNode標準の `node:test` でURL生成とリトライ判定を検証する。
...
- **ユニットテスト**: URL生成、リトライ判定、エラーメッセージ生成

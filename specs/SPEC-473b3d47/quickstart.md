...
**ステップ1**: ブランチ一覧画面で `c` キーを押下

**ステップ2**: 自動クリーンアップの進捗と結果を確認

```text
🔄 PRクリーンアップを実行しています...
✅ feature/user-auth をクリーンアップしました。
⏭️ hotfix/bug-456: 未コミットまたは未プッシュの変更があるためスキップしました。
```

**ステップ3**: 成功時は一覧が更新され、対象がなくなると次のメッセージが表示される

```text
✅ クリーンアップ対象のマージ済みPRはありません。
```

**補足**: 全ての処理が完了すると、結果アイコン（✅/⏭️/❌）を表示したまま3秒間待機し、その後カーソル表示に復帰して削除済みブランチを一覧から取り除きます。

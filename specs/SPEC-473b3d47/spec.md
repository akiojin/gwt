...
> **2025-10-26 更新:** `-c`, `-r/--resume`, `--tool` などのCLIフラグは廃止され、実行モードおよびツール選択はすべてInk UI内で行います。
...
開発者が前回の作業を中断した後、同じワークツリーで作業を再開したいとき、起動直後の「実行モード」画面で「続きから」または「セッション再開」を選択し、記録されたセッションから復元できます。
...
1. **前提条件** GitHub CLI認証済み、**操作** メニューから「マージ済みPRクリーンアップ」を選択、**期待結果** リモートから最新情報を取得し、マージ済みPRのブランチ一覧が表示される
2. **前提条件** クリーンアップ対象が表示されている、**操作** 削除対象を複数選択、**期待結果** 選択した項目のワークツリーとブランチが削除される
3. **前提条件** 未プッシュのコミットがある、**操作** クリーンアップを実行、**期待結果** プッシュするか確認され、選択に応じてプッシュまたはスキップされる
4. **前提条件** リモートブランチ削除を選択、**操作** 確認して実行、**期待結果** ローカルだけでなくリモートブランチも削除される
...
開発者がClaude Code または Codex CLI を選択して起動したいとき、InkベースのUIでツールと実行モード（通常/continue/resume）および権限設定を選択できます。
...
3. **前提条件** 実行モード画面が表示されている、**操作** 「通常」を選択し、AIツール選択画面でClaude Codeを選択、**期待結果** Claude Codeが起動対象として確定し、続く確認なしに起動される
...
- **FR-008**: システムは、Claude Code と Codex CLI の両方をサポートし、Ink UI 内で選択および起動できなければならない
...
- **FR-018**: システムは、マージ済みPRに対応するブランチとワークツリーを自動検出しなければならない
- **FR-019**: システムは、複数のクリーンアップ対象を選択して一括削除できなければならない
- **FR-020**: システムは、未プッシュのコミットがある場合、プッシュするか確認しなければならない
- **FR-021**: システムは、ローカルブランチとリモートブランチを個別または同時に削除できなければならない

#### リリース管理

- **FR-022**: システムは、releaseブランチ作成時にバージョンバンプタイプ（major/minor/patch）を選択できなければならない
- **FR-023**: システムは、package.jsonから現在のバージョンを読み取り、新しいバージョンを計算しなければならない
- **FR-024**: システムは、releaseブランチのワークツリーでpackage.jsonのバージョンを自動更新しなければならない
- **FR-025**: システムは、リリース作業完了時にリリースアクション（complete/continue/nothing）を選択できなければならない
- **FR-026**: システムは、「complete」選択時にブランチをプッシュし、mainブランチへのPRを自動作成しなければならない
- **FR-027**: システムは、PR作成後にタグ作成とdevelopへのマージバック手順を表示しなければならない

#### 変更管理

- **FR-028**: システムは、AIツール終了後に未コミットの変更を検出しなければならない
- **FR-029**: システムは、変更に対してcommit/stash/discard/continueのアクションを選択できなければならない
- **FR-030**: システムは、commitアクション選択時にコミットメッセージを入力できなければならない
- **FR-031**: システムは、discardアクション前に確認を求めなければならない
- **FR-032**: システムは、リリースブランチの変更を自動的にコミットしなければならない

#### ユーザーインターフェース

- **FR-033**: システムは、すべての対話的操作でキャンセル（Ctrl+C、ESC）を適切にハンドリングしなければならない
- **FR-034**: システムは、エラーメッセージ、成功メッセージ、情報メッセージを視覚的に区別して表示しなければならない
...
- **クリーンアップ対象（CleanupTarget）**: マージ済みPRのブランチとワークツリーを表し、ブランチ名、ワークツリーパス、PR番号、PR URL、未プッシュコミットの有無、リモートブランチの有無、クリーンアップタイプ（worktree-and-branch/branch-only）の属性を持つ

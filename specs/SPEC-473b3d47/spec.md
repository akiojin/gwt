...
6. **前提条件** 新規または再利用するワークツリーのルートに`bun.lock`・`pnpm-lock.yaml`・`package-lock.json`のいずれかが存在、**操作** ワークツリーを作成または開く、**期待結果** ツールがロックファイルを優先順（bun→pnpm→npm）で検出し、対応する `bun install --frozen-lockfile` / `pnpm install --frozen-lockfile` / `npm ci` を自動実行して完了ログまたはエラーを表示する
...
- **依存関係インストールに失敗 / ロックファイル未検出**: `bun.lock` / `pnpm-lock.yaml` / `package-lock.json` のいずれも存在しない、または自動実行した `bun install --frozen-lockfile` / `pnpm install --frozen-lockfile` / `npm ci` が失敗した場合、エラー内容を表示し、ユーザーが手動でコマンドを再実行できるように指示する
- **パッケージマネージャーバイナリが存在しない**: ロックファイルは存在するが `bun` / `pnpm` / `npm` のいずれもPATH上にない場合、自動インストールを警告付きでスキップし、ユーザーへ手動インストールの必要性を通知する
...
- **FR-038**: システムは、ワークツリーを新規作成または再利用する際に、対象ディレクトリ直下のロックファイルを優先順（`bun.lock` → `pnpm-lock.yaml` → `package-lock.json`）で検出し、最初に見つかったロックファイルに応じて `bun install --frozen-lockfile` / `pnpm install --frozen-lockfile` / `npm ci` を自動実行しなければならない。実行中はスピナーまたは進捗ログを表示し、コマンドが失敗した場合はエラーログと再実行手順を表示してAIツール起動をブロックしなければならない。対象のパッケージマネージャーバイナリが環境に存在しない場合（例: `bun` 未インストール）は、警告を表示した上で自動インストールをスキップし、AIツール起動を継続しなければならない。
...
- **PR-001**: すべての新機能およびバグ修正は Spec Kit に従ったTDD（Red→Green→Refactor）で実装し、テスト（ユニット/統合/E2E）を先に追加してから実装コードを変更しなければならない。テストが存在しない機能を変更する場合は、失敗するテストを追加してから着手すること。

...
- 追加で、タブの並び替え D&D が Tauri WebView で no-op になる不具合があり、タブ操作の基本 UX を阻害している
...
### ユーザーストーリー 8 - タブ並び替えD&Dの安定化 (優先度: P0)

ユーザーとして、タブをドラッグして順番を入れ替える操作が、Tauri 実行時でも確実に動作してほしい。

**独立したテスト**: `pointermove` が `window` に配信されたケースでも `onTabReorder` が発火すること

**受け入れシナリオ**:

1. **前提条件** 3つ以上のタブが開いている、**操作** タブをドラッグして別タブ上へ移動、**期待結果** ドロップ位置（before/after）に従って順序が更新される
2. **前提条件** タブをドラッグ中にポインターがタブバー外を経由する、**操作** 継続して対象タブ上に移動、**期待結果** 並び替えが継続して機能する（`window` レベルの pointer イベントで追跡）
3. **前提条件** タブ上の close ボタンをクリック、**操作** ボタン押下、**期待結果** ドラッグ開始扱いにならず、誤った並び替えが発生しない

### ユーザーストーリー 9 - トラックパッド/ホイールで長文を確実にスクロール (優先度: P0)

ユーザーとして、出力行数が多いエージェントタブでもトラックパッドやホイールで途切れなく上下スクロールしたい。

**独立したテスト**: 1000 行超の出力が蓄積されたターミナル画面で wheel 入力によるスクロール位置更新を観測できること

**受け入れシナリオ**:

1. **前提条件** エージェントタブの `xterm-viewport` にスクロール余地がある、**操作** トラックパッドで上下スクロール、**期待結果** `scrollTop` が連続して変化し、目的位置まで遷移できる
2. **前提条件** `deltaX` と `deltaY` が同時発生する wheel イベント、**操作** 何度もスクロール、**期待結果** 主要軸の移動量で `scrollTop` が変化する
3. **前提条件** 画面上部に到達している状態でさらに上方向に wheel、**操作** さらにスクロール、**期待結果** `viewport` 側で変化しないため、イベントは terminal 側で prevent されず親へ自然に伝播する
...
- タブドラッグ中にポインターがタブバー外へ出る場合 → `window` の pointermove/up/cancel を購読し追跡を継続する
...
- **FR-013**: タブ並び替え D&D は、ドラッグ中の pointermove/up/cancel を `window` 単位で購読し、タブバー外を経由しても動作継続できなければならない
- **FR-014**: タブ close ボタン押下時はドラッグ状態を開始してはならない
- **FR-015**: D&D 回帰防止として `gwt-gui/src/lib/components/MainArea.test.ts` に `window` 配信 pointermove の自動テストを含めなければならない
- **FR-016**: エージェントタブの wheel イベントは `deltaX` または `deltaY` の非零値を検知した場合にローカルで `xterm-viewport` スクロールにフォールバックし、`deltaY=0` のイベントを含む再現条件でも反応すること

### 非機能要件

- **NFR-001**: OSC 7 パースの処理コストは PTY I/O ループ内で最小限に抑える（正規表現ではなくバイトスキャンを使用）
- **NFR-002**: ターミナル起動から入力可能状態まで 500ms 以内
- **NFR-003**: タブD&Dの挙動はブラウザ依存の DragEvent 実装差異に影響されず、pointer イベントベースで一貫して動作する
...
- **SC-006**: Tauri WebView 実行時に、タブD&Dで順序変更が反映され、`window` 経由 pointermove ケースの自動テストが成功する
- **SC-007**: `deltaY=0, deltaX!=0` を含む wheel 入力でも、長文表示中のエージェントタブで `scrollTop` が変化し継続スクロールが可能であること

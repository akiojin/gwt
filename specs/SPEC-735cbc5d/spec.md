...
4. **Given** 基準ブランチを `main` から `develop` に切り替える際に旧リクエストが失敗,
   **When** 旧リクエストの応答が遅れて返る, **Then** Git タブと Worktree Summary 全体は表示を維持し、
   最新選択（`develop`）のみ反映される。
5. **Given** 基準ブランチ切り替え時に Git 取得エラーが発生, **When** 再描画される,
   **Then** エラーは Git タブ内に表示され、他タブとパネル全体は利用可能なままである。
...
- **基準ブランチ切り替え競合**: 旧リクエスト（summary/changes/commits）の遅延応答は無視し、
  最新選択の結果のみ描画
- **基準ブランチ候補外値**: 候補に存在しない値が入力されてもクラッシュせず、
  有効候補へフォールバック
- **基準ブランチ切り替え失敗**: 例外は Git タブ内エラー表示に閉じ込め、Worktree Summary 全体を消失させない
...
- **FR-018**: 基準ブランチ切り替え時に例外が発生しても、Worktree Summary 全体を消失させてはならない。
- **FR-019**: 基準ブランチ切り替え中に旧リクエストの応答が遅れて返ってきた場合、
  最新リクエスト以外の結果は破棄しなければならない。
- **FR-020**: 基準ブランチ変更イベントで候補外値が渡された場合、候補値へフォールバックし、
  例外を送出してはならない。
- **FR-021**: 基準ブランチ切り替え時の取得失敗は Git タブ内のエラー表示に限定し、
  他タブや親パネルを巻き込んだ失敗にしてはならない。
...
- **SC-011**: エッジケース（upstream 未設定、0 コミット、大規模 diff、
  バイナリファイル）が正しく処理される。
- **SC-012**: `main -> develop` への基準ブランチ切り替え時に、UI 全体が白画面化しない。
- **SC-013**: 基準ブランチ連続切り替え時に、最終選択の結果のみ表示される。
- **SC-014**: 基準ブランチ切り替え失敗時に、エラーが Git タブ内に留まり、
  Worktree Summary の他タブ操作が継続できる。

...
18. For agent panes that still require local snapshot fallback, preserve distinct clear+redraw and qualified home-repaint frames even when the event loop coalesces multiple PTY chunks into one payload, so fallback history does not collapse to a single final frame.

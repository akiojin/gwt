...
2. **前提条件** エラーポップアップが表示されている、**操作** ポップアップ内容を確認、**期待結果** エラーコード（GWT-E001形式）、メッセージ、詳細セクション、サジェスチョンセクションが表示される
...
エラーポップアップ表示中に[l]キーを押すと、ログビューアー画面に遷移し、該当エラーが選択された状態で表示される。

**この優先度の理由**: エラーの詳細な履歴や文脈を確認するためのナビゲーションとして必須。

**独立したテスト**: エラーポップアップで[l]キーを押し、ログ画面に遷移して該当エラーが表示されることを確認できる。

**受け入れシナリオ**:

1. **前提条件** エラーポップアップが表示されている、**操作** [l]キーを押す、**期待結果** ポップアップが閉じ、ログビューアー画面に遷移する
2. **前提条件** ログビューアー画面に遷移した、**操作** ログ一覧を確認、**期待結果** 直前のエラーが選択された状態で表示される
3. **前提条件** ログビューアーでエラーを確認中、**操作** Escキーで戻る、**期待結果** 元のブランチ一覧画面に戻る
...
- **FR-003**: エラーコードはGWT-EXXX形式（例: GWT-E001, GWT-E002）で付与**しなければならない**
...
- **ErrorCode**: エラーの一意識別子（GWT-EXXX形式）、カテゴリとサジェスチョンを持つ

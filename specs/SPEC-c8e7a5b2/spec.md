...
**ステータス**: 廃止
**廃止日**: 2025-12-14
**廃止理由**: CLI/Web UI分離設計への移行により無効化

## 廃止経緯

この仕様は CLI と Web UI の同時利用を目的として設計されましたが、以下の理由により廃止されました:

1. **関心の分離**: CLI と Web UI サーバーは独立したコンポーネントとして動作すべき
2. **リソース効率**: Web UI を使用しないユーザーに対して不要なサーバープロセスを起動しない
3. **明示的な起動**: ユーザーが Web UI を必要とする場合、明示的にサーバーを起動する

## 代替手段

- `gwt serve` コマンドでブロッキングモードで Web UI サーバーを起動
- `npm run start:web` または `bun run start:web` で開発用サーバーを起動

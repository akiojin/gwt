...
- **FR-048**: エージェントペインにフォーカス中、マウススクロールを以下のようにPTYに送信**しなければならない**:
  - PTY内アプリがマウスプロトコルを有効にしている場合: X10プロトコルのスクロールバイト列を送信
  - マウスプロトコル無効時: 矢印キー（ScrollUp→Up×3、ScrollDown→Down×3）をPTYに送信

...
| Header (gwt - Screen)     |                           |
|---------------------------|   エージェントペイン（右側）  |
|   gwt UI（左側）           |   [Tab1|Tab2|Tab3]        |
|   BranchList              |   エージェント出力          |
|   AgentMode (タスク一覧)   |                           |
|   Settings                |   ステータスバー           |
|   GitView等               |   branch [agent] Running  |
|---------------------------|                           |
| Footer (keybinds)         |                           |
| StatusBar                 |                           |
+---------------------------+---------------------------+
...
- **左側**: 既存gwt UI。ヘッダー/フッター/ステータスバーを含む。Tab切替でBranchList/AgentMode/Settings間を遷移（FR-120〜FR-122）
- **右側**: エージェントペイン（FR-047）。常時表示（FR-046）。ヘッダー/フッター不要（FR-121）
...
- **FR-120**: 全Screenにおいて常時左右分割レイアウトを適用**しなければならない**（左側: Screen別UI、右側: エージェントペイン）
- **FR-121**: ヘッダー、フッター、ステータスバーは左ペイン内にのみ表示**しなければならない**（右側エージェントペインにはヘッダー/フッター不要）
- **FR-122**: Tab キーで BranchList → AgentMode → Settings → BranchList の左ペイン内画面切替を行え**なければならない**（エージェントペインは常に右側に表示され続ける）
- **FR-123**: AgentMode画面のタスクパネルにPaneManagerの実行中エージェント一覧を表示**しなければならない**
- **FR-124**: AgentMode画面でUp/Downキーによるタスク選択、Enterキーでエージェントペインのアクティブタブ切替を行え**なければならない**
- **FR-125**: AISettingsWizard画面では左ペイン全体を使用し、ヘッダー/フッター/ステータスバーを非表示**にしなければならない**

...
### ユーザーストーリー 4 - セッション内容の欠落を最小化できる (優先度: P1)

開発者がセッション変換やプレビューを行う際、各エージェントのセッションに含まれる `tool_use` / `tool_result` / `thinking` / `parts` などの要素が欠落せずに取り込まれ、内容の連続性を保ったまま確認・変換できる。

**この優先度の理由**: 欠落が多いと変換結果が無意味になり、再利用できない。

**独立したテスト**: 代表的なセッション断片を用意し、解析結果に `tool_result` や `thinking` の内容が含まれることを検証できる。

**受け入れシナリオ**:

1. **前提条件** `tool_result` を含むセッションがある、**操作** セッションを解析、**期待結果** 解析済み内容に `tool_result` の情報が含まれる。
2. **前提条件** `thinking` を含むセッションがある、**操作** セッションを解析、**期待結果** 解析済み内容に `thinking` の情報が含まれる。
3. **前提条件** `parts` 形式のセッションがある、**操作** セッションを解析、**期待結果** 解析済み内容に `parts` のテキストが含まれる。
...
- `tool_result` の内容がテキスト以外の場合でも、欠落しないよう英語のプレースホルダで保持する。
- `parts` に画像/バイナリが含まれる場合でも、英語のプレースホルダで存在を示す。
...
- **FR-014**: システムは `tool_use` / `tool_result` / `thinking` を含むセッション内容を欠落させずに抽出**しなければならない**。
- **FR-015**: システムは `parts` 形式のセッション内容を抽出**しなければならない**。
- **FR-016**: システムは画像/バイナリなどテキスト以外の要素に英語のプレースホルダを付与**しなければならない**。
...
- **SC-007**: `tool_result` / `thinking` / `parts` の内容が欠落せずに抽出される。
...
- 各エージェントのセッションフォーマット差異はパーサー側の拡張で吸収できる。

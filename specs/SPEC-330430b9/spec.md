...
開発者がセッション変換の一覧を開いたとき、セッションIDだけでなくセッション名と開始ユーザーメッセージを一覧に表示して、変換対象を見分けられる。
...
2. **前提条件** セッション名が空または存在しないセッションがある、**操作** 一覧を表示、**期待結果** セッション名欄に英語のプレースホルダが表示され、一覧は崩れない。
...
1. **前提条件** セッション一覧が表示されている、**操作** Spaceキーを押す、**期待結果** プレビュー画面が表示され、ヘッダーにセッション名が表示される。
...
- セッション名が取得できない場合、英語のプレースホルダを表示する。
...
- **FR-011**: システムはセッション変換の一覧行にセッション名を表示**しなければならない**。
- **FR-012**: システムはセッションプレビューのヘッダーにセッション名を表示**しなければならない**。
- **FR-013**: システムはセッション名が取得できない場合に英語のプレースホルダを表示**しなければならない**。

### 主要エンティティ *(機能がデータを含む場合は含める)*

- **ConvertSessionEntry**: 変換一覧の表示用データ（セッション名、更新日時、開始ユーザーメッセージ抜粋など）。
- **SessionPreview**: プレビュー画面に表示するセッション内容（先頭10メッセージ、ヘッダーにエージェント名・セッション名・更新日時）。

...
### ユーザーストーリー 6: エージェント→マスター連携（優先度: 高）

サブエージェントがMCP経由でマスターエージェントに計画依頼・進捗報告を送信できる。

#### 受け入れシナリオ

**シナリオ 6.1**: サブエージェントからマスターへの入力送信

- **前提条件**: マスターエージェント画面が起動中
- **操作**: `gwt_master_send({ command_type: "input_text", payload: { text: "進捗報告" } })`
- **期待結果**: マスターの会話キューに受理され、`accepted=true`, `status=queued` が返る

**シナリオ 6.2**: マスター状態取得

- **操作**: `gwt_master_get_state({ include_messages: true, limit: 20 })`
- **期待結果**: マスター状態（`ai_ready`, `is_waiting`, `messages` 等）が返る

**シナリオ 6.3**: MCP登録不備時のガード

- **前提条件**: MCPランタイム未検出または一部エージェント設定未登録
- **操作**: `gwt_master_send(...)` を呼び出す
- **期待結果**: 実行は拒否され、`accepted=false` と `suggested_action=repair_mcp_registration` が返る

**シナリオ 6.4**: MCP登録状態の取得と修復

- **操作**: `gwt_master_get_mcp_registration_status()` / `gwt_master_repair_mcp_registration()`
- **期待結果**: runtime/script/agent登録状態を返し、修復後は `overall=ok` へ収束する
...
- **FR-022**: `gwt_master_send(command_type, payload, ...)` はサブエージェントからマスターエージェントへの制御メッセージ送信を処理しなければ**ならない**
- **FR-023**: `gwt_master_get_state(include_messages, limit)` はマスターエージェント状態を返却しなければ**ならない**
- **FR-024**: `gwt_master_get_mcp_registration_status()` は MCP 登録ヘルス（runtime/script/agents/overall）を返却しなければ**ならない**
- **FR-025**: `gwt_master_repair_mcp_registration()` は stale state cleanup + 再登録を実行し、最新ヘルスを返却しなければ**ならない**
- **FR-026**: MCP登録ヘルスが `ok` でない場合、`gwt_master_send` は処理を拒否し、修復アクションを案内しなければ**ならない**
- **FR-027**: Settings UI から `get_mcp_registration_status_cmd` / `repair_mcp_registration_cmd` を呼び出して状態確認・修復できなければ**ならない**
- **FR-028**: gwt はユーザー単位で単一起動でなければ**ならない**。2つ目の起動は既存インスタンスを前面化して終了しなければ**ならない**
- **FR-029**: 単一起動ロックはクラッシュ後に自動回復可能でなければ**ならない**（stale lock file が残っても次回起動を阻害しない）
...
| `gwt_master_send` | `command_type: string, payload?: object, text?: string` | `{ accepted: boolean, command_id: string, status: string }` |
| `gwt_master_get_state` | `include_messages?: boolean, limit?: number` | `AgentModeState` |
| `gwt_master_get_mcp_registration_status` | なし | `McpRegistrationStatus` |
| `gwt_master_repair_mcp_registration` | なし | `McpRegistrationStatus` |

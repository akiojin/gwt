...
- [x] The ranked Specs search path now has focused coverage for artifact-aware ordering, `selected_spec()` resolution through search results, score/snippet rendering, and detail-view search guarding.
- [x] Each remaining gap is completion-gate review only; no code-side execution gap remains.

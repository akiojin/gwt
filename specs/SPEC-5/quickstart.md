...
5. From Specs detail, press `e` to edit phase, `s` to edit status, and `Ctrl+e` to open a raw edit buffer for the active artifact file.
6. Track semantic search, markdown-rendered detail parity, and section-scoped markdown editing as the remaining execution steps.

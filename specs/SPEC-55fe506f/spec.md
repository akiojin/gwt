...
4. **前提条件** ブランチ一覧画面が表示されている、**操作** 保護ブランチ(main/develop)にカーソルを合わせてスペースキーを押す、**期待結果** 選択され、`*`マーカーが表示される
...
2. **前提条件** プッシュされていないコミットがあるブランチ、**操作** 画面を確認する、**期待結果** 黄色の`!`アイコンが表示される
3. **前提条件** 未コミット変更があるブランチ、**操作** 画面を確認する、**期待結果** 赤色の`!`アイコンが表示される
4. **前提条件** マージされていないブランチ、**操作** 画面を確認する、**期待結果** 黄色の`*`アイコンが表示される
5. **前提条件** 安全に削除可能なブランチ、**操作** 画面を確認する、**期待結果** 緑色の`o`アイコンが表示される
6. **前提条件** 複数のブランチが選択されている、**操作** 画面下部を確認する、**期待結果** "選択中: N個のブランチ"というメッセージがリアルタイムで表示される
...
- 保護ブランチ(main/develop)も選択対象とし、チェック済みの場合はクリーンアップ対象に含める
- クリーンアップ実行中に選択状態を変更できないようにする(入力ロック)
- クリーンアップ実行中にリフレッシュ操作(r)が行われても、クリーンアップ中の表示と入力ロックを維持する
- クリーンアップ対象のブランチ行は薄い灰色の背景色で表示し、操作不可であることを視覚的に明示する
...
- **FR-003**: システムはブランチの安全ステータスを以下のアイコンと色で表示**しなければならない**:
  - 未コミット変更あり: `!` (赤色) - 最も危険
  - 未プッシュコミットあり: `!` (黄色) - 警告
  - 未マージ: `*` (黄色) - 警告
  - 安全（削除可能）: `o` (緑色)
...
- **FR-004b**: システムはupstreamが未設定のブランチでもmainブランチとの差分を確認し、適切な安全ステータスを判定**しなければならない**
- **FR-005**: システムは保護ブランチ(main/develop)を選択対象に含め、チェック済みの場合はクリーンアップ対象に含め**なければならない**
...
- **FR-013**: システムはクリーンアップ実行中のブランチ行を薄い灰色（DarkGray）の背景色とテキスト色で表示し、カーソル選択・マウス選択が無効であることを視覚的に明示**しなければならない**
- **FR-014**: システムは全てのクリーンアップ処理が完了した後、ブランチリストを自動的に更新**しなければならない**
- **FR-015**: システムはクリーンアップ実行中にリフレッシュ操作が行われても、クリーンアップ中の表示と入力ロックを継続**しなければならない**
...
- **SC-003**: 誤選択(0個選択での実行)が100%防止される
...
- 保護ブランチ(main/develop)も選択対象となるため、選択時の警告や確認によって誤削除リスクを抑制する必要がある

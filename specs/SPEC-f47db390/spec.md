...
### ユーザーストーリー 6 - Quick Startは選択ブランチの最新セッションを確実に提示する (優先度: P1)

開発者がQuick StartでResumeを選ぶとき、表示されるセッションIDは**選択ブランチのworktreeに紐づく最新セッション**であり、他ブランチのセッションが混在しない。

**この優先度の理由**: ブランチを跨いだ誤セッション再開は作業ミスにつながるため、Quick Startの信頼性を最優先で担保する必要がある。  
**独立したテスト**: ブランチA/Bでそれぞれセッションを開始→終了し、ブランチAを選択してQuick Startを開く。AのResume行にはAのセッションIDのみが表示され、BのIDは表示されないことを確認する。

**受け入れシナリオ**:
1. **前提条件** ブランチA/Bにそれぞれセッションが存在、**操作** ブランチAを選択してQuick Startを表示、**期待結果** Resume行にブランチAのセッションIDのみが表示され、ブランチBのIDは表示されない。
2. **前提条件** Quick Start履歴のsessionIdが古い、**操作** Quick Startを表示、**期待結果** セッションファイル再走査により最新IDが表示される。
3. **前提条件** 選択ブランチに対応するworktreeが存在しない、**操作** Quick Startを表示、**期待結果** 履歴のみで表示し、gwt起動ディレクトリに基づく再走査は行わない。
...
- 選択ブランチに一致するworktreeが無い/複数存在する場合のQuick Start表示と再走査の扱い。
...
- **FR-013**: Quick Start/Continue/Resumeに利用するセッションID解決は、**選択ブランチとそのworktree**に基づいて行い、gwt起動ディレクトリに依存してはならない。他ブランチのセッションIDが混在してはならない。
- **FR-014**: Quick Start表示時はツール別セッションファイルを再走査し、履歴より新しいセッションIDがあれば更新して表示しなければならない。再走査に失敗した場合は履歴表示へフォールバックし、sessionIdが無い場合はResume行を表示してはならない。
...
- **SC-006**: Quick StartのResume行が選択ブランチのセッションIDのみを表示し、他ブランチのIDが混在しないことをテストで確認。
- **SC-007**: Quick Start表示時のセッション再走査が機能し、履歴が古い場合に最新IDへ更新されること、IDが無い場合にResumeが表示されないことをテストで確認。
...
- セッション再解決は選択ブランチのworktreeに限定し、gwtの起動ディレクトリや他ブランチのworktreeを参照しない。

...
4. **前提条件** gwt GUIが起動しておりシステムトレイに常駐している、**操作** OS標準の終了操作（例: macOSの Cmd+Q / アプリメニューのQuit）を実行、**期待結果** アプリは終了せず常駐し、必要に応じてウィンドウが非表示になる（完全終了はトレイの「Quit」のみ）
...
### ユーザーストーリー 4 - ウィンドウタイトルにプロジェクトディレクトリを表示 (優先度: P2)

開発者として、開いているプロジェクトのディレクトリをウィンドウタイトルで確認し、
複数のウィンドウ/モニタ運用でも迷子にならないようにしたい。

**受け入れシナリオ**:

1. **前提条件** Open Project でプロジェクトを開く、**操作** タイトルバーを確認する、**期待結果** タイトルが `gwt - <projectPath>` 形式で表示される
2. **前提条件** プロジェクトが開かれている、**操作** Close Project を実行する、**期待結果** タイトルが `gwt` に戻る
...
- Open Projectで、Gitリポジトリではないディレクトリ（gwtの作業ルート等）を選択した場合でも、配下にbareリポジトリ（`*.git`）が存在すれば自動検出して開けること
...
- OS標準の終了操作（例: macOSの Cmd+Q / アプリメニューのQuit / 最後のウィンドウクローズによる終了要求）が発生した場合でも、トレイ「Quit」以外では終了しないこと
- Release ビルドでは、WebView の右クリックメニューに「Inspect Element」等の開発者向け項目が表示されないこと
...
- **FR-314**: トレイ「Quit」以外の終了要求（OS標準の終了操作など）ではアプリを終了**してはならない**
...
- **FR-323**: プロジェクトを開いている間、ウィンドウタイトルに `gwt - <projectPath>` を表示しなければ**ならない**
- **FR-324**: プロジェクト未選択時はウィンドウタイトルを `gwt` としなければ**ならない**

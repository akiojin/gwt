# 調査: npm postinstall + bunxオンデマンド取得のダウンロード安定化
...
- `bin/gwt.js` はオンデマンド取得で `releases/latest` を参照しており、bunxで指定バージョンが無視される。
...
- bunxオンデマンド取得（`bin/gwt.js`）はタグURLのみを使用し、リトライは行わない。
- `scripts/` 配下の共通モジュールにURL生成とバージョン取得を集約する。

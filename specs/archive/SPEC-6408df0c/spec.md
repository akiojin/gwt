...
### ユーザーストーリー 3 - commit-msgでcommitlintを自動取得 (優先度: P2)

開発者が `git commit` を実行した際に、`node_modules` が無い状態でも commitlint が自動取得され、コミットメッセージの検証が行われる。

**この優先度の理由**: commitlint をローカルで必ず通し、CIの差し戻しを防ぐため。

**独立したテスト**: `.husky/commit-msg` が `bunx --package @commitlint/cli` を使用していることを検証する。

**受け入れシナリオ**:

1. **前提条件** `node_modules` が無い、**操作** `git commit` を実行、**期待結果** commitlint が自動取得され検証が完了する

---

### ユーザーストーリー 4 - 失敗時に明確な英語エラー (優先度: P3)
...
- **node_modules未配置**: commit-msg でも自動取得で検証を継続する
...
- **FR-008**: システムは`commit-msg`フックで`bunx --package @commitlint/cli`を使用し、commitlintを自動取得**しなければならない**
...
- **SC-003**: `node_modules` が無くても commitlint の検証が実行できる

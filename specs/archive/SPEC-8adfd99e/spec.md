...
**この優先度の理由**: 現状は `~/.gwt/tools.json` を直接開かない限り、どのツールにどの環境変数が設定済みか把握できないため、ブラウザ上での可視化が第一歩となる。
...
- `~/.gwt/tools.json` が存在しない / 破損している場合は、空状態 UI とエラーバナーを出し、復旧手順リンクを表示する
...
- **FR-002**: `GET /api/config` は `~/.gwt/tools.json` の `customTools` セクションを読み込み、各 `env` を配列（key/value/lastUpdated）として返却しなければならない
...
- `gwt serve` を実行している OS ユーザーが `~/.gwt/tools.json` へ読み書きできる必要がある

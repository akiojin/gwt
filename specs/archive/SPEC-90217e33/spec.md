...
6. **前提条件** 対象ブランチにキャッシュ済みのAI要約がある（かつセッションファイルは更新済み）、**操作** Summary を開く、**期待結果** キャッシュ要約が即座に表示され、`Updating...` のような更新中表示が出る。更新完了後、自動で最新要約に差し替わる（UI は固まらない）

---

### ユーザーストーリー 6 - エージェント終了後に結果を確認してからタブを閉じる (優先度: P0)

開発者として、エージェントが終了してもタブが勝手に閉じず、終了表示を確認した上で `Enter` でタブを閉じたい。

**独立したテスト**: 終了後の `Enter` 入力判定（Enterのみ・終了時のみでクローズ）がユニットテストで検証できる。

**受け入れシナリオ**:

1. **前提条件** いずれかのエージェントを起動し、プロセスが終了する、**操作** 画面を確認、**期待結果** タブは自動で閉じず、終了表示が確認できる
2. **前提条件** 同上、**操作** ターミナル出力末尾を確認、**期待結果** `Press Enter to close this tab.` が表示される
3. **前提条件** 同上、**操作** タブ上で `Enter` を押す、**期待結果** 該当タブが閉じる（他タブは維持される）
...
- エージェント終了後のタブは自動で閉じない（終了表示を確認できる）。ユーザーが `Enter` を押したときに該当タブを閉じる。
- AI 要約が生成できない場合（例: backend が Responses API をサポートしない等）は、過剰に待たずにエラーとして提示する。キャッシュ要約がある場合はそれを保持し、更新失敗が分かるように表示する。
...
- **FR-023**: エージェント（PTYプロセス）が終了しても、該当タブを自動で閉じては**ならない**
- **FR-024**: エージェント終了時、ターミナル出力末尾に `Press Enter to close this tab.` を表示しなければ**ならない**
- **FR-025**: エージェントが終了しているタブでユーザーが `Enter` を入力した場合、該当タブを閉じなければ**ならない**

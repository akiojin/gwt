...
**入力**: ユーザー説明: "tmux必須化を取りやめ、シングルモードを復活したい。"

**BREAKING CHANGE**: なし（tmux必須化を撤回）

## 概要

gwtはtmux環境内ではマルチモードで動作し、tmux外ではシングルモードで動作する。tmux外でもエラー終了せず、従来の単一エージェント起動フローを提供する。1ブランチに対して1ペインのみ許可し、ブランチリストの右側にエージェント情報（スピナー、エージェント名、稼働時間）を表示する。PaneListパネルは廃止し、ブランチリストに統合する。

gwtは左列に固定し、右側にエージェント用のペインを1つだけ配置する（シングルアクティブペイン制約）。画面構成は常に「gwt（左）+ エージェント（右）」の最大2ペイン。新しいエージェントを起動すると、既存のアクティブペインは自動的に非表示になり、新しいペインがアクティブになる。非表示のペインはブランチ選択+Enterで再表示できる。

## ユーザーシナリオとテスト *(必須)*

### ユーザーストーリー 1 - tmux自動判定 (優先度: P0)

開発者がgwtを起動したとき、tmux環境内ではマルチモード、tmux環境外ではシングルモードで起動する。

**この優先度の理由**: tmux強制が難しい環境でもgwtを利用可能にするため。

**独立したテスト**: tmux内外でgwtを起動し、それぞれのモードで動作することを確認する。

**受け入れシナリオ**:

1. **前提条件** tmux環境外でgwtを起動、**操作** gwtを起動、**期待結果** シングルモードで起動し、ブランチリストが表示される
2. **前提条件** tmux環境内でgwtを起動、**操作** gwtを起動、**期待結果** マルチモードで起動し、ブランチリストが表示される
3. **前提条件** TMUX環境変数が設定されていない、**操作** gwtを起動、**期待結果** シングルモードで起動する
...
1. **前提条件** ブランチAでエージェントがアクティブ表示中、**操作** ブランチAで再度Enterを押す、**期待結果** 既存のペインにフォーカスが移動する（新規起動しない）
2. **前提条件** ブランチAでエージェントが非表示（background）、**操作** ブランチAでEnterを押す、**期待結果** ペインを表示してフォーカスを移動し、tmuxのmouseが有効化される。既存のアクティブペイン（他ブランチ）があれば自動非表示になる
...
### ユーザーストーリー 4 - エージェントペインの起動（シングルアクティブ） (優先度: P0)

開発者がブランチを選択してエージェントを起動したとき、新しいtmuxペインが作成されてエージェントが起動する。既存のアクティブペインがあれば自動的に非表示になる。
...
1. **前提条件** ブランチAでエージェント未起動、アクティブペインなし、**操作** Enterを押してウィザードでエージェント選択、**期待結果** 新しいペインが右側に作成され、エージェントが起動する
2. **前提条件** ブランチBでエージェントがアクティブ表示中、**操作** ブランチAでEnterを押してウィザードでエージェント選択、**期待結果** ブランチBのペインが自動非表示になり、ブランチAの新しいペインが右側に作成され、エージェントが起動する
3. **前提条件** ペイン作成完了、**操作** 自動、**期待結果** フォーカスが新しいペインに移動する
4. ~~**前提条件** 複数エージェント起動中（1〜3件）、**操作** 新しいブランチでエージェントを起動、**期待結果** 右側の列で水平分割（上下積み）され、高さが均等に配置される~~ **[廃止: シングルアクティブ化]**
5. ~~**前提条件** 3件のエージェントが稼働中、**操作** 4件目のエージェントを起動、**期待結果** 右側に新しい列が追加され、全体が3列（gwt + 2列）で幅を自動調整する~~ **[廃止: シングルアクティブ化]**
...
3. **前提条件** ペインの現在コマンドがシェル（例: bash/sh）として取得される、**操作** gwtを再起動、**期待結果** 作業ディレクトリで関連付けた上で再接続され、可能なら直近履歴のツール名で表示される
...
### ユーザーストーリー 10 - ESCキーによるペイン非表示 (優先度: P0)

開発者がgwtペインでESCキーを押したとき、現在アクティブなエージェントペインが非表示になる。

**この優先度の理由**: シングルアクティブペイン制約に伴う新しい非表示操作。

**独立したテスト**: gwtペインでESCを押し、アクティブペインが非表示になることを確認する。

**受け入れシナリオ**:

1. **前提条件** ブランチAでエージェントがアクティブ表示中、gwtペインにフォーカス、**操作** ESCキーを押す、**期待結果** ブランチAのペインが非表示になり、gwt単独表示になる
2. **前提条件** アクティブペインなし（全て非表示または未起動）、gwtペインにフォーカス、**操作** ESCキーを押す、**期待結果** 何も起こらない（空振り）
3. **前提条件** エージェントペインにフォーカス、**操作** ESCキーを押す、**期待結果** 何も起こらない（エージェント側の操作）

---

### ユーザーストーリー 11 - 非表示ペインの背景ウィンドウ名簡素化 (優先度: P2)

開発者がtmuxのウィンドウ一覧を確認したとき、非表示ペインの背景ウィンドウ名がブランチ名だけで判別できる。

**この優先度の理由**: tmux上での識別性向上と混乱の防止。

**独立したテスト**: 非表示ペインの背景ウィンドウ名生成がブランチ名のみであることを自動テストで確認する（自動テスト必須）。

**受け入れシナリオ**:

1. **前提条件** ブランチAでエージェントがアクティブ表示中、gwtペインにフォーカス、**操作** ESCキーを押す、**期待結果** 背景ウィンドウ名がブランチ名のみになる
2. **前提条件** ブランチBでエージェントがアクティブ表示中、**操作** ブランチAで新規起動して自動非表示が発生、**期待結果** 背景ウィンドウ名がブランチ名のみになる

---

### エッジケース

- **tmux未インストール**: gwtはシングルモードで起動する（tmuxコマンドが不要なフローのみ使用）
...
- **FR-001**: システムは、起動時にTMUX環境変数の有無を確認し、設定されている場合はマルチモード、未設定の場合はシングルモードで起動**しなければならない**
- **FR-002**: システムは、tmux環境外で起動された場合でもエラー終了せず、シングルモードで動作**しなければならない**
- **FR-003**: システムは、tmuxがインストールされていない環境でも正常に動作し、シングルモードで起動**しなければならない**
...
- **FR-033**: ~~システムは、右側のエージェント列内でペインを水平分割（上下積み）し、最大3ペインまで均等配置しなければならない~~ **[廃止: シングルアクティブ化]**
- **FR-034**: ~~システムは、右側列のペイン数が3に達した場合、次のエージェントを新しい列として右側に追加しなければならない~~ **[廃止: シングルアクティブ化]**
- **FR-035**: ~~システムは、列数に応じて全体の幅を自動調整し、gwt列を含めた各列を均等幅で配置しなければならない~~ **[廃止: シングルアクティブ化]**
- **FR-036**: システムは、同時にアクティブ（表示）できるエージェントペインを最大1つに制限**しなければならない**（シングルアクティブ制約）
- **FR-037**: システムは、新しいエージェントペインを起動または表示する際、既存のアクティブペインを自動的に非表示**しなければならない**
...
- **FR-063**: システムは、ペインの現在コマンドがシェル等でエージェント判別できない場合でも再接続を継続し、直近の履歴があればそのツール名を優先表示**しなければならない**
...
- **FR-090**: ~~システムは、vキーで選択中ブランチのペインを表示/非表示切り替えしなければならない~~ **[廃止: ESCキーに変更]**
...
- **FR-095**: システムは、gwtペインでESCキーを押すと現在アクティブなエージェントペインを非表示**しなければならない**
- **FR-096**: システムは、アクティブペインがない状態でESCキーを押しても何もしない**ことができる**
- **FR-097**: システムは、ブランチ選択+Enterで非表示ペインを表示**しなければならない**（既存FR-012と連携）
- **FR-098**: システムは、非表示ペインの背景ウィンドウ名にブランチ名のみを使用**しなければならない**
...
- **SC-006**: 非表示ペインの背景ウィンドウ名がブランチ名のみであることを自動テストで検証できる

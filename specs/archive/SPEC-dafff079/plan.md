...
**仕様ID**: `SPEC-dafff079` | **日付**: 2026-01-30 | **仕様書**: `/specs/SPEC-dafff079/spec.md`
...
- 環境変数編集の保存/編集キャンセル/削除/リセットの挙動を明確化し、入力確定の整合性を維持する。
- 起動ログに環境変数一覧を表示し、`Version:` は `@` を除いた表記に統一する。
...
- 環境変数編集画面では `s` で保存して終了し、`Esc` は編集モード中は編集キャンセル、非編集時は保存して終了する。
- OS環境変数は同じリストに統合表示し、上書きは黄色、プロファイル追加のみは緑、OSのみは通常色で表示する。
- 上書き項目は `r` でOS値へリセットできる。
- OS環境変数は `d` で無効化し、赤色の取り消し線で表示する。
- 起動ログの `Env:` は空なら `(none)` を表示し、存在する場合は `KEY=VALUE` の一覧を表示する。
...
- 既存のユニットテストは維持。
- 統合表示の色分け/スクロール/入力挙動、保存/編集キャンセル、編集確定、起動ログ表示のユニットテストを追加する。

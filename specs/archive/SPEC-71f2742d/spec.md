...
開発者がTabキーでブランチモード→エージェントモード→設定画面を切り替えできる。設定画面にはカスタムエージェント管理と環境変数プロファイル（Environment）が統合される。Environmentは名前、説明、環境変数のみを管理し、AI設定は専用の「AI」タブで管理される。
...
4. **前提条件** 設定画面を開いている、**操作** カスタムエージェントセクションとEnvironmentセクションを確認、**期待結果** 両方のセクションが表示される
5. **前提条件** Environmentカテゴリでプロファイルを選択中、**操作** Enterを押す、**期待結果** 環境変数編集モードに入る
6. **前提条件** Environmentカテゴリでプロファイルを選択中、**操作** Eを押す、**期待結果** プロファイル名・説明の編集フォームが表示される
7. **前提条件** 設定画面を開いている、**操作** 「AI」タブに切り替えてEnterを押す、**期待結果** AISettingsWizardが開く
...
- **FR-022**: システムは設定画面でカスタムエージェント管理とEnvironment設定（環境変数プロファイル）を統合表示**しなければならない**
- **FR-023**: システムはEnvironment設定で名前、説明、環境変数のみを管理し、AI設定は含め**てはならない**
- **FR-029**: システムはEnvironmentカテゴリでEnterキーを押すと環境変数編集モードに入ら**なければならない**
- **FR-030**: システムはEnvironmentカテゴリでEキーを押すとプロファイル名・説明の編集フォームを表示**しなければならない**
...
- **FR-028**: システムは 'p' キーでブランチモードから設定画面のEnvironmentタブに直接遷移**しなければならない**
- **FR-031**: システムはEnvironment編集モードでOS環境変数とプロファイル環境変数を統合表示**しなければならない**（SPEC-dafff079 FR-009準拠）
- **FR-032**: システムはEnvironment編集モードで上書き変数を黄色、追加変数を緑色、OSのみを通常色、無効化を赤色取り消し線で表示**しなければならない**（SPEC-dafff079 FR-010準拠）
- **FR-033**: システムはEnvironment編集モードで 'R' キーでOS値へリセット**しなければならない**（SPEC-dafff079 FR-019準拠）
- **FR-034**: システムはEnvironment編集モードで 'D' キーでOS変数を無効化**しなければならない**（SPEC-dafff079 FR-020準拠）

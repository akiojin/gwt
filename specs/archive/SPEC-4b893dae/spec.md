...
開発者がブランチを選択してTabキーを押すと、セッション要約タブに切り替わり、そのブランチで稼働中または最後に使用されたエージェント（Claude Code, Codex CLI, Gemini CLI, OpenCode）のセッション内容がAIで要約されて表示される。セッション要約には、タスクと進捗状況、短文要約、バレットポイント一覧、メトリクス（トークン数、ツール実行数、経過時間）が含まれる。バレットポイントには、依頼(元)、直近の指示、決定事項、除外/やらないこと、ステータス、進捗、次（ユーザーの1アクション）、要確認（質問形式）、要点3語をラベル付きで含め、ラベルは要約言語に合わせる。
...
5. **前提条件** エージェントが作業中でセッション要約が表示されている、**操作** 60秒待機、**期待結果** セッション要約が自動更新される
6. **前提条件** セッション要約が既に表示されている、**操作** ポーリング更新が発生、**期待結果** 既存要約は表示されたまま、更新完了時に新しい要約へ差し替わる
7. **前提条件** セッション要約が長文、**操作** パネル表示を確認、**期待結果** 文字単位で折り返され、スクロールで全文が確認できる
...
11. **前提条件** セッション要約の生成が進行中、**操作** 60秒ポーリングが到来、**期待結果** 生成中の要約は中断されず、次回ポーリングは生成完了後に実行される
12. **前提条件** エージェントが作業中でセッションファイル更新が続いている、**操作** セッション要約タブで60秒ポーリングが発火、**期待結果** セッションファイルが直近の静止期間（例: 5-10秒）に到達していない場合は再生成を見送り、既存要約を保持したまま次回ポーリングで再評価する
13. **前提条件** AI応答が途中で途切れ、必須見出し/箇条書きを満たさない、**操作** 要約反映、**期待結果** 既存要約を保持し、警告（例: `Incomplete summary; keeping previous`）を表示して次回ポーリングで再試行する
14. **前提条件** ハイライトが4件以上生成される、**操作** セッション要約タブを確認、**期待結果** すべての箇条書きが省略されず表示され、スクロールで全件確認できる
15. **前提条件** セッション要約が生成済み、**操作** セッション要約タブを確認、**期待結果** ハイライトに依頼(元)、直近の指示、決定事項、進捗、次、要確認のラベル付き項目が含まれる
16. **前提条件** セッション要約が生成済み、**操作** セッション要約タブを確認、**期待結果** ハイライトに除外/やらないこと、ステータス、次（ユーザーの1アクション）、要確認（質問形式）、要点3語が含まれる
17. **前提条件** セッション要約がスクロール可能、**操作** セッション要約タブを確認、**期待結果** パネル右端にスクロールバーが表示される（スクロール不可なら表示しない）
18. **前提条件** セッション要約がスクロール可能、**操作** マウスホイールで上下スクロール、**期待結果** セッション要約が上下にスクロールする
19. **前提条件** AI API backend が Responses API をサポートしない（例: `Not Implemented ... does not support the Responses API`）、**操作** セッション要約を生成、**期待結果** 過剰な再試行による長時間待ちが発生せず、エラーが表示される。既存要約がある場合は保持される
...
3. **前提条件** ブランチAでセッション要約タブを表示中、**操作** ブランチBにカーソルを移動、**期待結果** ブランチBもセッション要約タブで表示される（グローバルタブ状態）
4. **前提条件** ブランチ詳細またはセッション要約タブが表示されている、**操作** パネル枠を確認、**期待結果** Tabキーで切り替えできる旨のヒントが表示される（例: `Tab: Switch`）
5. **前提条件** セッション要約タブを表示中、**操作** rキーでリフレッシュ、**期待結果** セッション要約タブが維持される（グローバルタブ状態が保持）
6. **前提条件** セッション要約タブを表示中、**操作** 新規ブランチ作成後に自動更新が発生、**期待結果** セッション要約タブが維持される（グローバルタブ状態が保持）
7. **前提条件** アプリを起動、**操作** ブランチ選択画面を表示、**期待結果** ブランチ詳細タブ（Details）がデフォルトで表示される
...
5. **前提条件** ブランチ詳細の行が長い、**操作** パネルを確認、**期待結果** 文字単位で折り返され、横幅で欠けない
6. **前提条件** ターミナルサイズが小さい、**操作** パネルを確認、**期待結果** ブランチリスト表示領域が減少し、パネルは固定高さを維持する
...
6. **前提条件** モデル一覧が表示領域より多い、**操作** モデル選択画面で上下キーを押下、**期待結果** 選択行がスクロールして常に可視領域に表示され、スクロールバーが表示される
7. **前提条件** モデル選択画面、**操作** モデルを選択してEnterを押下、**期待結果** 設定が保存され、ウィザードが終了する
8. **前提条件** モデル一覧取得失敗（401/403）、**操作** 画面を確認、**期待結果** 詳細なエラーメッセージ（例: `401 Unauthorized`）が表示され、URL入力画面に戻る
9. **前提条件** モデル一覧取得失敗（接続エラー）、**操作** 画面を確認、**期待結果** 詳細なエラーメッセージ（例: `Connection refused`）が表示され、URL入力画面に戻る
10. **前提条件** モデル一覧取得失敗（タイムアウト）、**操作** 10秒以上待機、**期待結果** タイムアウトエラーが表示され、URL入力画面に戻る
11. **前提条件** 各ウィザードステップ、**操作** Escキーを押下、**期待結果** 前のステップに戻る（URL入力画面でEscならウィザードをキャンセル）
12. **前提条件** 既存のAI設定がある、**操作** ウィザードを起動、**期待結果** URL/APIキー/モデルの既存値がプリセットされる
13. **前提条件** AI設定が存在する、**操作** 削除ボタンを押下、**期待結果** AI設定が削除される
...
### ユーザーストーリー 6c - セッション要約の有効/無効切り替え (優先度: P1)

開発者がAI設定ウィザード内でセッション要約の有効/無効を切り替え、設定を保存できる。セッション要約を無効化してもAI設定自体は保持され、Agent Modeなど他のAI機能には影響しない。セッション要約が無効な場合、セッション要約タブは専用メッセージを表示し、要約生成やポーリング更新を行わない。

**この優先度の理由**: セッション要約を一時的に停止したいニーズに対応するため。

**独立したテスト**: セッション要約のON/OFF切り替え後に表示と生成挙動が変化することを確認することで検証できる。

**受け入れシナリオ**:

1. **前提条件** AI設定ウィザードのモデル選択画面、**操作** セッション要約の切り替え操作を行う、**期待結果** 画面上の表示が`Session summary: On/Off`として更新される
2. **前提条件** セッション要約がOffで保存済み、**操作** セッション要約タブを確認、**期待結果** `Session summary disabled`が表示され、要約生成やポーリングが行われない
3. **前提条件** セッション要約がOffで保存済み、**操作** セッション要約をOnに切り替えて保存、**期待結果** 以後のセッション要約生成が再開される
4. **前提条件** セッション要約の切り替えを保存、**操作** Agent Modeを確認、**期待結果** Agent ModeのAI有効/無効に影響しない
5. **前提条件** 既存のAI設定がある、**操作** AI設定のクリア操作を実行、**期待結果** AI設定が削除され、セッション要約タブは「Configure AI in Profiles to enable session summary」を表示する
...
- **表示幅が狭い**: ブランチ詳細/セッション要約は文字単位で折り返し、コミットメッセージは末尾省略（`...`）で対応する
...
- **セッションファイルの更新が連続**: 直近の静止期間（例: 5-10秒）に達するまで再生成を遅延し、既存要約を保持する
- **要約が不完全/途中切れ**: 必須見出しや箇条書きを満たさない要約は無効として扱い、既存要約を保持し警告を表示する
- **長いセッション（1000ターン以上）**: 動的サンプリングにより、最初・途中・最後の部分を抽出して要約
- **選択中ブランチの削除**: 選択中のブランチが削除された場合、先頭のブランチに選択を移動し、グローバルタブ状態を維持
- **AI設定無効時のSessionタブ**: AI設定が無効でもSessionタブ選択は維持可能。空のパネル（「No session」等）を表示
- **セッション要約無効時のSessionタブ**: セッション要約が無効な場合は`Session summary disabled`を表示し、要約生成やポーリングを行わない
...
- **FR-041a**: システムは、セッション要約のMarkdownを以下のフォーマットで表示**しなければならない**: `目的`、`要約`、`ハイライト（箇条書き）`
  ハイライトには依頼(元)、直近の指示、決定事項、除外/やらないこと、ステータス、進捗、次（ユーザーの1アクション）、要確認（質問形式）、要点3語のラベル付き項目を含め、ラベルは要約言語に合わせる
- **FR-041b**: システムは、LLMから返されたMarkdownを**最小限の解析**で表示可能なテキストに変換し、**内容は改変せず**に表示**しなければならない**
- **FR-041c**: システムは、ハイライトの箇条書き件数を固定せず、重要度に応じて増減を許容し、受信した箇条書きを省略せず表示**しなければならない**
...
- **FR-046a**: システムは、セッション要約がスクロール可能な場合、セッション要約タブにスクロールバーを表示**しなければならない**
- **FR-046b**: システムは、セッション要約タブにおいてマウスホイールで上下スクロールできるように**しなければならない**
...
- **FR-074**: システムは、タブ状態を全ブランチで統一（グローバル）して管理**しなければならない**
- **FR-074a**: システムは、Tab切り替え時に即座にグローバルタブ状態を更新**しなければならない**
- **FR-074b**: システムは、rキーによるリフレッシュや操作後の自動更新でグローバルタブ状態を維持**しなければならない**
- **FR-074c**: システムは、新規ブランチ作成後の自動更新でグローバルタブ状態を維持**しなければならない**
- **FR-074d**: システムは、ブランチ切り替え時に現在のグローバルタブ状態を維持**しなければならない**
- **FR-075**: システムは、アプリ起動時のデフォルトタブをブランチ詳細タブ（Details）として表示**しなければならない**

#### セッションパーサー

- ~~**FR-080**: システムは、gwtのsession_idを使用して該当エージェントのセッションを特定**しなければならない**~~ *廃止: FR-121で代替*
- ~~**FR-081**: システムは、Claude Codeのセッションファイル（`~/.claude/projects/*/`配下JSONL）を解析**できなければならない**~~ *廃止: FR-121で代替*
- ~~**FR-082**: システムは、Codex CLIのセッションファイルを解析**できなければならない**~~ *廃止: FR-121で代替*
- ~~**FR-083**: システムは、Gemini CLIのセッションファイルを解析**できなければならない**~~ *廃止: FR-121で代替*
- ~~**FR-084**: システムは、OpenCodeのセッションファイルを解析**できなければならない**~~ *廃止: FR-121で代替*
- ~~**FR-085**: システムは、会話履歴（user/assistantメッセージ）とツール実行履歴（tool_use/tool_result）を抽出**しなければならない**~~ *廃止: FR-121で代替*
- ~~**FR-086**: システムは、長いセッション（1000ターン以上）の場合、動的サンプリング（最初・途中・最後）を適用**しなければならない**~~ *廃止: FR-125で代替*
- ~~**FR-087**: システムは、共通のSessionParserトレイトを定義し、各エージェント別に実装**しなければならない**~~ *廃止: FR-121で代替*

#### スクロールバックベース要約

- **FR-121**: システムは、builtin terminalで起動したエージェントのスクロールバックファイルをセッション要約のデータソースとして使用**しなければならない**
- **FR-122**: システムは、スクロールバックファイルからANSIエスケープシーケンスを除去し平文テキストを取得**しなければならない**
- **FR-123**: システムは、branch名とpane_idのマッピングを`~/.gwt/terminals/index.json`に永続保存**しなければならない**
- **FR-124**: システムは、スクロールバックファイルが存在しない場合、従来のSessionParserにフォールバック**しなければならない**
- **FR-125**: システムは、大きなスクロールバック（MAX_PROMPT_CHARS超過）を先頭部分＋末尾部分のサンプリングで要約**しなければならない**

#### ポーリング更新

- **FR-090**: システムは、セッション要約タブ表示中、60秒間隔でセッションファイルの更新をポーリング**しなければならない**
...
- **FR-094**: システムは、セッション要約の生成が進行中の場合、新たなポーリング起因の再生成を開始せず、生成完了後に次回ポーリングを実行**しなければならない**
- **FR-095**: システムは、セッションファイルが直近の静止期間（例: 5-10秒）に到達していない場合、再生成を延期**しなければならない**
- **FR-096**: システムは、AI応答が必須見出し/箇条書きを満たさない場合、不完全要約として扱い、既存要約を保持し警告表示を行い**しなければならない**
...
- **FR-116**: システムは、モデル一覧が表示領域を超える場合、上下キー操作に応じてリストをスクロールし、スクロールバーを表示**しなければならない**
- **FR-117**: システムは、AI設定ウィザードのモデル選択画面でセッション要約の有効/無効を切り替え可能**しなければならない**
- **FR-118**: システムは、セッション要約が無効な場合、セッション要約タブに`Session summary disabled`を表示し、要約生成とポーリング更新を行わ**なければならない**
- **FR-119**: システムは、セッション要約の有効/無効をAI設定として永続化し、AI設定自体を保持したまま切り替えでき**なければならない**
- **FR-120**: システムは、AI設定ウィザードでAI設定のクリア操作を提供し、クリア後はAI設定が削除され**なければならない**
...
- **AISettings**: プロファイルに追加されるAI設定（エンドポイント、APIキー、モデル名、セッション要約有効フラグ）
...
- **SC-009**: セッション要約の60秒ポーリングが正確に動作する（誤差±5秒以内、生成中/静止期間待機を除く）
...
- **SC-012**: 不完全な要約応答が返っても、既存要約が保持され表示が途切れない
- **SC-013**: セッションファイルが連続更新中でも、再生成は静止期間後に実行される
- **SC-014**: AI設定ウィザードでモデル一覧取得が10秒以内にタイムアウトする
- **SC-015**: ウィザードのステップ遷移（Enter/Esc）が50ms以内に完了する
- **SC-016**: モデル一覧取得成功時、モデルリストが即座に表示される（100ms以内）
- **SC-017**: 疎通チェック失敗時、詳細なエラーメッセージがユーザーに表示される
- **SC-018**: ハイライトが4件以上ある場合でも、省略せずに全件が表示される
- **SC-019**: セッション要約が無効な場合、専用メッセージが表示され、要約生成やポーリングが実行されない

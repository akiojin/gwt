...
**仕様ID**: `SPEC-4b893dae` | **日付**: 2026-01-19 | **更新日**: 2026-01-25 | **仕様書**: [spec.md](./spec.md)
...
- Tabキーによるタブ切り替え
- Tabキー切り替えヒントの表示
- ブランチ詳細/セッション要約の文字単位折り返し + スクロール表示
- セッション要約がスクロール可能な場合のスクロールバー表示
- セッション要約タブのマウスホイールスクロール対応
- セッションパーサーによる4エージェント対応
- 60秒ポーリングによる自動更新
- ポーリング再入防止（生成中は次回を遅延）
- セッションファイル更新の静止期間後に再生成
- 要約Markdownの完全性検証（不完全要約は保持せず警告）
- ハイライト箇条書きは件数可変で省略しない
- AIによるセッション要約（タスク/進捗、短文要約、バレットポイント、メトリクス）
- セッション要約のAIリクエストは10分以上のタイムアウトを設定
- AI設定: エンドポイント/モデル必須、APIキー任意
- AI設定モデル一覧が長い場合はスクロール可能にする
- セッション内容に応じた要約言語推定（AI側で判定）
- セッション要約の有効/無効切り替え（AI設定ウィザード内）
- セッション要約無効時は専用メッセージ表示＋要約生成/ポーリング停止

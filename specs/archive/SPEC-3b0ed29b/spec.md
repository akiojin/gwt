...
**追記**: 2026-01-20 起動準備を非同期化し、シングル/ tmux モードの起動前処理を同一パイプラインに統一する。進捗はTUI上のステータス表示領域に英語で表示する。
...
1. **前提条件** gwtが起動しブランチが選択されている、**操作** エージェント選択画面でCodexを選択して起動、**期待結果** Codexが`-c web_search=live`（日付ベースバージョン0.1.YYYYMMDDNN）、`--enable web_search`（v0.90.0以上）、または`--enable web_search_request`（v0.89.x以下）、(v0.80.0未満の場合のみ)`--enable skills`、`--sandbox workspace-write`などのデフォルト引数付きで起動する
...
**独立したテスト**: 各コーディングエージェントを権限スキップモードで起動し、適切なフラグ（Codex v0.80.0+は`--dangerously-bypass-approvals-and-sandbox`、Codex v0.79.x以下は`--yolo`、Claude Codeは`--dangerously-skip-permissions`、Geminiは`-y`）がプロセスに渡されることを確認すれば検証できる。

**受け入れシナリオ**:

1. **前提条件** Codex CLI v0.80.0以上・権限スキップモードが有効、**操作** Codexを起動、**期待結果** `--dangerously-bypass-approvals-and-sandbox`フラグが渡され、自動承認モードで起動する
2. **前提条件** Codex CLI v0.79.x以下・権限スキップモードが有効、**操作** Codexを起動、**期待結果** `--yolo`フラグが渡され、自動承認モードで起動する
3. **前提条件** 権限スキップモードが有効、かつWindows環境、**操作** Claude Codeを起動、**期待結果** `--dangerously-skip-permissions`フラグが渡され、`IS_SANDBOX=1`環境変数は設定されない
4. **前提条件** 権限スキップモードが有効、かつWindows以外、**操作** Claude Codeを起動、**期待結果** `--dangerously-skip-permissions`フラグと`IS_SANDBOX=1`環境変数が設定される
5. **前提条件** 権限スキップモードが有効、**操作** Geminiを起動、**期待結果** `-y`フラグが渡され、自動承認モードで起動する
6. **前提条件** 権限スキップモードが有効、**操作** OpenCodeを起動、**期待結果** OpenCodeはデフォルトで自動承認のため追加フラグなしで起動する（現時点で権限スキップ用のフラグは未提供）
...
### ユーザーストーリー 10 - コーディングエージェント名の一貫した色づけ (優先度: P2)

開発者がgwtを使用する際、コーディングエージェント名（Claude Code、Codex CLI、Gemini CLI、OpenCode）が表示されるすべての箇所で、エージェントごとに一貫した色で表示され、視覚的にどのエージェントかを素早く識別できる。

**この優先度の理由**: 複数のコーディングエージェントを使い分ける開発者にとって、視覚的な識別は作業効率と誤操作防止に貢献する。ブランチ一覧では既に色づけが実装されているため、他の画面でも統一することでUXの一貫性を高める。

**独立したテスト**: 各画面（ブランチ一覧、エージェント選択画面、Quick Start、Web UI等）でコーディングエージェント名が表示される箇所を確認し、同じエージェントが同じ色で表示されることを検証すれば確認できる。

**色の定義**:

- **Claude Code**: 黄色 (yellow / #f6e05e)
- **Codex CLI**: シアン (cyan / #4fd1c5)
- **Gemini CLI**: マゼンタ (magenta / #d53f8c)
- **OpenCode**: 緑 (green / #48bb78)
- **カスタム/不明**: グレー (gray / #a0aec0)

**対象画面**:

- CLI: ブランチ一覧のツール名表示（既存）
- CLI: コーディングエージェント選択画面（ウィザードポップアップ）
- CLI: Quick Start履歴のエージェント名
- Web UI: セッション履歴テーブルのツール列
- Web UI: Last Activityバッジのエージェント名
- Web UI: コーディングエージェント選択ドロップダウン

**受け入れシナリオ**:

1. **前提条件** ブランチ一覧画面を表示、**操作** ツール名の色を確認、**期待結果** Claude Codeは黄色、Codex CLIはシアン、Gemini CLIはマゼンタ、OpenCodeは緑で表示される
2. **前提条件** コーディングエージェント選択画面を表示、**操作** 選択肢の色を確認、**期待結果** 各エージェント名が定義された色で表示される
3. **前提条件** Quick Start画面を表示、**操作** 履歴のエージェント名の色を確認、**期待結果** 各エージェント名が定義された色で表示される
4. **前提条件** Web UIのセッション履歴テーブルを表示、**操作** ツール列の色を確認、**期待結果** 各エージェント名が定義された色で表示される
5. **前提条件** Web UIのLast Activityカードを表示、**操作** エージェント名バッジの色を確認、**期待結果** 各エージェント名が定義された色で表示される

---

### ユーザーストーリー 11 - 起動直後の異常終了を検知して可視化する (優先度: P1)

開発者がコーディングエージェントを起動した際、起動直後に終了してしまう場合でも、gwtは「成功」扱いにせず、原因が分かるエラー情報を提示して再現・解決に進める。

**この優先度の理由**: 起動直後の終了は作業が開始できず致命的であるため、ユーザーに失敗が伝わらない状態を最優先で排除する必要がある。

**独立したテスト**: エージェントの起動が直後に終了するケース（非ゼロ終了や想定外シグナル）を再現し、エラー表示とログ記録が行われることを確認すれば検証できる。

**受け入れシナリオ**:

1. **前提条件** エージェント起動後すぐに非ゼロ終了コードで終了、**操作** 起動フローを完了、**期待結果** 画面にエラーが表示され、成功完了メッセージは表示されず、ブランチ一覧へ戻る（CLIは終了しない）
2. **前提条件** エージェント起動直後に想定外シグナルで終了、**操作** 起動フローを完了、**期待結果** エラー原因（exit code/signal）がログに記録され、ブランチ一覧へ戻る（CLIは終了しない）
3. **前提条件** ユーザーが明示的に中断（Ctrl+Cなど）した、**操作** 起動フローを終了、**期待結果** 中断は警告扱いで終了し、異常終了としては扱われず、ブランチ一覧へ戻る（CLIは終了しない）

---

### ユーザーストーリー 12 - OpenCodeのモデル選択が空で止まらない (優先度: P1)

開発者がOpenCodeを選択した際、モデル選択が空で進行不能にならず、デフォルト選択や任意入力で必ず起動まで進める。

**この優先度の理由**: モデル選択画面が空だと起動不能になるため、OpenCodeの基本利用がブロックされる。

**独立したテスト**: OpenCode選択時にモデル選択が空にならないことと、選択後に次のステップへ進めることを確認すれば検証できる。

**受け入れシナリオ**:

1. **前提条件** OpenCodeを選択、**操作** モデル選択画面を表示、**期待結果** 少なくとも「Default (Auto)」が表示され、選択して次に進める
2. **前提条件** 任意モデル入力が可能、**操作** provider/model形式を入力して選択、**期待結果** `-m provider/model` が引数として渡される
3. **前提条件** モデル一覧の取得が失敗、**操作** モデル選択画面を表示、**期待結果** デフォルト選択のみでも起動できる

---

### ユーザーストーリー 13 - セッション完了後にブランチ一覧へ戻る (優先度: P2)

開発者がコーディングエージェントを終了した後、gwtが終了せずにブランチ一覧へ戻れると、連続作業が途切れない。

**この優先度の理由**: 作業の切り替え回数が多い開発者にとって、再起動せずに戻れることは生産性に直結する。

**独立したテスト**: エージェントを正常終了させた後にブランチ一覧へ戻れることと、成功メッセージが表示されることを確認すれば検証できる。

**受け入れシナリオ**:

1. **前提条件** エージェントを正常終了、**操作** 起動フローを終了、**期待結果** 成功メッセージが表示され、ブランチ一覧へ戻る（CLIは終了しない）

---

### ユーザーストーリー 14 - 起動/終了時のTUI遷移が固まらない (優先度: P1)

開発者がコーディングエージェントを起動したり終了したりする際、TUIが閉じた直後や終了後の復帰で「無反応」に見えないよう、即時に状態が表示されて待機時間が最小化される。

**この優先度の理由**: TUIの消失後に無反応が続くとフリーズと誤認され、作業継続が妨げられるため。

**独立したテスト**: 起動確定直後に英語のステータス行が表示されることと、終了後に長時間の待機が発生しないことを確認すれば検証できる。

**受け入れシナリオ**:

1. **前提条件** エージェント起動を確定、**操作** TUIが終了して起動処理へ移行、**期待結果** 1秒以内に英語のステータス行（例: `Launching Codex CLI...`）が表示され、空白の待機が発生しない
2. **前提条件** 依存インストールが有効、**操作** エージェント起動を確定、**期待結果** 依存インストール開始前に英語のステータス行が表示される
3. **前提条件** エージェントが終了、**操作** 起動フローを終了、**期待結果** 1秒以内にブランチ一覧へ復帰し、終了直後の待機が長時間続かない

---

### ユーザーストーリー 15 - 起動準備の進捗がセンターモーダルで見える (優先度: P1)

開発者がコーディングエージェント起動を確定した直後、画面中央にモーダルが表示され、Worktree作成処理の各ステップがチェックマーク形式のリストで表示される。処理が動いていることが明確にフィードバックされ、安心して待機できる。

**この優先度の理由**: 「Preparing worktree」が3箇所に冗長表示され停止状態に見える問題を解決し、ユーザー体験を改善するため。

**独立したテスト**: 起動確定直後にセンターモーダルが表示され、各ステップが順次進行し、完了時にサマリが表示されることを確認すれば検証できる。

**受け入れシナリオ**:

1. **前提条件** ブランチを選択し起動を確定、**操作** Skip PermissionsでEnterを押す、**期待結果** 1秒以内に画面中央に半透明オーバーレイ付きモーダルが表示され、ステップリストが表示される
2. **前提条件** モーダル表示中、**操作** 各ステップが進行、**期待結果** 完了ステップに[x]、進行中ステップに[>]、待機中ステップに[ ]が表示され、色分けされる（緑/黄/灰）
3. **前提条件** 進行中ステップが3秒以上継続、**操作** 経過を確認、**期待結果** 進行中ステップに経過時間（例: 5.2s）が表示される
4. **前提条件** 全ステップ完了、**操作** 処理完了、**期待結果** サマリ（Completed in X.Xs）が表示され、エージェント起動に遷移する
5. **前提条件** モーダル表示中、**操作** ESCキーを押す、**期待結果** 処理がキャンセルされ、モーダルが閉じ、ブランチ一覧に戻る
6. **前提条件** git fetch中にネットワークエラー発生、**操作** エラー発生、**期待結果** 失敗ステップに[!]マーク（赤色）とエラーメッセージが表示される
7. **前提条件** モーダル表示中、**操作** ブランチ詳細エリアとセッション要約エリアを確認、**期待結果** 「Preparing worktree」ではなく通常の情報が表示される（冗長排除）
8. **前提条件** tmux環境内で起動確定、**操作** 起動フローを進める、**期待結果** シングルモードと同一のモーダル表示とステップが実行される

---

### ユーザーストーリー 16 - macOSのPTYラッパーで引数が失われず起動できる (優先度: P1)

開発者がmacOSでコーディングエージェントを起動する際、PTYラッパーがエージェントの引数を正しく渡し、`-c`や`--resume`などの引数がラッパー側に誤解釈されないことで起動直後の終了が発生しない。

**この優先度の理由**: macOS環境で起動できない場合は作業開始そのものが不可能となるため。

**独立したテスト**: macOS向けPTYラッパー生成が`script -q -- /dev/null <command> <args...>`形式になり、`-c`等がラッパー側に解釈されないことをユニットテストで確認すれば検証できる。
...
- Windows環境のClaude Code起動では、権限スキップ時でも`IS_SANDBOX=1`を付与しない（起動クラッシュ回避）
...
- 起動準備が長時間化してもモーダル内の進捗表示は最新状態に更新され、3秒以上かかるステップは経過時間を表示する
- モーダル表示中にESCキーでキャンセルした場合、不完全なworktreeがあればクリーンアップを試みる
- git fetchがタイムアウトした場合、即座に失敗としてエラー表示する（リトライしない）
- エラーメッセージがモーダル幅（80文字）を超える場合、ステップ行とエラーメッセージ行を分離し、エラーメッセージはインデント付き別行で複数行に折り返して全文を表示する。モーダル高さはエラー行数に応じて動的に拡張する
- シングル/ tmux モードで起動準備のステップ順序が異ならない
- rootユーザーでClaude Codeを起動する場合、Docker/サンドボックス環境として検出され、追加の警告が表示される
- OpenCodeのモデル一覧が空の場合でも、デフォルト選択で起動できるようにする
- エージェント終了直後にバックグラウンド更新が待機中でも、TUI復帰が長時間遅延しない
...
- **FR-002a**: bunxコマンドが`node_modules/.bin`配下にある場合は、そのbunxを使用せず`npx`へフォールバックし、`--yes`を付与してプロンプトを抑止**しなければならない**
...
- **FR-008**: 各コーディングエージェントは、起動時に実際に渡されるコマンドライン引数と起動情報（Working directory / Model / Reasoning / Mode / Skip permissions / Version / 実行方法[installed/bunx]）をログに表示**しなければならない**
...
- **FR-012a**: macOSではPTYラッパーが`script -q -- /dev/null <command> <args...>`形式で起動し、`-c`などの引数がラッパー側に誤解釈されないように**しなければならない**
...
- **FR-028**: gwtは起動時に全bunx型エージェントのnpmバージョン一覧を事前取得し、アプリケーション状態にキャッシュ**しなければならない**
- **FR-029**: バージョン選択画面は、キャッシュ済みのバージョン一覧を使用し、npm レジストリへの再アクセスを行ってはならない
- **FR-029a**: バージョン選択画面は、起動時にキャッシュしたインストール済みエージェント情報を使用し、画面表示後にローカルコマンドの再検出を行ってはならない
- **FR-030**: バージョン事前取得はバックグラウンドで行い、UI のブロッキングを避け**なければならない**
- **FR-031**: バージョンキャッシュが空の場合（取得失敗・タイムアウト）は、フォールバックとして latest のみを表示**しなければならない**
- **FR-032**: コーディングエージェントが起動直後に異常終了した場合、成功完了メッセージを表示せず、エラーとして扱い**しなければならない**
- **FR-033**: 起動失敗時は、エージェントID・バージョン・終了コード/シグナル・経過時間をログに記録**しなければならない**
- **FR-034**: 起動直後の異常終了は、ユーザー向けに再試行/環境確認を促すエラーメッセージを表示**しなければならない**
- **FR-035**: エージェントの起動フローが終了した場合、CLIは終了せずブランチ一覧へ戻る**しなければならない**
- **FR-036**: 正常終了時は成功メッセージを表示し、異常終了時はエラーメッセージ（再試行/環境確認の案内）を表示**しなければならない**
- **FR-037**: TUI終了後の起動開始時は、依存インストールや検出処理の前に英語のステータス行を表示**しなければならない**
- **FR-038**: エージェント終了後の復帰処理は、バックグラウンド更新の待機により1秒以上遅延してはならない
- **FR-039**: シングル/ tmux モードの起動前処理は同一の準備パイプラインを使用し、ステップ順序を一致させ**なければならない**
- **FR-040**: 起動準備（Worktree解決、依存インストール、コマンド組み立て等）はTUIのイベントループをブロックせずバックグラウンドで実行**しなければならない**
- **FR-041**: 起動準備の進捗は画面中央のモーダル内にステップリスト形式で表示**しなければならない**
- **FR-042**: 依存インストールが有効な場合、tmuxモードでもシングルモードと同様に同一パイプライン内で実行**しなければならない**
- **FR-043**: モーダル表示中は他のUI操作（ブランチリストスクロール、カーソル移動等）を無効化**しなければならない**

#### 進捗モーダル表示要件

- **FR-044**: モーダルは半透明オーバーレイで背景を覆い、背景のブランチリストがうっすら見える状態に**しなければならない**
- **FR-045**: モーダルの幅は80文字以上とし、全ステップが1行で表示できる**しなければならない**
- **FR-046**: モーダルのタイトルは現在のステップに応じて動的に変化**しなければならない**
- **FR-047**: ステップはチェックマーク形式で表示**しなければならない**（[x] 完了、[>] 進行中、[ ] 待機中、[!] 失敗、[skip] スキップ）
- **FR-048**: 表示するステップは以下の6段階とする**しなければならない**: 1. Fetching remote... 2. Validating branch... 3. Generating path... 4. Checking conflicts... 5. Creating worktree... 6. Checking dependencies...
- **FR-049**: 進行中ステップが3秒以上継続した場合、経過時間を表示**しなければならない**（例: [>] Creating worktree... 5.2s）
- **FR-050**: 完了済みステップは緑色、進行中ステップは黄色、待機中ステップは灰色、失敗ステップは赤色で表示**しなければならない**
- **FR-051**: 全ステップ完了時、サマリを2秒間表示**しなければならない**（例: Completed in 8.2s (6 steps)）
- **FR-052**: エラー発生時、失敗したステップを明示し、エラーメッセージを表示**しなければならない**。ユーザーが任意のキーを押すまでモーダルを維持**しなければならない**
- **FR-052a**: エラーメッセージがモーダル幅を超える場合、ステップ行の直下にインデント付きの別行としてエラーメッセージを表示**しなければならない**。エラーメッセージはモーダル内部幅で複数行に折り返し、全文を表示**しなければならない**。モーダル高さはエラー行数に応じて動的に拡張**しなければならない**
- **FR-053**: ネットワークエラー（git fetch失敗等）は即座に失敗として処理し、リトライしてはならない
- **FR-054**: モーダル表示中、ESCキーでキャンセル可能に**しなければならない**
- **FR-055**: キャンセル時、進行中のgit操作を中断し、不完全なworktreeディレクトリがあれば自動削除**しなければならない**
- **FR-056**: キャンセル後、ブランチ一覧に戻る**しなければならない**
- **FR-057**: モーダル表示中、ステータスバーに「Preparing worktree」を表示してはならない
- **FR-058**: モーダル表示中、ブランチ詳細エリアに「Preparing worktree」を表示してはならない
- **FR-059**: モーダル表示中、セッション要約エリアに「Preparing worktree」を表示してはならない
- **FR-060**: 既存worktreeを再利用する場合、不要なステップは[skip]マーク（灰色）で表示し、実行をスキップ**しなければならない**

#### コーディングエージェント名色づけ要件

- **FR-024**: システムは、コーディングエージェント名が表示されるすべての箇所で、エージェントごとに一貫した色を使用**しなければならない**
- **FR-025**: 色の定義は以下の通りとする: Claude Code=黄色(yellow/#f6e05e)、Codex CLI=シアン(cyan/#4fd1c5)、Gemini CLI=マゼンタ(magenta/#d53f8c)、OpenCode=緑(green/#48bb78)、カスタム/不明=グレー(gray/#a0aec0)
- **FR-026**: CLI UIおよびWeb UIの両方で、共通の色定義を参照**しなければならない**
- **FR-027**: 色づけ対象には、ブランチ一覧、エージェント選択画面、Quick Start履歴、セッション履歴テーブル、Last Activityバッジ、エージェント選択ドロップダウンを含め**なければならない**

#### Claude Code固有要件

- **FR-101**: Claude Codeは、権限スキップモード時に`--dangerously-skip-permissions`フラグを渡し、`IS_SANDBOX=1`環境変数はWindows以外で設定**しなければならない**
...
- **FR-201**: Codexは、デフォルト引数としてWeb検索機能を有効化する設定を含め**なければならない**。設定形式はCodex CLIバージョンに応じて切り替える（FR-210参照）
...
- **FR-206**: Codexは、権限スキップモード時にCodex CLI v0.80.0以上では`--dangerously-bypass-approvals-and-sandbox`を、v0.79.x以下では`--yolo`を渡さ**なければならない**。バージョン判定不能の場合は`--dangerously-bypass-approvals-and-sandbox`を優先する
...
- **FR-210**: Codexは、日付ベースバージョン（0.1.YYYYMMDDNN形式）の場合は`-c web_search=live`を、v0.90.0以上の従来形式では`--enable web_search`を、v0.89.x以下の場合は`--enable web_search_request`を渡さ**なければならない**。バージョン判定不能の場合は`-c web_search=live`（最新の挙動）を優先する
...
- **FR-406**: OpenCodeのモデル選択肢が取得できない場合でも、少なくともデフォルト選択を提示し、起動フローを進められるように**しなければならない**
- **FR-407**: OpenCodeで任意のモデル識別子が選択された場合、`-m provider/model` 形式で引数を渡さ**なければならない**
...
- **SC-008**: 起動直後の異常終了時に「成功完了」表示が出ず、エラーメッセージが表示されることを自動テストで確認できる
- **SC-009**: OpenCode選択時のモデル選択が空にならず、必ず次のステップに進めることを確認できる
- **SC-010**: TUI終了後の起動開始メッセージが1秒以内に表示されることを確認できる
- **SC-011**: エージェント終了後にTUIへ戻るまでの待機が1秒以内であることを確認できる
- **SC-012**: Skip Permissions 確定後、1秒以内にセンターモーダルが表示され、ステップリストが見えることを確認できる
- **SC-013**: 依存インストール有効時、tmuxモードでもシングルモードと同一の準備ステップが実行されることを確認できる
- **SC-015**: モーダル表示中に「Preparing worktree」が他のUI領域（ステータスバー、ブランチ詳細、セッション要約）に表示されないことを確認できる
- **SC-016**: 3秒以上かかるステップで経過時間が表示されることを確認できる
- **SC-017**: ESCキーでキャンセルが可能で、ブランチ一覧に戻れることを確認できる
- **SC-018**: エラー発生時に失敗ステップが[!]マーク（赤色）で明示されることを確認できる
- **SC-014**: macOSのPTYラッパーが`script -q -- /dev/null <command> <args...>`形式で生成されることをユニットテストで確認できる
...
- Claude Codeの権限スキップモードでは、Windows以外は`IS_SANDBOX=1`環境変数も設定され、サンドボックス環境として動作する

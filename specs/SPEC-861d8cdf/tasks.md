...
**見積もり**: S  
**前提タスク**: T-102  
**関連要件**: FR-102d, FR-102e, FR-102f

**説明**:
- gwt起動時にHook登録済みなら再登録を実行する
- 再登録はgwt hookのみを解除し、非gwt hookは保持する
- 失敗時は起動を継続し、ログに理由を記録する

**受け入れ基準**:
- [ ] Hook登録済みの状態で起動すると再登録が実行される
- [ ] 再登録後にgwt実行パスが最新へ更新される
- [ ] 非gwt hookが保持される
- [ ] 再登録に失敗してもgwt起動は継続する
- [ ] 再登録の挙動を検証する自動テストが追加される
...
2. **Phase 2 - Hook基盤** (T-101, T-102, T-107)
   - hookサブコマンド実装
   - Claude Code設定機能
   - 起動時の再登録

...
tmuxモードで起動している複数のコーディングエージェントの状態（running/waiting_input/stopped）をリアルタイムで可視化する機能。Claude CodeのHook APIを利用してエージェント状態を取得し、他のエージェントはペイン出力からヒューリスティックに推測する。状態に応じてスピナーの色と形状を変更し、ステータスバーにも集計情報を表示する。Hook設定は起動時に再登録して常に最新のgwt実行パスに同期する。
...
3. **前提条件** エージェントが1つも起動していない、**操作** ブランチリスト画面を表示、**期待結果** ステータスバーは非表示にならず、「Agents: none」のように0件を明示して表示される
...
### ユーザーストーリー 4 - 起動時のHook再登録 (優先度: P1)

開発者が一度Hookセットアップを承認した後は、gwt起動のたびにClaude CodeのHook設定が自動で再登録され、常に現在のgwt実行パスに同期される。これにより、過去のworktreeのgwtパスがsettings.jsonに残っても起動時に自動修復される。

**この優先度の理由**: Hookはエージェント状態可視化の前提であり、パス不整合が起きると状態更新が完全に止まる。起動時の自動再登録は運用上の致命的な停止を防ぐ。

**独立したテスト**: settings.jsonに古いgwtパスが登録された状態でgwtを起動し、起動後にhookコマンドが現行のgwt実行パスへ更新されていることを確認すれば検証できる。

**受け入れシナリオ**:

1. **前提条件** Hook登録済み、**操作** gwtを起動、**期待結果** Hookが再登録され、最新のgwt実行パスが設定される
2. **前提条件** Hook登録済みかつsettings.jsonに古いgwtパスが残存、**操作** gwtを起動、**期待結果** 古いパスが上書きされる
3. **前提条件** Hook登録済み、**操作** gwtを起動、**期待結果** 再登録は確認ダイアログなしで実行される

---

### ユーザーストーリー 5 - 初回起動時のHookセットアップ提案 (優先度: P2)
...
### ユーザーストーリー 6 - 他エージェント（Codex, Aider等）の状態推測 (優先度: P2)
...
- **FR-102d**: Hook登録済みの場合、gwt起動時に既存のgwt hookを再登録し、現在のgwt実行パスに同期し**なければならない**
- **FR-102e**: 再登録は既存の非gwt hook設定を保持したまま実行し**なければならない**
- **FR-102f**: 再登録に失敗してもgwt起動は継続し、失敗理由をログに記録し**なければならない**
- **FR-102g**: ブランチリスト画面で `u` キーを押すと、Claude Code Hook設定を手動で上書き再登録できる
- **FR-102h**: 手動再登録の結果（成功/失敗）はステータスメッセージで表示し**なければならない**
- **FR-102i**: gwtがbunx/npxなどの一時実行環境（`.bun/install/cache/`, `/tmp/bunx-`, `/.npm/_npx/`, `node_modules/.cache/`）から起動された場合、Hook登録の警告を表示し、グローバルインストールを促す。一時実行時の確認ダイアログではデフォルト選択を「Skip」とする
- **FR-102j**: gwt hookコマンドの検出パターンは、標準形式（`gwt hook`）に加え、ビルドバイナリ形式（パスに`/gwt`を含み` hook `を含む）も認識し、重複登録を防止し**なければならない**
...
- **FR-104a**: ブランチリスト画面の最下部に1行のステータスバーを常時表示し、条件によって非表示にしてはならない
- **FR-104b**: ステータスバーにエージェント状態の集計を表示（例: "Agents: 2 running | 1 waiting"）
- **FR-104c**: waiting_inputのカウントが1以上の場合、該当部分を黄色で強調表示する
- **FR-104d**: エージェントが0件の場合は、ステータスバーに "Agents: none" のような0件表示を出さなければならない
...
- 起動時の再登録は `unregister_gwt_hooks` → `register_gwt_hooks` の順で実行し、gwt実行パスを最新化する

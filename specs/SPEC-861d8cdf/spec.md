...
- **FR-102i**: bunx/npx等の一時実行環境（`.bun/install/cache/`, `/tmp/bunx-`, `/.npm/_npx/`, `node_modules/.cache/`）からの起動を検出した場合、Hook設定時に警告ダイアログを表示し**なければならない**
- **FR-102j**: gwt hookコマンドの検出パターンは`"gwt hook"`に加え、ビルドバイナリの`"/gwt"を含み" hook "を含む`形式（例: `gwt-HASH hook PreToolUse`）も対応し、重複登録を防止し**なければならない**

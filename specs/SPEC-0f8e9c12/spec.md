...
- Agent 状態フォールバック（10秒）で `localRefreshKey` を更新すると、Local/All キャッシュキーが毎回変化して `Loading...` が周期表示される
...
### ユーザーストーリー 6 - Agent状態更新中もWorktree表示を維持したい (優先度: P0)

開発者として、Agent 状態の定期更新が走っても Worktree 一覧が `Loading...` に戻らず、表示を維持してほしい。

**独立したテスト**: Agent タブ有効時の10秒フォールバックと `agent-status-changed` イベントで `Loading...` が表示されないこと

**受け入れシナリオ**:

1. **前提条件** Agent タブを1つ以上開いて Local/All 一覧を表示中、**操作** 10秒フォールバック更新が発生、**期待結果** 一覧表示を維持したまま背景更新され `Loading...` が表示されない
2. **前提条件** Agent タブを1つ以上開いて Local/All 一覧を表示中、**操作** `agent-status-changed` イベントを受信、**期待結果** 一覧表示を維持したまま背景更新され `Loading...` が表示されない
...
- Agent 状態フォールバック更新時は `localRefreshKey` を更新せず、キャッシュを保持した背景更新のみ実行する
...
- **FR-014**: Agent 状態更新（10秒フォールバック / `agent-status-changed`）は `localRefreshKey` を更新せず、Local/All キャッシュを背景再取得して `Loading...` を再表示してはならない
- **FR-015**: `fetch_pr_status` はバックエンド側のTTLキャッシュ（30秒）を優先し、フロントエンドの30秒周期が重なっても不要なGraphQL再実行を抑止しなければならない
...
- **NFR-007**: レートリミット到達時もUI表示は維持し、フロントエンド側で追加のエラー表示や即時リトライを発生させない
...
- **SC-011**: Agent タブ有効時の10秒フォールバック更新で `Loading...` 表示が出ないことをテストで確認できる
- **SC-012**: `agent-status-changed` 受信時の更新で `Loading...` 表示が出ないことをテストで確認できる
- **SC-013**: バックエンドTTL内の `fetch_pr_status` 呼び出しで追加GraphQL実行が発生しないことをコマンドテストで確認できる

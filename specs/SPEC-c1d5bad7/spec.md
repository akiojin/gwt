...
- ログビューアで最新ログが表示されず、古いログしか見えないケースが発生した
...
- 対象ブランチのログが空または古い場合、起動ディレクトリのログに最新エントリが存在すればフォールバックして表示する（Sourceに理由を明記する）
...
- 最新ログの判定はファイル名ではなく最終更新時刻（mtime）を優先する
- 各行は以下の形式で表示する:
  ```
  [HH:MM:SS] [LEVEL] [CATEGORY] message
  ```
- `HH:MM:SS` は **システムロケール（OSのタイムゾーン）** に従ったローカル時刻で表示する
- `LEVEL` と `CATEGORY` は列幅を揃えて表示する（一覧の可読性を優先する）
- ログ一覧は **折り返し表示を行わず**、表示幅を超える場合は **末尾を `...` で省略**する
...
### 6. 一覧表示の補助機能（フィルタ・リロード・tail・リセット）

- `f` でフィルタ入力モードに切り替える
  - `Enter` で確定、`Esc` でキャンセル（入力済みの場合はクリア優先）
  - フィルタは **category/level/message** を対象に部分一致（大文字小文字は区別しない）
  - フィルタ中は「表示件数/総件数」を表示する
- `v` でログレベルの表示しきい値を切り替える（循環）
  - `ALL` → `INFO+` → `WARN+` → `ERROR+`
- `r` で現在のログを再読み込みする
- `t` で tail（自動再読込）を切り替える
  - tail は 1〜2 秒間隔で再読み込みする
- `x` でログ対象ディレクトリ配下のログファイル（`YYYY-MM-DD.jsonl`）を空にし、ログをリセットする
  - リセットは **明示的な操作** のみで実行し、起動のたびにログが削除されることはない
  - リセット後はログ一覧を空にし、「No logs available.」を表示する
...
### 8. CLIカラーリング統一（選択行/ログレベル）

- CLIの選択行は共通ルールとして **前景=黒 / 背景=シアン** を適用する
- ログ一覧の選択行は **1行全幅** をシアン背景で塗りつぶす
- ログ一覧の `LEVEL` 列はレベルに応じて色分けする
  - TRACE: gray
  - DEBUG: blue
  - INFO: green
  - WARN: yellow
...
7. `f` でフィルタが適用されることを確認
8. `v` で表示レベルが切り替わることを確認
9. `r` でログが再読み込みされることを確認
10. `t` で tail が切り替わることを確認
11. 表示幅を超える行が `...` で省略され、折り返し表示にならないことを確認
12. （stdout/stderr 取り込みが有効な場合）エージェント出力がログとして表示されることを確認
13. ログ一覧の選択行がシアン背景・黒文字で 1 行全幅に表示されることを確認
14. ログ一覧の `LEVEL` 列がレベル配色で表示されることを確認（選択行では選択配色優先）
15. `x` でログがリセットされ、ログ一覧が空になることを確認
16. worktreeログが空の場合でも起動ディレクトリの最新ログが表示され、Sourceに理由が表示されることを確認
17. 最新ログがファイル名ではなく最終更新時刻で選択されることを確認

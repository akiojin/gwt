...
- [ ] 起動中判定のシェルを`if COMPOSE_PROJECT_NAME=... docker compose ... ps -q {service}; then`形式で生成する
...
### TASK-024: Service Selectのカーソル背景反転を1行全体に適用
...
- Dockerfile/compose変更検知がある場合のみ選択ダイアログが表示される
- 変更検知がない場合は選択ダイアログが表示されない
- No Build選択で`--no-build`が付与される
- Build選択で`--build`が付与される

---

### TASK-026: Codex認証のホスト同期
...
### TASK-027: 起動時のRecreate/Reuse選択ダイアログ

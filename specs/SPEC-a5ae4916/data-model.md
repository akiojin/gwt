...
| `latestCommitTimestamp` | `number \| undefined` | 最新コミットのUNIXタイムスタンプ（UIで「YYYY-MM-DD HH:mm」表示に使用） | worktree有無が同じグループ内での最新順ソートに使用 |
...
最新コミット時刻は UI レイヤーで `YYYY-MM-DD HH:mm` 形式に整形し、選択行は背景色をシアンに設定する。`Select` コンポーネントにはカスタム `renderItem` を渡し、ターミナル幅に合わせた右寄せと省略表示（`…`）を行う。

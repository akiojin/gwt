...
ブランチ一覧のソート順を拡張し、現在のブランチ → main → develop → worktree あり → 最新コミット時刻の新しい順 → ローカル → その他 → 名前順（リモートのみ）で表示する。加えて、各行に最新コミット時刻を表示し、選択中の行は背景色（シアン）で強調する。既存の `createBranchTable` 内ソート処理を最小改修で実現しつつ、UI の基本構造と既存データ構造を維持する。
...
3. **P2**: ブランチ一覧 UI に最新コミット時刻の表示と選択行ハイライトを追加
4. **P3**: エッジケーステストとパフォーマンス検証
...
- ストーリー4で最新コミット時刻の可視化と選択行の強調を提供

## テスト戦略

- **ユニット**: `createBranchTable` のソート結果をモックデータで検証。現在/ main/ develop/ worktree/ ローカル/ release/hotfix/ 名前順の全パターンをテスト。
- **ユニット(UI)**: `BranchListScreen` のレンダリングをテストし、最新コミット時刻の表示数・選択行ハイライト・既存アイコン表示を検証。

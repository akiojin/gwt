...
ブランチ一覧のソート順を拡張し、現在のブランチ → main → develop → worktree あり → 最新コミット時刻の新しい順 → ローカル → その他 → 名前順（リモートのみ）で表示する。既存の `createBranchTable` 内ソート処理を最小改修で実現し、UI 表示形式と既存データ構造を維持する。
...
**制約**: `BranchInfo` への追加は最小限に留める（既存プロパティ互換を維持）、UI アイコン/テキストを保持、ソートは安定に依存
...
2. **技術的決定**
   ...
   - 最新コミット時刻は Git から取得した UNIX タイムスタンプで比較
   - ローカル優先は `branch.type` 判定（コミット時刻が同一の場合）
...
2. **P2**: 最新コミット時刻の降順ソートを導入し、ローカル優先/ release・hotfix の一般ルールとの整合性を確認

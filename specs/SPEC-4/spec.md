...
### US-4 (P1): View Git Status (Staged/Unstaged Files, Diffs, Commits) — NOT IMPLEMENTED
...
### US-5 (P1): View PR Status, CI Checks, Merge State — NOT IMPLEMENTED
...
| FR-004 | Git View tab in management panel: file status (S/U/?), file diffs (lazy, max 50 lines), last 5 commits, divergence, PR link | P1 | Not Implemented |
| FR-005 | PR dashboard as independent management tab: PR list, CI check status, merge state, review status | P1 | Not Implemented |
| FR-006 | PR detail with CI check badges | P1 | Not Implemented |
| FR-007 | PR status from gwt-core::git::pr_status (PrStatus struct) | P1 | Not Implemented |
| FR-008 | GraphQL primary, REST fallback for GitHub API | P1 | Not Implemented |

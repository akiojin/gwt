...
**制約**: TDD必須、シークレットは***masked***、コンポーネント別ファイル分割禁止
**スケール/範囲**: ローカル利用前提（ログ容量は日上限なし・7日保持）
...
- `specs/SPEC-b9f5c4a1/quickstart.md`（pinoロガー設定手順と環境変数の使い方）
...
- **ユニットテスト**: ログフォーマット（timestamp/level/message/category）、ローテーション処理（起動時に7日超を削除）、categoryの付与、環境変数による設定上書き。

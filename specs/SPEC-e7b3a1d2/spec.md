...
ユーザーとして、Cmd+\` で複数の GWT ウィンドウ間をローテーション順で切り替えたい。
...
2. **前提条件** ウィンドウ A, B, C が開いており A がフォーカス中、**操作** Cmd+\` を3回連続押下、**期待結果** B→C→A と全ウィンドウを巡回する
...
ユーザーとして、Cmd+Shift+\` でウィンドウリストを逆方向に巡回したい。

**独立したテスト**: 3つのウィンドウで Cmd+Shift+\` を押下して逆方向に巡回すること

**受け入れシナリオ**:

1. **前提条件** ウィンドウ A, B, C が開いており A がフォーカス中、**操作** Cmd+Shift+\` を押下、**期待結果** C にフォーカスが移る（逆順巡回）
...
- **FR-008**: Cmd+\`（`CmdOrCtrl+Backquote`）でウィンドウをローテーション順（リスト巡回）に切り替える（Tauri accelerator 検証後、未サポートの場合は別キーに変更）
- **FR-009**: Cmd+Shift+\`（`CmdOrCtrl+Shift+Backquote`）でローテーション逆方向にウィンドウを切り替える
...
- **SC-004**: Cmd+\` / Cmd+Shift+\` でローテーション順にウィンドウが切り替わる（Tauri 対応確認後）

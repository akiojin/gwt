...
- Cmd+` の巡回対象に `New Window`（project 未選択）が含まれ、既存プロジェクトウィンドウ間の切り替え効率を下げている
...
ユーザーとして、Cmd+\` で project を開いている既存の GWT ウィンドウ間をローテーション順で切り替えたい。
...
2. **前提条件** project を開いているウィンドウ A, B があり、project 未選択の `New Window` C がある、A がフォーカス中、**操作** Cmd+\` を3回連続押下、**期待結果** B→A→B と巡回し、C は対象外になる
3. **前提条件** project を開いているウィンドウ A が表示中、project を開いている B を X ボタンで閉じた（非表示）、**操作** A で Cmd+\` を押下、**期待結果** B は巡回対象外となり何も起きない
...
ユーザーとして、Cmd+Shift+\` で project を開いているウィンドウリストを逆方向に巡回したい。

**独立したテスト**: 3つのウィンドウで Cmd+Shift+\` を押下して逆方向に巡回すること

**受け入れシナリオ**:

1. **前提条件** project を開いているウィンドウ A, B, C が開いており A がフォーカス中、**操作** Cmd+Shift+\` を押下、**期待結果** C にフォーカスが移る（逆順巡回）
2. **前提条件** project を開いているウィンドウ A, B があり、project 未選択の `New Window` C がある、A がフォーカス中、**操作** Cmd+Shift+\` を押下、**期待結果** B にフォーカスが移り、C は対象外になる
...
- project 未選択の `New Window` は Cmd+\` / Cmd+Shift+\` の巡回対象に含めない
- X ボタンで閉じた（非表示）ウィンドウは MRU 履歴から除外され、Cmd+\` / Cmd+Shift+\` の巡回対象に含めない
...
- **FR-008**: Cmd+\`（`CmdOrCtrl+Backquote`）で project を開いているウィンドウのみをローテーション順（リスト巡回）に切り替える（Tauri accelerator 検証後、未サポートの場合は別キーに変更）
- **FR-009**: Cmd+Shift+\`（`CmdOrCtrl+Shift+Backquote`）で project を開いているウィンドウのみを逆方向に切り替える
- **FR-010**: ウィンドウ切り替え時、X ボタンで閉じた（非表示）ウィンドウは MRU 履歴から除外し、巡回対象に含めない。再度プロジェクトを開いてフォーカスされた場合は MRU に自動復帰する
- **FR-011**: ウィンドウが1つのみの場合は Cmd+\` は何もしない
- **FR-012**: Window メニューに "Next Window" / "Previous Window" 項目を追加し、対応する accelerator を表示する
- **FR-013**: AppState にウィンドウフォーカス履歴（MRU リスト）を追加し、project を開いているウィンドウのみを保持する。メモリ内のみ、再起動時リセット
...
- **SC-004**: Cmd+\` / Cmd+Shift+\` で project を開いているウィンドウのみがローテーション順に切り替わる（Tauri 対応確認後）
- **SC-005**: X ボタンで閉じた（非表示）ウィンドウは Cmd+\` / Cmd+Shift+\` の巡回対象に含まれず、復活しない
- **SC-008**: project 未選択の `New Window` が存在しても、Cmd+\` / Cmd+Shift+\` の巡回対象に含まれない

...
Phase 5 (統合テスト)
    ↓
Phase 6 (US4: P1) - ローカルインストール版自動優先
```

**独立性**: US2とUS3は技術的にUS1完了を待たずに並行実装可能だが、テストの一貫性のため順次実装を推奨。US4はUS1完了後に実装することで、bunx基盤の上に自動検出機能を追加できる
...
### フェーズ6（US4）での並列実行
- T501（`isClaudeCommandAvailable()`実装）
- T503（ログ出力追加）
- T504（ユニットテスト作成）
- T505（統合テスト作成）
...
3. **MVP3（US1+US2+US3）**: UI/ドキュメント整合性完了
4. **完全版（US1+US2+US3+US4）**: ローカルインストール版自動優先
...
## フェーズ6: ユーザーストーリー4 - ローカルインストールClaude Codeの自動優先 (優先度: P1)

**ストーリー**: 開発者として、公式インストール方法でClaude Codeをローカルにインストール済みの場合、それを優先的に使用したい。

**価値**: 公式インストール版は最適化されており、起動が高速で安定性が高い。bunxは便利だが、インストール済みのツールがあればそれを使う方が開発体験が向上する。

**独立したテスト**: ローカルに`claude`コマンドがインストールされた環境と、されていない環境の両方でCLIを起動し、適切な方式が選択されるかを確認する。

### コマンド検出機能の実装

- [ ] T501 [P] [US4] `isClaudeCommandAvailable()`関数を実装（コマンド存在チェック） (src/claude.ts)
  - Unix系: `which claude`コマンドで検出
  - Windows系: `where claude`コマンドで検出
  - execaを使用してコマンド実行
  - 成功時: true、失敗時: false を返す

- [ ] T502 [US4] `launchClaudeCode()`に自動検出ロジックを追加 (src/claude.ts)
  - `isClaudeCommandAvailable()`でローカルインストール版を確認
  - ローカル版がある場合: `claude`コマンドで起動
  - ローカル版がない場合: bunxにフォールバック
  - 両方式で同じ引数（`-c`, `-r`, `--dangerously-skip-permissions`等）をサポート

- [ ] T503 [P] [US4] 起動方式のログ出力を追加 (src/claude.ts)
  - ローカル版使用時: 「Using locally installed claude command」
  - bunx使用時: 「Falling back to bunx @anthropic-ai/claude-code@latest」
  - chalkを使って視認性を向上

### テスト

- [ ] T504 [P] [US4] ユニットテスト: コマンド検出ロジックのテスト (tests/unit/claude.test.ts)
  - `isClaudeCommandAvailable()`のテスト
  - ローカル版コマンド存在時のテスト（モック）
  - ローカル版コマンド不在時のテスト（モック）
  - 自動選択ロジックのテスト

- [ ] T505 [P] [US4] 統合テスト: 両方式での起動確認 (tests/integration/ai-tool-launch.test.ts)
  - ローカル版優先起動のテスト
  - bunxフォールバックのテスト
  - 引数互換性のテスト（`-c`, `-r`等）
  - ログ出力の検証

**✅ MVP3チェックポイント**: US4完了後、ローカルインストール版が優先使用され、なければbunxフォールバック

...
3. **前提条件** 起動ウィザードが開いている、**操作** Model 入力欄の候補一覧を確認、**期待結果** エージェントごとに代表的なモデル候補（Codex: `gpt-5.1-codex`, `gpt-5.1-codex-max`, `gpt-5.2` / Claude: `sonnet` 等）が表示され、任意入力も可能である
...
- **FR-028**: Agent Version の候補一覧は、取得可能な場合は npm レジストリ由来のバージョン一覧を表示しなければ**ならない**。取得に失敗しても UI はクラッシュしては**ならない**（`latest` のみ表示 + 手入力で継続）

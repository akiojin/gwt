...
ローカルで `gwt` から Codex CLI を起動する開発者は、追加設定をしなくても既定で `gpt-5.1-codex` が利用され、OpenAI 側の最新機能をすぐに試すことができる。
...
1. **前提条件** Codex CLI がローカルにインストール済み、**操作** `gwt` で Codex ツールを選択、**期待結果** コンソールに新しいモデルがログされ、CLI は正常起動

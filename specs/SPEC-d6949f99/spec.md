...
1. **前提条件** Worktree一覧が表示されている、**操作** ソートボタンを確認する、**期待結果** 現在のモードが `Updated` である
2. **前提条件** 複数行が表示されている、**操作** ソートボタンを1回押下、**期待結果** 表示順が `Name`（ブランチ名昇順）へ切り替わる
...
- **FR-022**: Worktree一覧はソートモード切替をサポートし、`name` と `updated` の2種を切り替え可能であること。起動時のデフォルトは `updated`（更新日時降順）であること
...
- **FR-029**: `fetch_pr_status` はブランチ別 alias クエリではなく、Open PR 一覧の軽量 GraphQL クエリ（`pullRequests(states: OPEN)`）を使用しなければならない
- **FR-030**: `fetch_pr_status` のレスポンスは軽量サマリー（number/state/url/mergeable/base/head/check suites）のみを返し、レビュー/ラベル/変更サマリーは `fetch_pr_detail` でのみ取得しなければならない
- **FR-031**: バックエンドは `fetch_pr_status` に対してリポジトリ単位の短TTLキャッシュ（30秒）を保持し、TTL内は再フェッチを抑止しなければならない
- **FR-032**: レートリミット検知時はエラーを返す代わりに既存キャッシュを返し、クールダウン中の再フェッチを抑止しなければならない
- **FR-033**: クールダウン終了時刻は GraphQL `rateLimit.resetAt` を優先し、取得できない場合は 60 秒固定バックオフを使用しなければならない
...
- **NFR-006**: ツリー表示用 GraphQL クエリはレビュー/ラベル/アサイニー/変更サマリーを含めない（コスト最小化）
- **NFR-007**: レートリミット抑制（TTLキャッシュ + クールダウン）はバックエンドで完結し、フロントエンドの30秒ポーリング間隔は変更しない
- **NFR-008**: レートリミット発生時はユーザー向けエラー表示を増やさず、既存表示を維持する
...
- **SC-016**: `fetch_pr_status` 実行時の GraphQL クエリが Open PR 一覧ベースの軽量クエリになっていること（branch alias クエリを使用しない）
- **SC-017**: レートリミット検知時に `fetch_pr_status` が stale キャッシュを返し、フロントエンドの表示が維持されること
- **SC-018**: `fetch_pr_status` が軽量サマリーのみを返し、PR詳細情報は `fetch_pr_detail` で継続取得できること

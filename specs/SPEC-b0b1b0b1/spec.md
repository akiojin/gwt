...
1. **前提条件** ワークツリーに未コミット変更がある、**操作** AIツールを終了、**期待結果** 未コミット警告が表示され、Enterキー入力後にメインメニューへ戻る
...
- **FR-006**: 未プッシュ確認はAIツール終了直後に行い、未コミット変更がある場合はEnterキー待機、そうでなければ3秒待機を確認後に実行**しなければならない**
...
- 既存の`handleAIToolWorkflow`終了処理を維持する（未コミット変更がない場合は3秒待機）

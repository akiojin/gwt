...
3. **前提条件** プロジェクトが開かれている、**操作** ネイティブ `gwt > Preferences...` を実行する、**期待結果** Settings タブが開く

---

### ユーザーストーリー 4 - 同一プロジェクトの重複オープン防止（ディレクトリ基準）(優先度: P0)

開発者として、同一プロジェクト（同一ディレクトリ実体）を別ウィンドウで開こうとしたときは既存ウィンドウにフォーカスし、
同名でもディレクトリが異なるプロジェクトは別物として開きたい。

**この優先度の理由**: 1プロジェクト=1ウィンドウの運用を崩す重複オープンを防ぎ、マルチウィンドウ運用の一貫性を守るため。

**独立したテスト**: 2つのウィンドウで同一パス/異なるパス（同名ディレクトリ）を開くケースを自動テストで判定できる。

**受け入れシナリオ**:

1. **前提条件** ウィンドウAで `/work/a` が開かれている、**操作** ウィンドウBで `/work/a` を Open Project する、**期待結果** ウィンドウAが前面化・フォーカスされ、ウィンドウBの project 状態は変更されない
2. **前提条件** ウィンドウAで `/work/x/repo` が開かれている、**操作** ウィンドウBで `/work/y/repo`（同名ディレクトリ）を Open Project する、**期待結果** ウィンドウBで新規に開かれ、重複扱いにならない
3. **前提条件** `/work/a` と `/work/link-to-a` が同一実体（symlink）を指す、**操作** 別ウィンドウでそれぞれ開く、**期待結果** 同一プロジェクトとして重複防止が適用される

### エッジケース

- 同名ディレクトリ名のプロジェクトが複数開かれている場合でも、Windowメニュー上で区別できること
- プロジェクト未選択のウィンドウは Window メニューに表示されなくても良い（ユーザー要望）
- ウィンドウを閉じて非表示になったものは Window メニューに表示されないこと
...
- 存在しない（破棄済み）ウィンドウラベルが状態に残っていた場合、重複判定で検出しても stale state をクリーンアップして通常オープンへフォールバックできること

## 要件 *(必須)*

### 機能要件

- **FR-400**: アプリケーションはネイティブメニューバー（gwt/File/Edit/Git/Tools/Window）を提供**しなければならない**
...
- **FR-409**: 非表示（閉じて hide された）ウィンドウは `Window` メニューに表示**してはならない**
- **FR-410**: `open_project` は project root を canonical path へ正規化し、同一実体ディレクトリかどうかで重複判定**しなければならない**
- **FR-411**: 同一 canonical path が別ウィンドウで既に開かれている場合、既存ウィンドウを show+focus し、現在ウィンドウの project 状態を変更**してはならない**
- **FR-412**: canonical path が異なる場合（ディレクトリ名が同じでも）、別プロジェクトとして現在ウィンドウで開く**ことができなければならない**
- **FR-413**: フロントエンドは `open_project` の結果 `action` が `opened` のときのみ現在ウィンドウの `projectPath` を更新**しなければならない**
- **FR-414**: 本仕様の重複判定ロジックは Rust ユニットテスト（state/project）と GUI 側型チェックで回帰防止**されなければならない**
...
- **SC-403**: 同一 canonical path のプロジェクトを別ウィンドウで開こうとすると、既存ウィンドウへフォーカスされる
- **SC-404**: 同名ディレクトリでも canonical path が異なれば別プロジェクトとして同時に開ける
- **SC-405**: `cargo test -p gwt-tauri state::tests::` と `cargo test -p gwt-tauri commands::project::tests::` が成功する
- **SC-406**: `cd gwt-gui && pnpm exec svelte-check --tsconfig ./tsconfig.json` が成功する

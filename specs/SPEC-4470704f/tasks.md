...
## フェーズ5: 追加修正 - 同一プロジェクト重複オープン防止（2026-02-17）

### US4（TDD: RED → GREEN → VERIFY）

- [x] **T501** [US4] [RED] `crates/gwt-tauri/src/state.rs` に project identity 検索テスト（除外ウィンドウ考慮）を追加
- [x] **T502** [US4] [RED] `crates/gwt-tauri/src/commands/project.rs` に canonical path 正規化テストと `OpenProjectResult` シリアライズテストを追加
- [x] **T503** [US4] [GREEN] `crates/gwt-tauri/src/state.rs` に `window_project_identities` / `set_project_for_window_with_identity` / `find_window_by_project_identity` を実装
- [x] **T504** [US4] [GREEN] `crates/gwt-tauri/src/commands/project.rs` の `open_project` に canonical path 重複判定と既存ウィンドウフォーカス返却（`focusedExisting`）を実装
- [x] **T505** [US4] [GREEN] `gwt-gui/src/App.svelte` / `gwt-gui/src/lib/components/OpenProject.svelte` / `gwt-gui/src/lib/types.ts` を `OpenProjectResult.action` ベースに更新
- [x] **T506** [US4] [GREEN] `gwt-gui/e2e/support/tauri-mock.ts` の `open_project` モックを新レスポンス形式に更新
- [x] **T507** [US4] [VERIFY] `cargo test -p gwt-tauri state::tests::` を実行して成功
- [x] **T508** [US4] [VERIFY] `cargo test -p gwt-tauri commands::project::tests::` を実行して成功
- [x] **T509** [US4] [VERIFY] `cd gwt-gui && pnpm exec svelte-check --tsconfig ./tsconfig.json` を実行して成功
- [x] **T510** [US4] [VERIFY] `cd gwt-gui && pnpm exec playwright test e2e/open-project-smoke.spec.ts --project=chromium` を実行して成功

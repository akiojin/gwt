...
- **FR-031**: システムは、ペイン一覧に各ペインの状態アイコン、ブランチ名、エージェント名、稼働時間を表示**しなければならない**
- **FR-031a**: システムは、エージェントの状態を4段階（起動中/稼働中/停止/エラー）で判定**しなければならない**
- **FR-031b**: システムは、起動中・稼働中の状態をスピナーアニメーション（`|/-\`）で表示**しなければならない**
- **FR-031c**: システムは、停止状態を静的アイコン（`-`、グレー）で表示**しなければならない**
- **FR-031d**: システムは、エラー状態を静的アイコン（`!`、赤）で表示**しなければならない**
- **FR-031e**: システムは、ペイン内容の変更を監視し、5秒間更新がない場合に「停止」と判定**しなければならない**
...
- **FR-071**: システムは、tmuxモードでエージェント起動時にセッション情報をsession.jsonに保存**しなければならない**
- **FR-072**: システムは、tmuxモードでエージェント終了を検知した際にセッション情報を更新**しなければならない**
- **FR-073**: システムは、ブランチ一覧に最終使用エージェント情報を表示**しなければならない**（シングルモード・tmuxモード共通）

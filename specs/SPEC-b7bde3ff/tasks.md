...
### T-020: 右側列レイアウトの分割方向追加

- **ファイル**: `crates/gwt-core/src/tmux/launcher.rs`
- **内容**:
  - [ ] 右側列内の上下分割（水平分割）起動API追加
  - [ ] 既存の右側追加ロジックとの統合
- **依存**: T-007
- **テスト**: T-TEST-020

### T-021: 列/行の再配置とサイズ均等化

- **ファイル**: `crates/gwt-core/src/tmux/pane.rs`
- **内容**:
  - [ ] ペイン位置情報（left/top/width/height）の取得
  - [ ] 列ごとの行数判定と最大3行制約
  - [ ] 列数に応じた幅の均等化
  - [ ] 列内の高さ均等化
- **依存**: T-020
...
### T-TEST-020: 右側列分割APIテスト

- [ ] 分割方向の生成テスト
- [ ] 起動パラメータ生成テスト

### T-TEST-021: 列/行均等化テスト

- [ ] 列数算出テスト
- [ ] 高さ/幅均等化計算テスト

### T-TEST-022: レイアウト適用タイミングテスト

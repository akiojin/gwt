...
- **ファイル**: `crates/gwt-cli/src/tui/app.rs`
- **内容**:
  - [ ] gwtペインでESCキーハンドリング追加
  - [ ] ESC押下時にアクティブペインを非表示にする処理
  - [ ] アクティブペインがない場合は何もしない（空振り）
- **依存**: T-007
- **テスト**: T-TEST-029
...
- **内容**:
  - [ ] 新規エージェント起動時に既存アクティブペインを非表示にする
  - [ ] 非表示ペイン表示時に既存アクティブペインを非表示にする
  - [ ] シングルアクティブ制約の強制
- **依存**: T-029
- **テスト**: T-TEST-030
...
- **内容**:
  - [ ] group_panes_by_left() 等の複数ペイン用ヘルパーを廃止または簡略化
  - [ ] compute_equal_splits() の複数分割計算を削除
  - [ ] 関連するテストの更新または削除
- **依存**: T-030
- **テスト**: T-TEST-032
...
- **ファイル**: `crates/gwt-core/src/tmux/pane.rs`, `crates/gwt-cli/src/tui/app.rs`
- **内容**:
  - [ ] join-paneの`-P`/`-F`に依存しない再表示処理へ変更
  - [ ] show_pane/join_pane_to_targetの戻り値をpane_id維持に揃える
  - [ ] 呼び出し側のpane_id更新フローを維持
- **依存**: T-024
- **テスト**: T-TEST-025
...
- [ ] gwtペインでESC押下時にアクティブペインが非表示になること
- [ ] アクティブペインがない状態でESC押下時に何も起こらないこと

### T-TEST-030: 自動非表示テスト

- [ ] 新規起動時に既存アクティブが非表示になること
- [ ] 非表示ペイン表示時に既存アクティブが非表示になること
- [ ] 同時に表示されるペインが最大1つであること
...
- [ ] tmux set -g mouse on が実行されること

### T-TEST-025: join-pane 互換性テスト

- [ ] join-paneの`-P`/`-F`未対応環境でも再表示が成功すること
- [ ] pane_idが維持されること

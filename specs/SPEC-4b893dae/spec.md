...
11. **前提条件** セッション要約の生成が進行中、**操作** 30秒ポーリングが到来、**期待結果** 生成中の要約は中断されず、次回ポーリングは生成完了後に実行される
12. **前提条件** エージェントが作業中でセッションファイル更新が続いている、**操作** セッション要約タブで30秒ポーリングが発火、**期待結果** セッションファイルが直近の静止期間（例: 5-10秒）に到達していない場合は再生成を見送り、既存要約を保持したまま次回ポーリングで再評価する
...
- **SC-009**: セッション要約の30秒ポーリングが正確に動作する（誤差±5秒以内、生成中/静止期間待機を除く）

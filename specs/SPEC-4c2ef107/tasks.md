...
- [ ] T068 [US2] T067の後にsrc/ui/components/App.tsxにSessionSelector画面遷移を統合（ExecutionModeSelectorで「再開」を選択した場合）
...
- [ ] T105 [US2] src/index.tsからCLIフラグ解析・分岐を削除し、Ink UI起動のみを許可
- [ ] T106 [P] [US2] T105の後にshowHelp()およびusageテキストを更新（CLIフラグ非対応である旨を明記）
- [ ] T107 [P] README.md/README.ja.md/CHANGELOG.mdのCLIオプション記述を整理し、UI内フローへの移行手順を追記
- [ ] T108 [US2] 新UIで「続きから再開」「セッション再開」「ツール選択」が完了する統合テストを追加
...
- US2（Phase 4）: 25タスク
- US3（Phase 5）: 10タスク
- 統合・ポリッシュ（Phase 6）: 22タスク

**並列実行可能タスク**: 44タスク（[P]マーク付き）

**テストタスク**: 約51タスク（TDD により全コンポーネントでテスト作成）
...
- フィーチャーフラグは移行期間中のみ使用し、最終リリースではInk UIを唯一のエントリーポイントとする
- 旧CLIフラグ（`-c`/`-r`/`--tool`/`--` パススルー）は廃止予定のため、UI内の「続きから」「セッション再開」「ツール選択」導線を確実に整備する

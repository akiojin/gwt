...
### ユーザーストーリー 3 - 手動更新機能（rキー） (優先度: P3)

開発者がアプリケーションを使用中に、`r`キーを押すことで統計情報（ブランチ数、Worktree数、変更ファイル数など）を手動で更新できる。Git操作を実行した後、画面を再起動せずに統計を更新できる。

**この優先度の理由**: 既存のinquirer実装にはない新機能であり、Ink.jsの強みを活かせるが、コア機能ではないため、P3とする。

**独立したテスト**: 統計情報の取得と更新ロジックを独立してテストでき、手動更新機能のみを検証できる。

**受け入れシナリオ**:

1. **前提条件**: アプリケーションが起動している、**操作**: 別のターミナルでGit操作（ブランチ作成、削除など）を実行し、`r`キーを押す、**期待結果**: 統計情報が更新される
2. **前提条件**: ブランチ一覧が表示されている、**操作**: Worktreeを作成または削除した後、`r`キーを押す、**期待結果**: 統計情報が更新される

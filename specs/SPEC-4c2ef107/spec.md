...
- **FR-007**: システムは既存の業務フロー（ブランチ選択・作成、Worktree管理、PRクリーンアップ、セッション管理、AIツール選択）をUI内で完結させ、CLIフラグに依存せず提供**しなければならない**
...
- **FR-011**: システムはCLI引数による非対話型起動（`-c`, `-r`, `--resume`, `--tool`, `--`以降のパススルー）を廃止し、Ink UIの起動を唯一の公式エントリーポイントとして明示**しなければならない**
...
- **SC-005**: 旧CLIフラグ（`-c`, `-r`, `--tool` 等）で提供していたセッション再開・Worktree起動・ツール起動がUI経由で実行でき、回帰が発生しない
...
- **CLIオプションによる非対話型操作を提供しない**（`claude-worktree`は引数なしでのInk UI起動のみを公式サポートとする）
...
- **既存のユーザーは旧CLIフラグの代替として提供されるUI内アクションに移行できる**（キーボード操作が同じため）
...
- **旧CLIフラグやパススルー引数の維持**（`-c`, `-r`, `--resume`, `--tool`, `--`以降の任意引数は提供しない）

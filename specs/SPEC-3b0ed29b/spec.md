...
1. **前提条件** エージェント起動後すぐに非ゼロ終了コードで終了、**操作** 起動フローを完了、**期待結果** 画面にエラーが表示され、成功完了メッセージは表示されない
2. **前提条件** エージェント起動直後に想定外シグナルで終了、**操作** 起動フローを完了、**期待結果** エラー原因（exit code/signal）がログに記録される
3. **前提条件** ユーザーが明示的に中断（Ctrl+Cなど）した、**操作** 起動フローを終了、**期待結果** 中断は警告扱いで終了し、異常終了としては扱われない

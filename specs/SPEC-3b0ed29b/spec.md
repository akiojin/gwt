...
- エラーメッセージがモーダル幅（80文字）を超える場合、ステップ行とエラーメッセージ行を分離し、エラーメッセージはインデント付き別行で表示する。末尾は`...`で切り詰める
...
- **FR-052a**: エラーメッセージがモーダル幅を超える場合、ステップ行の直下にインデント付きの別行としてエラーメッセージを表示**しなければならない**。エラーメッセージ行はモーダル内部幅に収まるよう末尾を`...`で切り詰め**なければならない**

...
- **FR-013a**: 初期ツール選択画面（新規選択時）は、未サポートのビルトインツール（Qwen CLI: `qwen-cli`）を選択肢として表示してはならない
- **FR-013b**: Quick Start は、履歴として未サポートのビルトインツール（`qwen-cli`）を表示してもよいが、未サポートであることを明示し、選択できない状態（disabled）として表示しなければならない（FR-014参照）
- **FR-014**: Quick Start の履歴に未サポートのツール（`qwen-cli`）が含まれる場合、ユーザーに未サポートである旨を明示し、選択されても起動処理を中断しなければならない

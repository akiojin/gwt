...
1. **前提条件** gwtが起動しブランチが選択されている、**操作** エージェント選択画面でCodexを選択して起動、**期待結果** Codexが`--enable web_search`（v0.90.0以上）または`--enable web_search_request`（v0.89.x以下）、(v0.80.0未満の場合のみ)`--enable skills`、`--sandbox workspace-write`などのデフォルト引数付きで起動する
...
- **FR-201**: Codexは、デフォルト引数としてWeb検索機能を有効化する`--enable`フラグを含め**なければならない**。フィーチャー名はCodex CLIバージョンに応じて切り替える（FR-210参照）
...
- **FR-210**: Codexは、Codex CLIバージョンが0.90.0以上の場合は`--enable web_search`を、0.89.x以下の場合は`--enable web_search_request`を渡さ**なければならない**。バージョン判定不能の場合は`web_search`（最新の挙動）を優先する

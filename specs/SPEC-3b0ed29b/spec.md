...
**受け入れシナリオ**:
1. **前提条件** Claude Codeがグローバルインストール済み（`~/.bun/bin/claude`等に存在）、**操作** gwtを起動、**期待結果** ステータスに「Claude: installed」（緑色）と表示される
...
- **FR-019**: ツール状態はHeader下部の専用行に「ツール名: 状態」形式で表示**しなければならない**（状態: `installed` | `bunx`）
- **FR-020**: ブランチ選択時やツール起動時は、起動時にキャッシュした検出結果を再利用し、再検出を行っては**ならない**
- **FR-021**: ツール状態の表示は状態に応じて色分け**しなければならない**（`installed`: 緑、`bunx`: 黄色）

...
3. **前提条件** 権限スキップモードが有効、**操作** Claude Codeを起動、**期待結果** `--dangerously-skip-permissions`フラグと`IS_SANDBOX=1`環境変数が設定される
4. **前提条件** 権限スキップモードが有効、**操作** Geminiを起動、**期待結果** `-y`フラグが渡され、自動承認モードで起動する
5. **前提条件** 権限スキップモードが有効、**操作** OpenCodeを起動、**期待結果** OpenCodeはデフォルトで自動承認のため追加フラグなしで起動する（現時点で権限スキップ用のフラグは未提供）
...
### ユーザーストーリー 15 - 起動準備がUIをブロックせず進捗が見える (優先度: P1)

開発者がコーディングエージェント起動を確定した直後、TUIが固まらず、準備中の進捗が見えると安心して待機できる。

**この優先度の理由**: Skip Permissions 画面のまま停止して見える状態はフリーズに見え、作業継続を阻害するため。

**独立したテスト**: 起動確定直後にTUIが応答し続け、進捗表示が1秒以内に現れることを確認すれば検証できる。

**受け入れシナリオ**:

1. **前提条件** ブランチを選択し起動を確定、**操作** Skip PermissionsでEnterを押す、**期待結果** 1秒以内にTUI上のステータス表示領域へ英語の進捗が表示され、UI操作（カーソル移動など）が継続できる
2. **前提条件** tmux環境内で起動確定、**操作** 起動フローを進める、**期待結果** シングルモードと同一の起動準備パイプラインが実行され、依存インストールが有効な場合はtmuxモードでも同様に実行される
3. **前提条件** 起動準備が完了、**操作** 進捗を確認、**期待結果** 最終ステップ（起動開始）が明示され、TUIは次の状態に遷移する
...
- 起動準備が長時間化してもTUIの描画と入力は継続し、進捗表示は最新状態に更新される
...
- **FR-041**: 起動準備の進捗はTUI上のステータス表示領域に英語で表示し、ステップ更新ごとに最新状態へ更新**しなければならない**
- **FR-042**: 依存インストールが有効な場合、tmuxモードでもシングルモードと同様に同一パイプライン内で実行**しなければならない**
- **FR-043**: 起動準備中でもブランチ一覧のUI操作（カーソル移動/表示切替）が継続でき**なければならない**
...
- **SC-012**: Skip Permissions 確定後、1秒以内にTUI上で英語の進捗表示が見え、入力が応答することを確認できる

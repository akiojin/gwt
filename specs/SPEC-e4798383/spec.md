...
- Git管理外のディレクトリ（bareリポジトリの親ディレクトリなど）から起動しても、GitHubリモートを解決してIssue一覧を取得できる
...
- **FR-005d**: 現在ディレクトリがGitリポジトリでない場合でも、既定リモートから`owner/repo`を解決し、`gh issue list --repo owner/repo`でIssue一覧を取得**しなければならない**

...
開発者はブラウザでブランチ一覧を見て、作業したいブランチのリンクをクリックして個別のブランチ詳細ページにアクセスし、そこからWorktreeを作成できる。
...
1. **前提条件** Web UIが起動中、**操作** ブランチ一覧画面（`/`）にアクセス、**期待結果** ローカルとリモートのすべてのブランチがリンク付きでリスト表示される
2. **前提条件** ブランチ一覧が表示中、**操作** 任意のブランチリンクをクリック、**期待結果** ブランチ詳細ページ（`/:branchName`）に遷移し、ブランチ情報が表示される（例: `/feature-webui`、スラッシュを含むブランチ名は`/feature%2Fwebui`のようにURLエンコード）
3. **前提条件** ブランチ詳細ページ表示中、**操作** 「Worktreeを作成」ボタンをクリック、**期待結果** Worktreeが作成され、作成完了のメッセージが表示される
4. **前提条件** 既存のWorktreeがあるブランチの詳細ページ、**操作** ページにアクセス、**期待結果** 「Worktree already exists」のような情報が表示され、既存のWorktreeパスが示され、「Worktreeを作成」ボタンが無効化される
5. **前提条件** ブランチ一覧が表示中、**操作** 検索ボックスでブランチ名をフィルター、**期待結果** 入力内容に合致するブランチのみが表示される

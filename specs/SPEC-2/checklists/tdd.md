...
- [x] The latest implementation slice has spec-focused verification evidence attached to it (`cargo test -p gwt-tui ime_probe -- --nocapture`, `cargo build -p gwt-tui --example keytest`, `cargo run -p gwt-tui --example keytest -- --help`, plus smoke runs for `--mode redraw` and `--mode ratatui`).

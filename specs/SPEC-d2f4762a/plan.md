...
## 方針
- アイコンは絵文字で表示し、行頭のアイコン間にスペースを入れない。
  - 選択: `✅` / `☑️`
  - Worktree: `🟢` / `⚪` / `🔴`
  - 安全判定: `⭕️` / `❌`
...
3) **UI表示**: `BranchListScreen` を選択/Worktree/安全の絵文字アイコン列で表示し、ブランチ名開始位置を固定。フィルター中はスペース入力を優先して入力欄にし、選択トグルは通常モードで動作。
...
9) **UI補助表示**: フッターヘルプ直上に選択ブランチのフルパスを表示し、空リスト時は`Branch: (none)`を表示する。レイアウトの固定行数を調整する。

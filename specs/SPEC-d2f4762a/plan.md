...
- クリーンアップ実行中も画面更新を継続し、フッターにASCIIスピナーと`Cleanup: Running (done/total)`の進捗を英語で表示する。入力は継続可能とし、描画はブロックしない。
- クリーンアップ実行中は削除処理中のブランチの安全アイコン位置にASCIIスピナーを表示する。削除中ブランチはカーソル選択の対象外とし、マウスクリックでも選択しない。

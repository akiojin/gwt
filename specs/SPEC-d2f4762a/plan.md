...
5) **UI表示**: `BranchListScreen` を選択/Worktree/安全の ASCII アイコン列で表示し、ブランチ名開始位置を固定。Mode(tab)行の直下に安全アイコンの凡例行を追加する。フィルター中はスペース入力を優先して入力欄にし、選択トグルは通常モードで動作。unsafe警告ダイアログの反転表示はダイアログ枠内で完結させる。
...
12) **フッターヘルプ整理**: フッターのキーバインド一覧からFilter/Mode/Profilesを削除し、`Filter(f)`/`Mode(tab)`/`Profile(p)`を各表示行に併記する。

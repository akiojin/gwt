...
16. **前提条件** ブランチ一覧が表示されている、**操作** 画面を確認する、**期待結果** Mode(tab)行の直下に安全アイコンの凡例行が表示され、`Legend: o Safe  ! Uncommitted  ^ Unpushed  * Unmerged` が読める
...
- **FR-002**: システムは、ヘッダー下にMode(tab)行を表示**しなければならない**
...
- **FR-034**: システムは、現在の表示モードをMode(tab)行に「Mode(tab): All/Local/Remote」の形式で表示**しなければならない**
- **FR-034a**: システムは、Mode(tab)行の直下に安全アイコンの凡例行（`Legend: o Safe  ! Uncommitted  ^ Unpushed  * Unmerged`）を表示**しなければならない**

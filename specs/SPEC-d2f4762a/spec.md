...
開発者が誤削除を避けるためにクリーンアップ対象を目視確認しながら選びたい場合、ブランチ一覧で`space`キーを押すと行をトグル選択でき、複数ブランチを選んだ上で`c`キーを押すと選択済みだけをクリーンアップできます。ローカルブランチについては、未コミットがある場合は安全アイコン`!`を赤色、未プッシュがある場合は安全アイコン`!`を黄色、未マージの場合は安全アイコン`*`を黄色で表示します（未コミットがある場合は赤色`!`を優先）。リモートブランチは安全アイコンを空白で表示し、差分などの安全判定は非同期で行われるため判定中は安全アイコン位置にスピナーを表示します。upstreamに同名ブランチが存在しない場合は安全ではないと判定します。

**この優先度の理由**: 自動判定だけでは削除対象を誤解するリスクがあり、選択式を補完することで安全性と操作性を両立するため。

**独立したテスト**: 複数ブランチを選択→`c`で選択分のみ削除候補になること、選択が無い場合は従来の自動判定にフォールバックすること、未コミットのあるローカルブランチが赤色の`!`で表示されクリーンアップから除外されること、未プッシュのブランチが黄色の`!`で表示されること、未マージのブランチが黄色の`*`で警告表示されること、判定中は安全アイコン位置にスピナーが表示されること、リモートブランチは安全アイコンが空白で表示されることを確認する。
...
4. **前提条件** 未コミット変更があるローカルブランチを選択、**操作** `space`で選択→`c`を押す、**期待結果** 安全アイコンが赤色の`!`で表示され、クリーンアップ実行時にそのブランチは削除されず警告メッセージが表示される（クリーンな選択分は実行される）
5. **前提条件** 未プッシュのローカルブランチが表示されている、**操作** 画面を確認、**期待結果** 安全アイコン`!`が黄色で表示される
6. **前提条件** 未マージのローカルブランチが表示されている、**操作** 画面を確認、**期待結果** 安全アイコン`*`が黄色で表示される
7. **前提条件** 安全判定（差分・upstream）の取得中、**操作** 画面を確認、**期待結果** 安全アイコン位置にスピナーが表示される
8. **前提条件** リモートブランチが表示されている、**操作** 画面を確認、**期待結果** 安全アイコンは空白で表示される
...
- **安全判定の取得中**: 差分・upstream の判定中は安全アイコン位置にスピナーを表示する
...
- **FR-031**: システムは、表示アイコンとして以下の組み合わせを使用**しなければならない**。また、行頭のアイコン間にスペースを入れる  
  ...
  - 安全判定: `!`（未コミット/未プッシュ/安全でない）、`*`（未マージ）、(space)（安全にクリーンアップ可）
- **FR-031a**: システムは、安全アイコンの色を理由に応じて表示**しなければならない**  
  - 未コミットの場合は`!`を赤色  
  - 未プッシュの場合は`!`を黄色  
  - 未マージの場合は`*`を黄色  
  - それ以外の安全でない（例: upstreamに同名ブランチが存在しない）は`!`を赤色
- **FR-031b**: システムは、安全判定が非同期で進行中の場合は安全アイコン位置にスピナーを表示**しなければならない**

...
4. **前提条件** クリーンなローカルブランチが表示されている、**操作** 画面を確認、**期待結果** 安全アイコン`o`が明るい緑で表示される
5. **前提条件** 未コミット変更があるローカルブランチを選択、**操作** `space`で選択→警告でOKを選択→`c`を押す、**期待結果** 安全アイコンが赤色の`!`で表示され、クリーンアップ実行時にそのブランチも対象として削除される
6. **前提条件** 未プッシュのローカルブランチが表示されている、**操作** 画面を確認、**期待結果** 安全アイコン`!`が黄色で表示される
7. **前提条件** 未マージのローカルブランチが表示されている、**操作** 画面を確認、**期待結果** 安全アイコン`*`が黄色で表示される
8. **前提条件** 安全判定（差分・upstream）の取得中、**操作** 画面を確認、**期待結果** 安全アイコン位置にスピナーが表示され、完了したブランチから順に安全アイコンが更新される
9. **前提条件** 安全判定が取得中のローカルブランチがカーソル位置にある、**操作** `space`キーを押す、**期待結果** 「Safety check in progress. Select anyway?」の警告と OK/Cancel が表示され、OKを選ぶとチェックが付く
10. **前提条件** 安全判定が取得中のローカルブランチがカーソル位置にある、**操作** `space`キーを押して警告表示後に Cancel を選ぶ、**期待結果** チェックは付かず選択状態は変化しない
11. **前提条件** リモートブランチが表示されている、**操作** 画面を確認、**期待結果** 安全アイコンは空白で表示される
12. **前提条件** 安全判定が完了しており安全ではないローカルブランチ（`!`/`*`表示）がカーソル位置にある、**操作** `space`キーを押す、**期待結果** 「Unsafe branch selected. Select anyway?」の警告と OK/Cancel が表示され、OKを選ぶとチェックが付く
13. **前提条件** 安全判定が完了しており安全ではないローカルブランチ（`!`/`*`表示）がカーソル位置にある、**操作** `space`キーを押して警告表示後に Cancel を選ぶ、**期待結果** チェックは付かず選択状態は変化しない
14. **前提条件** 安全判定が完了しており安全ではないローカルブランチがチェック済み、**操作** `c`キーを押す、**期待結果** そのブランチはクリーンアップ対象に含まれ、処理が実行される
15. **前提条件** ブランチ一覧が表示されている、**操作** 画面を確認する、**期待結果** Mode(tab)行の直下に安全アイコンの凡例行が表示され、`Legend: o Safe  ! Uncommitted  ! Unpushed  * Unmerged` が読める
16. **前提条件** unsafe選択の警告ダイアログが表示されている、**操作** OK/Cancel を確認する、**期待結果** 反転表示はダイアログ枠内の幅に収まり、枠外へはみ出さない
17. **前提条件** unsafe選択の警告ダイアログが表示されている、**操作** Enter で OK/Cancel を確定する、**期待結果** ブランチ一覧の Enter 選択は発火せず、ブランチ選択ウィザードは表示されない
18. **前提条件** 選択済みブランチがあり`c`キーを押した直後、**操作** 画面を確認する、**期待結果** クリーンアップ確認ダイアログがブランチ一覧の上にポップアップ表示される
19. **前提条件** unsafe選択の警告ダイアログが表示されている、**操作** 背景を確認する、**期待結果** ブランチ一覧が背景として表示され、ダイアログが前面に表示される
20. **前提条件** クリーンアップを実行中、**操作** 画面を確認する、**期待結果** 画面は更新されず、進捗表示も行われない

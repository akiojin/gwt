...
12. **前提条件** ブランチ一覧にツール表示が存在する、**操作** 表示名を確認、**期待結果** `Codex` / `Claude` / `Gemini` / `OpenCode` の短縮名で表示される
...
### ユーザーストーリー 0c - マウスカーソル/クリックでブランチ選択 (優先度: P1)

開発者がマウス操作可能なターミナルでブランチ一覧を操作する場合、**マウスカーソルがある位置のブランチが選択状態になり**、ハイライトとWorktree表示が即座に追従する。さらに、**クリックでEnter相当のブランチ選択を確定**できる。キーボード操作は引き続き利用でき、マウス操作は補助として機能する。

**この優先度の理由**: 一覧操作の直感性を高め、キーボード操作に不慣れなユーザーでも迷わず対象ブランチを選択できるようにするため。

**独立したテスト**: ブランチ一覧に対してマウス移動イベントを発生させ、カーソル位置の行が選択されること、クリックでEnter相当の選択が実行されることをユニットテストで確認できる。

**受け入れシナリオ**:

1. **前提条件** ブランチ一覧が表示されている、**操作** マウスカーソルを任意の行に移動する、**期待結果** その行のブランチが選択状態になりハイライトが移動する
2. **前提条件** 別の行にカーソルが移動した、**操作** マウスカーソルを別行へ移動する、**期待結果** 選択状態が新しい行へ更新される
3. **前提条件** ブランチ一覧が表示されている、**操作** マウスカーソルを一覧領域外へ移動する、**期待結果** 選択状態は最後に選択された行のまま維持される
4. **前提条件** 確認ダイアログ/ウィザードポップアップが表示されている、**操作** マウスカーソルを一覧領域上に移動する、**期待結果** 背景のブランチ選択は変化しない
5. **前提条件** ブランチ一覧が空、**操作** マウスカーソルを動かす、**期待結果** 選択状態は変化せずエラーも発生しない
6. **前提条件** ブランチ一覧が表示されている、**操作** 任意の行をクリックする、**期待結果** そのブランチがEnter相当で選択され、ブランチ選択フロー（ウィザード）が開始される
...
- **空リストのマウス移動**: ブランチが存在しない場合でもマウス移動でエラーが発生しない
...
- **FR-005a**: システムは、ブランチ一覧領域内のマウスカーソル位置に応じて選択中ブランチを更新**しなければならない**
- **FR-005b**: システムは、一覧領域外のマウス移動やポップアップ表示中のマウス移動でブランチ選択を変更しては**ならない**
...
- **FR-007a**: システムは、ブランチ一覧の行クリックで`Enter`と同等のブランチ選択を実行**しなければならない**
...
- **SC-003a**: マウス移動でのブランチ選択変更が100ms以内に反映される
- **SC-003b**: ブランチ一覧のクリック選択が100ms以内に反映される

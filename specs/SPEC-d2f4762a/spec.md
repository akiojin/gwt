...
- **FR-012**: システムは、ブランチ一覧画面の"Working Directory"と"Stats"の間に、フィルター入力フィールドを常に表示**しなければならない**
- **FR-013**: システムは、フィルター入力フィールドでユーザーがテキストを入力できるようにし、入力内容に応じてリアルタイムでブランチリストを絞り込む**しなければならない**
- **FR-014**: システムは、ブランチ名に対して部分一致検索を実行し、大文字小文字を区別せずにマッチング**しなければならない**
- **FR-015**: システムは、ブランチにオープンPRが紐づいている場合、PRタイトルも検索対象に含める**しなければならない**
- **FR-016**: システムは、`Esc`キーが押されたときに、フィルター入力内容をクリアし、全ブランチを再表示する**しなければならない**
- **FR-017**: システムは、フィルター入力フィールドに入力中、他のキーバインド（`m`, `c`, `r`等）を無効化し、これらのキーを通常のテキスト入力として処理する**しなければならない**
- **FR-018**: システムは、フィルター入力フィールドに入力中でも、矢印キーまたは`j`/`k`キーによるブランチリストのカーソル移動と`Enter`キーによる選択を有効にする**しなければならない**

...
7. **前提条件** フィルターモードが有効、**操作** 矢印キー（↑/↓）を押す、**期待結果** ブランチ選択は移動しない
...
- **FR-004**: システムは、フッター行にキーバインドのヘルプ（Enter: Select, f: Filter, tab: Mode, n: New, r: Refresh, c: Cleanup, p: Profiles, l: Logs）を表示**しなければならない**
...
- **FR-021**: システムは、フィルターモード中の矢印キー入力でブランチ選択が移動しない**しなければならない**

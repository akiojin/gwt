...
- **FR-041a**: 最終アクティビティ時間の表示は、**システムロケール（OSのタイムゾーン）** に従ったローカル時刻で表示**しなければならない**
...
- **FR-070**: システムは、ブランチ一覧のツール表示を`ToolName@X.Y.Z | YYYY-MM-DD HH:mm`形式（**ローカル時刻**）で表示**しなければならない**（バージョン情報がない場合は`ToolName@latest | YYYY-MM-DD HH:mm`形式で表示する）

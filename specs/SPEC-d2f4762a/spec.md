...
開発者がgwtを起動した直後にブランチ一覧をすぐ見たい場合、CLIは長時間かかる状態チェック（未コミット/未プッシュ/未マージ・安全判定など）を待たずに一覧を即座に表示し、状態更新はバックグラウンドで進行します。更新中は「今何が起きているか」を明確に示す進捗表示があり、UIが固まらずに操作できるため、不安なく作業を継続できます。
...
3. **前提条件** 状態更新が進行中、**操作** 画面を確認する、**期待結果** 完了したブランチから安全アイコン/変更状態が順次更新され、進捗表示のカウントが増える
...
- **FR-075a**: システムは、詳細状態取得（未コミット/未プッシュ/未マージ/安全判定など）をバックグラウンドで実行し、完了したブランチから順次UIへ反映**しなければならない**

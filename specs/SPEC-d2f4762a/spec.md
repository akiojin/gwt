...
開発者がマウス操作可能なターミナルでブランチ一覧を操作する場合、**クリックした位置のブランチが選択状態になり**、ハイライトとWorktree表示が即座に追従する。マウス移動（ホバー）だけでは選択状態は変化せず、**クリックはブランチ選択フロー（Enter相当）を開始しない**。キーボード操作は引き続き利用でき、マウス操作は補助として機能する。

**この優先度の理由**: 一覧操作の直感性を高め、キーボード操作に不慣れなユーザーでも迷わず対象ブランチを選択できるようにするため。

**独立したテスト**: ブランチ一覧に対してマウスクリックイベントを発生させ、クリック位置の行が選択されること、クリックしてもウィザードが開始されないことをユニットテストで確認できる。

**受け入れシナリオ**:

1. **前提条件** ブランチ一覧が表示されている、**操作** マウスカーソルを任意の行に移動する、**期待結果** 選択状態は変化しない
2. **前提条件** ブランチ一覧が表示されている、**操作** 任意の行をクリックする、**期待結果** そのブランチが選択状態になり、ウィザードは開始されない
...
5. **前提条件** ブランチ一覧が空、**操作** マウスカーソルを動かす/クリックする、**期待結果** 選択状態は変化せずエラーも発生しない
...
- **FR-005a**: システムは、ブランチ一覧領域内の行クリックに応じて選択中ブランチを更新**しなければならない**
- **FR-005b**: システムは、マウス移動や一覧領域外/ポップアップ表示中のクリックでブランチ選択を変更しては**ならない**
...
- **FR-007a**: システムは、ブランチ一覧の行クリックで選択状態を更新**しなければならない**
...
- **SC-003a**: ブランチ一覧のクリック選択が100ms以内に反映される

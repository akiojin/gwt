...
9. **前提条件** リモートブランチが表示されている、**操作** 画面を確認、**期待結果** 安全アイコンは空白で表示される
10. **前提条件** 安全判定が完了しており安全ではないローカルブランチ（`!`/`*`表示）がカーソル位置にある、**操作** `space`キーを押す、**期待結果** 「Unsafe branch selected. Select anyway?」の警告と OK/Cancel が表示され、OKを選ぶとチェックが付く
11. **前提条件** 安全判定が完了しており安全ではないローカルブランチ（`!`/`*`表示）がカーソル位置にある、**操作** `space`キーを押して警告表示後に Cancel を選ぶ、**期待結果** チェックは付かず選択状態は変化しない
12. **前提条件** 安全判定が完了しており安全ではないローカルブランチがチェック済み、**操作** `c`キーを押す、**期待結果** そのブランチはクリーンアップ対象に含まれ、処理が実行される
13. **前提条件** ブランチ一覧が表示されている、**操作** 画面を確認する、**期待結果** Mode(tab)行の直下に安全アイコンの凡例行が表示され、`Legend: o Safe  ! Uncommitted  ! Unpushed  * Unmerged` が読める
14. **前提条件** unsafe選択の警告ダイアログが表示されている、**操作** OK/Cancel を確認する、**期待結果** 反転表示はダイアログ枠内の幅に収まり、枠外へはみ出さない
15. **前提条件** unsafe選択の警告ダイアログが表示されている、**操作** Enter で OK/Cancel を確定する、**期待結果** ブランチ一覧の Enter 選択は発火せず、ブランチ選択ウィザードは表示されない

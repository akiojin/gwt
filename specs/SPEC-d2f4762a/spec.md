...
- **FR-012**: システムは、ブランチ一覧画面で`f`キーが押されたときにフィルターモードを開始し、入力フィールドを"Working Directory"と"Stats"の間に表示**しなければならない**
- **FR-013**: システムは、フィルターモードが有効な間、ユーザーがテキストを入力できるようにし、入力内容に応じてリアルタイムでブランチリストを絞り込む**しなければならない**
- **FR-014**: システムは、ブランチ名に対して部分一致検索を実行し、大文字小文字を区別せずにマッチング**しなければならない**
- **FR-015**: システムは、ブランチにオープンまたはマージ済みのPRが紐づいている場合、PRタイトルも検索対象に含める**しなければならない**
- **FR-016**: システムは、フィルターモード中に`Esc`キーが押されたときに、フィルターモードを終了し、入力内容をクリアし、全ブランチを再表示する**しなければならない**
- **FR-017**: システムは、フィルターモードが有効な間、他のキーバインド（`m`, `c`, `r`等）を無効化し、それらを通常のテキスト入力として扱う**しなければならない**
- **FR-018**: システムは、フィルターモードが有効な間でも、矢印キーまたは`j`/`k`キーによるカーソル移動と`Enter`キーによる選択を有効にする**しなければならない**
- **FR-019**: システムは、フィルター検索の結果として現在表示されているブランチ数と全体のブランチ数を表示する**しなければならない**
- **FR-020**: システムは、フィルター入力が空の場合、全てのブランチを表示する**しなければならない**
- **FR-021**: システムは、検索キーワードがどのブランチにもマッチしない場合、空のリストを表示する**しなければならない**

...
7. **前提条件** Bunがインストール済みで`bunx @akiojin/gwt`を実行、**操作** gwtを起動する、**期待結果** ブランチ選択画面が表示される（bunxがNode実行になってもBunで再実行される）
8. **前提条件** リモート取得が認証待ち/ネットワーク遅延で完了しない、**操作** ブランチ一覧画面を表示、**期待結果** ローカルブランチとワークツリーが表示され、操作が継続できる（リモート取得は致命的なブロッカーにならない）
9. **前提条件** ブランチ情報の読み込み中、**操作** ブランチ一覧画面を表示、**期待結果** ブランチ一覧領域の先頭に1行のローディング表示（ASCIIスピナー付き）が表示され、読み込み完了時に消える
10. **前提条件** ブランチ一覧画面が表示されている、**操作** 一覧の表示領域を確認、**期待結果** ブランチ一覧が枠線で囲まれて表示される
11. **前提条件** ブランチ一覧にツール表示が存在する、**操作** 表示名を確認、**期待結果** `Codex` / `Claude` / `Gemini` / `OpenCode` の短縮名で表示される

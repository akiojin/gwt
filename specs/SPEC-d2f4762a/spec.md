...
5. **前提条件** 未コミット変更があるローカルブランチを選択、**操作** `space`で選択→`c`を押す、**期待結果** 安全アイコンが赤色の`!`で表示され、クリーンアップ実行時にそのブランチは削除されず警告メッセージが表示される（クリーンな選択分は実行される）
...
- **upstream未設定**: upstream に同名ブランチが存在しない場合、安全アイコンは`!`（赤）となりクリーンアップ対象から除外される
...
- **FR-030**: システムは、未コミットまたは未プッシュの変更があるブランチを含む状態で`c`キーが押された場合、そのブランチをクリーンアップ対象から除外し、除外理由をフッターまたはメッセージで即時通知**しなければならない**（クリーンな選択分は継続実行する）
- **FR-030a**: システムは、安全アイコン`!`または`*`が表示されているブランチをクリーンアップ対象から除外し、除外理由をフッターまたはメッセージで即時通知**しなければならない**（クリーンな選択分は継続実行する）

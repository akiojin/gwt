...
1. **コーディングエージェント選択** - Claude Code / Codex CLI / Gemini CLI / カスタム
2. **モデル選択** - 各エージェントで利用可能なモデル一覧から選択
3. **推論レベル選択**（Codex のみ）- low / medium / high
4. **実行モード選択** - Normal（新規セッション）/ Continue（前回セッションから続行）/ Resume（セッションを再開）
5. **権限スキップ確認** - Yes / No
...
8. **前提条件** コーディングエージェント選択ステップ、**操作** Claude Code を選択、**期待結果** モデル選択ステップに切り替わる
9. **前提条件** モデル選択ステップ（Claude/Gemini）、**操作** モデルを選択、**期待結果** 実行モード選択ステップに切り替わる
10. **前提条件** コーディングエージェント選択ステップ、**操作** Codex CLI を選択、**期待結果** モデル選択ステップに切り替わる
11. **前提条件** Codex のモデル選択後、**操作** モデルを選択、**期待結果** 推論レベル選択ステップに切り替わる
12. **前提条件** 推論レベル選択後、**操作** レベルを選択、**期待結果** 実行モード選択ステップに切り替わる
13. **前提条件** 実行モード選択ステップ、**操作** Normal を選択、**期待結果** 権限スキップ確認ステップに切り替わる
14. **前提条件** 権限スキップ確認ステップ、**操作** Yes を選択、**期待結果** ウィザードが閉じ、ワークツリー準備とAIツール起動が実行される
15. **前提条件** 任意のステップ、**操作** Escape キーを押す、**期待結果** 前のステップに戻る（最初のステップではウィザード終了）

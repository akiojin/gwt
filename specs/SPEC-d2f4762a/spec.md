...
開発者がgwtを起動した直後にブランチ一覧をすぐ見たい場合、CLIは長時間かかる状態チェック（未コミット/未プッシュ/未マージ・安全判定、upstream/差分、Worktreeアクセス可否など）を待たずに、最小情報（ブランチ名・現在ブランチ・Worktree有無）のみで一覧を即座に表示し、状態更新はバックグラウンドで進行します。更新中は「今何が起きているか」を明確に示す進捗表示があり、UIが固まらずに操作できるため、不安なく作業を継続できます。PRタイトル取得は既存の非同期処理として維持します。
...
1. **前提条件** 500件以上のブランチ/Worktreeが存在する、**操作** gwtを起動する、**期待結果** ブランチ一覧が状態チェック完了を待たずに表示され、進捗表示（例: `Status: Updating branch status (0/500)`）が表示される
2. **前提条件** 状態更新が進行中、**操作** 矢印キーやフィルター操作を行う、**期待結果** UIが固まらずに操作できる
3. **前提条件** 状態更新が進行中、**操作** 画面を確認する、**期待結果** 安全アイコン位置にスピナーが表示され、完了したブランチから安全アイコン/変更状態が順次更新され、進捗表示のカウントが増える
...
- **FR-075**: システムは、起動直後にブランチ一覧を表示する際、未コミット/未プッシュ/未マージ/安全判定などの詳細状態取得を待たずに最小情報（ブランチ名・現在ブランチ・Worktree有無）のみで一覧を表示**しなければならない**
- **FR-075a**: システムは、詳細状態取得（未コミット/未プッシュ/未マージ/安全判定、upstream有無/差分、Worktreeアクセス可否）をバックグラウンドで実行し、完了したブランチから順次UIへ反映**しなければならない**
- **FR-075b**: システムは、状態更新が進行中であることを示す進捗表示（ASCIIスピナー + 件数）をUI上に英語で表示**しなければならない**（例: `Status: Updating branch status (12/500)`）
- **FR-075c**: システムは、状態更新中もカーソル移動/フィルター/画面遷移などの入力がブロックされないように**しなければならない**
- **FR-075d**: システムは、状態更新の一部が失敗/タイムアウトした場合でもUIを継続表示し、未更新のブランチは不明状態として扱い、警告ログを残さなければ**ならない**
- **FR-075e**: システムは、`r`キーによる再取得時に進行中の状態更新を無効化し、最新の更新結果のみを反映**しなければならない**

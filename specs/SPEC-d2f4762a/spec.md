...
- **FR-063**: システムは、バージョン選択肢として「installed」「latest」および直近10バージョン（プレリリース含む）を表示**しなければならない**
- **FR-064**: システムは、npmレジストリからバージョン一覧を取得し、`dist-tags`と`versions`から直近10件を抽出**しなければならない**
- **FR-065**: システムは、npmレジストリ取得失敗時（タイムアウト・ネットワークエラー等）に「installed」「latest」のみを表示**しなければならない**
- **FR-066**: システムは、バージョン選択で「installed」が選択された場合、bunxのデフォルト動作（キャッシュ優先）で起動**しなければならない**
- **FR-067**: システムは、バージョン選択で「latest」が選択された場合、`@latest`サフィックスを付与して起動**しなければならない**
- **FR-068**: システムは、バージョン選択で具体的バージョンが選択された場合、`@X.Y.Z`サフィックスを付与して起動**しなければならない**
- **FR-069**: システムは、選択されたバージョン情報をセッション履歴（`ToolSessionEntry.toolVersion`）に保存**しなければならない**
- **FR-070**: システムは、ブランチ一覧のツール表示を`ToolName@X.Y.Z | YYYY-MM-DD HH:mm`形式で表示**しなければならない**（バージョン情報がない場合は従来通り`ToolName | YYYY-MM-DD HH:mm`）

...
- **FR-017**: システムは、フィルター入力フィールドに入力中、矢印キーまたは`j`/`k`キーによるブランチリストのカーソル移動と`Enter`キーによる選択を有効にする**しなければならない**
- **FR-018**: システムは、フィルター検索の結果として現在表示されているブランチ数と全体のブランチ数を表示する**しなければならない**
- **FR-019**: システムは、フィルター入力が空の場合、全てのブランチを表示する**しなければならない**
- **FR-020**: システムは、検索キーワードがどのブランチにもマッチしない場合、空のリストを表示する**しなければならない**

...
- **FR-025**: ChromaDB collections use `intfloat/multilingual-e5-base` as the embedding model. The runner transparently prepends a `passage:` prefix when embedding documents and a `query:` prefix when embedding queries; existing prefixes are detected to avoid double-application on re-index.

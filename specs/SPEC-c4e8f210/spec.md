...
1. **前提条件** ブランチ `feature/x` のエージェントタブが開いている、**期待結果** Sidebar（Local）で該当ブランチ名の前に ASCII スピナー（`| / - \\`）が表示される（reduced-motion の場合は `@` 固定表示）
2. **前提条件** Cleanup モーダルが開いている、**期待結果** 該当 Worktree 行のブランチ名の前に ASCII スピナー（`| / - \\`）が表示される（reduced-motion の場合は `@` 固定表示）
3. **前提条件** Cleanup モーダルに複数 Worktree が表示されている、**期待結果** スピナーが付いた Worktree は一覧の先頭に表示される（disabled を除く）
...
- **FR-515**: Sidebar（Local）のブランチ行に、該当ブランチの agent tab が開いている場合はブランチ名の前に ASCII スピナー（`| / - \\`）を表示しなければ**ならない**（reduced-motion の場合は `@` 固定表示）
- **FR-516**: Cleanup モーダルのブランチ列に、該当ブランチの agent tab が開いている場合はブランチ名の前に ASCII スピナー（`| / - \\`）を表示しなければ**ならない**（reduced-motion の場合は `@` 固定表示）

...
### ユーザーストーリー 6 - アクティブなエージェントタブの可視化（優先度: P1）

開発者として、どの Worktree にエージェントタブが開いているかを一目で把握し、誤ってクリーンアップしないようにしたい。

**受け入れシナリオ**:

1. **前提条件** ブランチ `feature/x` のエージェントタブが開いている、**期待結果** Sidebar（Local）で該当ブランチ名の前にグラフィカルなアクティビティインジケーター（アニメーションする3本バー）が表示される（reduced-motion の場合は `@` 固定表示）
2. **前提条件** Cleanup モーダルが開いている、**期待結果** 該当 Worktree 行のブランチ名の前にグラフィカルなアクティビティインジケーター（アニメーションする3本バー）が表示される（reduced-motion の場合は `@` 固定表示）
3. **前提条件** Cleanup モーダルに複数 Worktree が表示されている、**期待結果** インジケーター付き Worktree は一覧の先頭に表示される（disabled を除く）
4. **前提条件** スピナー付き Worktree を選択して Cleanup を実行しようとする、**期待結果** "Active Agent Tabs Detected" の確認ダイアログが表示される

**注記**: この仕様でいう "アクティブ" はバックエンドの `is_agent_running` ではなく、GUI のタブとして「該当ブランチの agent tab が存在する」ことを意味する。
...
- リモートブランチは削除しない（ローカルブランチのみ削除）
  - **⚠️ SPEC-ad1ac432 により上書き**: 「Also delete remote branches」トグル ON 時はリモートブランチも削除対象
...
- **FR-508**: 削除は Worktree ディレクトリとローカルブランチをセットで削除しなければ**ならない**。リモートブランチは削除しては**ならない**
  - **⚠️ SPEC-ad1ac432 により上書き**: トグル OFF 時のみ適用。トグル ON 時はリモートブランチも削除対象
...
- **FR-512**: コンテキストメニューに "Cleanup this branch"（単体即時削除、常に確認ダイアログ）と "Cleanup Worktrees..."（モーダル起動、プリセレクト付き）の2項目を提供しなければ**ならない**
  - **⚠️ SPEC-ad1ac432 により上書き**: "Cleanup this branch" は CleanupModal を該当ブランチプリセレクトで開くように変更。単体即時削除ルートは廃止
- **FR-513**: 新規 "Git" メニューをメニューバーに追加し、"Cleanup Worktrees..." を配置しなければ**ならない**
- **FR-514**: Cmd+Shift+K で Cleanup モーダルを起動できなければ**ならない**
- **FR-515**: Sidebar（Local）のブランチ行に、該当ブランチの agent tab が開いている場合はブランチ名の前にグラフィカルなアクティビティインジケーター（アニメーションする3本バー）を表示しなければ**ならない**（reduced-motion の場合は `@` 固定表示）
- **FR-516**: Cleanup モーダルのブランチ列に、該当ブランチの agent tab が開いている場合はブランチ名の前にグラフィカルなアクティビティインジケーター（アニメーションする3本バー）を表示しなければ**ならない**（reduced-motion の場合は `@` 固定表示）
- **FR-517**: Cleanup モーダルは、agent tab が開いている Worktree を一覧の先頭に表示しなければ**ならない**（disabled は常に末尾）
- **FR-518**: agent tab が開いている Worktree が選択されている場合、Cleanup 実行前に追加の確認ダイアログを表示しなければ**ならない**
- **FR-519**: Cleanup モーダルが開いている間に Worktree 一覧が更新された場合、一覧をリフレッシュしつつ有効な選択状態を維持しなければ**ならない**
...
- ~~リモートブランチの削除~~ → **SPEC-ad1ac432 で対応済み**
- 自動クリーンアップ（起動時の orphan 自動削除）
- ~~GitHub PR マージ状態の確認による安全性判定~~ → **SPEC-ad1ac432 で対応済み**

...
1. **前提条件** ブランチ `feature/x` のエージェントタブが開いている、**期待結果** Sidebar（Local）で該当ブランチ名の前に ASCII アイコン `@` が表示される
2. **前提条件** Cleanup モーダルが開いている、**期待結果** 該当 Worktree 行のブランチ名の前に ASCII アイコン `@` が表示される
3. **前提条件** Cleanup モーダルに複数 Worktree が表示されている、**期待結果** `@` が付いた Worktree は一覧の先頭に表示される（disabled を除く）
4. **前提条件** `@` 付き Worktree を選択して Cleanup を実行しようとする、**期待結果** "Active Agent Tabs Detected" の確認ダイアログが表示される
...
- **FR-515**: Sidebar（Local）のブランチ行に、該当ブランチの agent tab が開いている場合はブランチ名の前に ASCII アイコン `@` を表示しなければ**ならない**
- **FR-516**: Cleanup モーダルのブランチ列に、該当ブランチの agent tab が開いている場合はブランチ名の前に ASCII アイコン `@` を表示しなければ**ならない**

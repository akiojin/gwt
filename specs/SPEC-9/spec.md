...
5. Given a user starts with a rough idea, when `gwt-spec-brainstorm` is invoked, then it performs one-question-at-a-time intake, searches existing owners first, and routes to an existing SPEC, a new SPEC, or a plain Issue without creating `spec.md` prematurely.
...
- **FR-010**: `gwt-spec-brainstorm` must provide a cross-agent pre-SPEC intake workflow that performs duplicate search first, interviews the user one question at a time, and routes to `EXISTING-SPEC`, `NEW-SPEC`, or `ISSUE` before any `spec.md` is drafted.

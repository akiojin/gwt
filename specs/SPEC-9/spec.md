...
- **FR-024b**: Embedded runtime-hook distribution must not assume interactive Codex emits `SessionStart` immediately on launch. Downstream launch/bootstrap logic may pre-seed a `Running` sidecar before the first interactive hook event arrives.

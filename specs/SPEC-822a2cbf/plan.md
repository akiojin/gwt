...
**制約**: 80桁想定のターミナル幅で崩れない。ANSI非対応時も `> ` などのプレフィックスで識別可能。
...
2. **技術的決定**
   - 強調表示の実装手段: 背景色ハイライトを基本とし、モノクロ時は `> ` プレフィックスで代替
...
1. **chalk背景色がテーマによって読みにくい**
   - **緩和策**: `chalk.supportsColor` に応じ `> ` プレフィックスのみのフォールバックを継続し、アイコン整列が崩れないよう空白パディングを徹底

...
- **FR-003**: 変更状態アイコンは以下のいずれかを表示し、同じく固定幅を維持しなければならない：
  - ✏️（未コミットあり）: ローカルブランチに未コミットの変更が存在する
  - ⬆️（未プッシュあり）: ローカルブランチにリモートへ未プッシュのコミットが存在する
  - 🔀（未マージPRあり）: オープン状態のプルリクエストが存在する
  - ✅（マージ済みPR）: マージ済みだがローカルブランチがクリーンアップされていない
  - ⚠️（警告／異常）: アクセス不可能なワークツリーなど異常状態を示す
  - ⭐（現在ブランチ）: 現在チェックアウトされているブランチ
  - 空欄（なし）: 上記いずれにも該当しない通常状態
  - 複数条件が該当する場合は優先度順（✏️ > ⬆️ > 🔀 > ✅ > ⚠️ > ⭐）で1つのみ表示する

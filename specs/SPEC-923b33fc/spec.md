...
- Agent タブ有効時は Sidebar 側のフォールバック再取得が周期的に走り、`list_worktree_branches` / `list_remote_branches` / `list_worktrees` の実行負荷が重なると体感フリーズになりやすい
...
### ユーザーストーリー 3 - 周期再取得が来てもUI操作を止めたくない (優先度: P0)

開発者として、Agent状態のフォールバック再取得が発生しても、バックエンド列挙処理がUI応答を阻害しないようにしたい。

**独立したテスト**: ブランチ/ワークツリー列挙コマンドの回帰テストが通り、既存レスポンス契約を維持したまま実行モデル変更が適用されること

**受け入れシナリオ**:

1. **前提条件** Agent タブを開いた状態で周期再取得が発生、**操作** Branch/Worktree 系コマンドを実行、**期待結果** 重い列挙処理が `spawn_blocking` 経路で実行される
2. **前提条件** コマンド名・引数・戻り値の既存契約がある、**操作** フロントエンドから既存 `invoke` を実行、**期待結果** API 契約互換を維持したまま応答できる
...
- **FR-006**: `list_worktree_branches` / `list_remote_branches` / `list_worktrees` は、重い列挙処理を `spawn_blocking` 経路で実行しなければならない
- **FR-007**: FR-006 の適用後も、Tauri command 名・引数・戻り値スキーマを変更してはならない
...
- Agent タブ有効時の周期再取得（フォールバック）は残るため、バックエンド側で列挙処理の実行モデルを分離する
...
- **SC-005**: `cargo test -p gwt-tauri commands::branches -- --nocapture` が成功する
- **SC-006**: `cargo test -p gwt-tauri commands::cleanup -- --nocapture` が成功する

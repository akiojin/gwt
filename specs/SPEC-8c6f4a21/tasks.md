# タスクリスト: Windows での Git 実行時コンソール点滅抑止
...
- [x] T003 [US1] `crates/gwt-core/src` の Git 実行を `git_command()` 経由へ統一
- [x] T004 [US1] `crates/gwt-tauri/src` の Git 実行を `gwt_core::process::git_command()` 経由へ統一
- [x] T005 [US1] 置換に伴う未使用 import と軽微な整形差分を解消

## Phase 3: ストーリー 1

- [x] T006 [US1] Windows で `CREATE_NO_WINDOW` を適用する実装を追加 (`crates/gwt-core/src/process.rs`)
- [x] T007 [US1] `process.rs` のユニットテスト追加

## Phase 4: ストーリー 2

- [x] T008 [US2] 直接 `Command::new("git")` 混入を検出するガードテスト追加 (`crates/gwt-core/tests/no_direct_git_command.rs`)

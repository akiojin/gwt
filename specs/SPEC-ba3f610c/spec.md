...
| 層 | 内部名 | UI表示名 | 役割 | 実行場所 |
|---|---|---|---|---|
| PM | PM | Lead | プロジェクト統括・要件定義・GitHub Issue仕様管理・進捗管理・段階的委譲 | gwt内蔵AI |
...
3. Leadが要件をIssue単位に分割し、各IssueについてGitHub Issueを作成する（仕様・計画・タスク・TDDをIssueに記録）
4. LeadがGitHub Projectに各Issueを登録する
5. Leadがユーザーに**プロジェクト全体の計画**を提示し承認を得る
6. 承認後、Leadが各Issueに対してCoordinatorを内蔵ターミナルペインで起動し、**GitHub Issue番号**を渡す（複数Coordinator並列起動可）
7. 各CoordinatorがGitHub Issueから仕様・タスクを読み取り、Worktreeを作成してDeveloperを起動する（1 Task = N Developer = N Worktree可）
8. CoordinatorがDeveloper完了検出 → テスト検証 → PR作成を行う
9. CoordinatorがCI結果を監視し、失敗時はDeveloperに修正を指示する（自律修正ループ）
10. Leadが全Coordinatorの進捗を把握し、ユーザーに定期報告する
11. 各Issue完了（全タスク完了・CI全パス）後、LeadがGitHub Issueをクローズし、Projectステータスを更新する
12. 全Issue完了後、プロジェクト完了とする
...
**この優先度の理由**: GitHub Issueベースの仕様管理がプロジェクトチームの核心機能であり、Coordinator/Developer実行の前提条件となる。
...
3. **前提条件** Lead画面表示中、**操作** ダッシュボードでIssueを展開、**期待結果** Coordinator詳細と配下Developer一覧が表示される
...
- CoordinatorにはGitHub Issue番号を渡し、Coordinator自身がissue_specツール（スキル）でIssueを読んでタスクを把握する。
...
- これらのツールはgwt内蔵（`agent_tools.rs`）とMCPサーバーの両方で提供される。
- **issue_specツールはClaude Codeプラグインのスキルとしても公開**し、ブランチモードの各エージェントからも利用可能にする。
- GitHub Issueに記録するセクション:
  ...
  - **tdd**: テスト戦略とテストケース
- **モード横断**で利用可能:
  - プロジェクトチーム: Leadが自動的にGitHub Issueを作成・更新
  - ブランチモード: 各エージェントがissue_specスキルでGitHub Issueを参照・更新

### GitHub Issue仕様管理ワークフロー（プロジェクトチーム）
...
- **Leadが仕様管理ワークフロー全体を実行する**（Coordinatorは完成済みGitHub Issue番号を受け取る）:
  ...
  9. **実行**: CoordinatorがGitHub Issueからタスクを読み取り、各Developerに割り当てて自律実行する
- LeadはGitHub Issueに仕様4セクション（spec/plan/tasks/tdd）が揃うまでCoordinatorを起動してはならない。

### タスク分割の入出力仕様

- タスク生成はLeadがGitHub Issueのtasksセクションに記録する。
- CoordinatorはGitHub Issueから読み取ったタスク定義の各タスクにDeveloperを割り当てる。
...
- **Developerのエージェント種別はプロジェクトチーム起動時にユーザーが指定する**。指定されたエージェント種別が全Developerに適用される。
...
- プロジェクトチームは、**ダッシュボード（左）+ Leadチャット（右）**の2カラム構成を基本とする。
- Developer表示は既存のブランチ/Worktreeタブ（Branch Mode）をそのまま利用する。
- タブ名: `Project Team`
...
|  T1: 2 devs  | Lead: "Plan approved. Starting 3           |
|  T2: 1 dev   |        Coordinators..."                    |
|              |                                            |
...
- 表示階層: Issue → Task → Developer（折りたたみ可）
- 各Issueに表示する情報:
  - Issue番号 + タイトル
...
- Issueをクリックすると展開し、配下のTask/Developer詳細を表示する。
- Taskをクリックすると、Branch Mode側で当該Developer/Worktreeの詳細にジャンプする。
...
#### Developer表示（Branch Mode連携）

- Developer（Worker）は既存のBranch Modeタブで**ブランチ/Worktree単位**で表示する。
- `agent/`プレフィックスのブランチがProject Teamの成果物として表示される。
...
- Coordinator名、ステータス、CI結果、配下Developer一覧。
...
- **層ごとに異なる管理方式**:
  - **Developer**: LLMの自動コンテキスト圧縮に任せる（Claude Code等の組み込み機能を利用）。gwt側では制御しない。
...
- **FR-051**: CoordinatorはLeadから受け取ったGitHub Issue番号に基づき、Issueから仕様・タスクを読み取りDeveloperを管理しなければならない
...
- **FR-080**: プロジェクトチームUIはダッシュボード（左）+ Leadチャット（右）の2カラム構成でなければならない
- **FR-081**: ダッシュボードにIssue/Task/Developerの階層をステータス付きで常時表示しなければならない
...
- **FR-084**: Developer表示は既存のBranch Modeタブをそのまま利用しなければならない
- **FR-085**: Developerのターミナルペイン（Branch Mode側）にユーザーが直接キー入力できなければならない
...
- **FR-018**: issue_specツール群はgwt内蔵（agent_tools.rs）とMCPサーバーの両方で提供しなければならない
- **FR-019**: issue_specツールはClaude Codeプラグインスキルとしても公開し、プロジェクトチーム・ブランチモード両方から利用可能でなければならない
...
- **TR-002**: タスク選択時にダッシュボード内でCoordinator/Developer情報が展開表示されることを自動テストで検証する

...
**ステータス**: 更新済み
**カテゴリ**: GUI
**入力**: ユーザー説明: "エージェントモード: マスターエージェント（gwt内蔵LLM、既存AI要約と同じAPI設定共有）がユーザーと対話し、タスクを完全自律的に分割・計画し、複数のサブエージェント（Claude Code等）をGUI内蔵ターミナルでオーケストレーションする新機能。Tool Callingで送信/キャプチャを行う。"

## ユーザーシナリオとテスト *(必須)*

### ユーザーストーリー 1 - モード切り替えと基本対話 (優先度: P1)

開発者がGUIのタブバーで`Agent Mode`タブを選ぶと、マスターエージェントとの対話画面が表示される。ユーザーは自由形式でタスクを入力し、マスターエージェントがタスクを分析・計画を応答する。

**この優先度の理由**: エージェントモードへの入口であり、マスターエージェントとの対話がすべての機能の基盤となる。

**独立したテスト**: `Agent Mode`タブを開き、タスクを入力して応答を受け取ることで検証できる。

**受け入れシナリオ**:

1. **前提条件** ブランチモードが表示されている、**操作** `Agent Mode`タブを選択、**期待結果** エージェントモードに切り替わり、チャット画面が表示される
2. **前提条件** エージェントモードが表示されている、**操作** `Settings`や`Version History`など別タブを選択、**期待結果** ブランチモードに戻る
...
マスターエージェントがサブエージェント（Claude Code等）をGUI内蔵ターミナルペインで起動し、Tool Callingの`send_keys_to_pane`でプロンプトを送信する。プロンプトには「タスクが完了したらqで終了して」という指示を含める。

**この優先度の理由**: サブエージェントへの指示がタスク実行の実体であり、エージェントモードの価値を実現する核心機能。

**独立したテスト**: タスク実行を開始し、サブエージェントがGUI内蔵ターミナルペインで起動してプロンプトを受け取ることを確認することで検証できる。

**受け入れシナリオ**:

1. **前提条件** worktreeが作成済み、**操作** タスク実行開始、**期待結果** GUI内蔵ターミナルペインでサブエージェント（Claude Code等）が起動
2. **前提条件** サブエージェントが起動、**操作** マスターエージェントがプロンプト送信、**期待結果** Tool Callingの`send_keys_to_pane`でプロンプトが入力される
3. **前提条件** プロンプト送信、**操作** プロンプト内容を確認、**期待結果** タスク指示と「完了したらqで終了」の指示が含まれる
4. **前提条件** 複数サブエージェントを起動、**操作** 並列実行、**期待結果** 各エージェントが別々のGUI内蔵ターミナルペインで動作

---

### ユーザーストーリー 4 - サブエージェント完了検出 (優先度: P1)

マスターエージェントがサブエージェントの完了を検出する。Claude CodeはHookのStop経由、他のエージェントはGUI内蔵ターミナルの複合方式（プロセス終了監視 + スクロールバック出力パターン監視 + アクティビティ監視）で検出する。
...
2. **前提条件** Claude Code以外のエージェントが動作中、**操作** プロセスが終了、**期待結果** ペイン終了検出で完了が検出される
...
- サブエージェント起動中にGUIウィンドウが閉じられた場合、どう復旧するか？
...
- マスターエージェントは以下のフローでSpec Kitを自動的に組み込む:
  ...
  5. **TDD策定**: tasks.mdを入力として、テスト戦略とテストケースを定義した`tdd.md`を自動生成する
  6. **一括承認**: 仕様概要 + 計画 + タスク一覧 + TDD要約をチャットでユーザーに提示し、一括承認を得る
  7. **実行**: 承認後、各タスクをサブエージェントに割り当てて自律実行する
- 仕様の承認は計画の一括承認に含まれる（別ステップは設けない）。
- サブエージェントはTDDで実装を行う（テスト作成→実装→テストパスの順）。
- MAは`spec.md`/`plan.md`/`tasks.md`/`tdd.md`が揃うまで実装実行フェーズに進んではならない。
...
- 以下のイベントがLLMコールをトリガーする:
  - サブエージェント完了検出（Hook / ペイン終了 / スクロールバックパターン）
...
### Tool Calling I/O（GUI版）

- Tool CallingはOpenAI互換Responses APIの`tool_calls`を使用する。
- 対応ツールは次の3つのみ。
  - `send_keys_to_pane`
  - `send_keys_broadcast`
  - `capture_scrollback_tail`
- 引数は`pane_id`/`paneId`、`max_bytes`/`maxBytes`の表記ゆれを許容する。
- 失敗時のエラーはsystemメッセージとして会話に記録し、ユーザーに可視化する。
- `send_keys_to_pane`の戻り値は`"ok"`固定とする。
- `send_keys_broadcast`の戻り値は送信成功したペイン数の文字列とする。
- `capture_scrollback_tail`の戻り値はANSI除去済みのプレーンテキストとする。
- マスターエージェントはReAct形式で応答し、UIに`Thought/Action/Observation`を表示する。

### GUIターミナル要件と非対応時の挙動

- GUI内蔵ターミナルが初期化できない場合はエージェントモード実行を拒否する。
- 画面には英語で"Terminal is required"などのメッセージと再起動手順を表示する。
...
- 複合方式の判定条件は以下のいずれか:
  ...
  - Tool Callingによる完了確認（マスターがサブエージェントに状態確認クエリを送信）
...
- `GWT_TASK_DONE`はサブエージェントの標準出力に含まれることを必須とする。
- 完了確認クエリの送信は`send_keys_to_pane`のみを使用する。
...
- ユーザーが承認（Enterまたは"y"）すると、以降のWT作成・サブエージェント起動・テスト検証・PR作成はすべて自律実行される。
...
- 保存フォーマットはJSONで、会話履歴・タスク一覧・進捗状態を含む。
- GUI再起動時は最新の未完了セッションを自動で復元する。
- 復元時に参照worktreeが消失している場合は該当タスクを`Failed`にする。

### エージェントモードUI

- エージェントモードは、**マスターエージェントチャット**と**サイドバーのAgentビュー**で構成する。
- Agentビューにはタスク一覧を表示し、各タスクの状態（pending/running/completed/failed）を可視化する。
- Agentビューのタスク一覧は `running > pending > failed > completed` の順で表示する。
- ユーザーがタスクを選択すると、Agentビュー下部に当該タスクを担当するサブエージェント一覧を表示する。
- サブエージェント一覧には、エージェント名・状態・割り当てworktreeパスを表示する。
- サブエージェント一覧は上限を設けず、当該タスクに紐づく担当エージェントを全件表示する。
- 1つのタスクに対して複数サブエージェントの同時割り当てを許可する。
- worktree表示はリポジトリ相対パスをデフォルトとし、ホバーまたは詳細表示で絶対パスを確認できるようにする。
- タスク一覧とチャットは同一セッションを参照し、マスターエージェントの再計画結果を即時反映する。
- 再計画で担当が変更された場合、Agentビューには**現在の担当エージェントのみ**を表示し、過去担当は一覧に表示しない。
- チャット画面の下部に入力エリア、上部にチャット履歴を表示する。
- チャット履歴は会話形式（バブル）で表示し、ユーザー発言は右寄せ、アシスタントは左寄せとする。
- システム/ツール/内部種別（Thought/Action/Observation/Error）は中央寄せでメタラベルを表示する。
- 入力はEnter送信、Shift+Enter改行とし、IME変換中（composition中）のEnterでは送信しない。
- 送信中（リクエスト待機中）は送信ボタンにスピナーを表示し、連打を防止する。
...
- 中断時の処理:
  1. 全サブエージェントのGUI内蔵ターミナルペインにSIGTERMを送信する
...
- 承認時には、`tdd.md`の要約（対象機能、主要テストケース、失敗系テスト）を必ず併記する。
...
- マスターエージェントのLLM APIコール数と推定トークン数をGUI上に表示する。
...
- **FR-001**: システムはGUIのタブバーでブランチモードとエージェントモードを切り替えできなければならない
...
- **FR-005**: システムはGUI内蔵ターミナルとTool Calling（`send_keys_to_pane`, `capture_scrollback_tail`等）でサブエージェントを起動・制御できなければならない
- **FR-006**: システムはClaude CodeのHook（Stop）経由で完了を検出できなければならない
- **FR-007**: システムはGUI内蔵ターミナル複合方式（プロセス終了+出力パターン+アクティビティ）でClaude Code以外のエージェント完了を検出できなければならない
...
- **FR-015**: マスターエージェントのLLM APIコール数と推定トークン数をGUI上で可視化しなければならない
...
- **FR-026**: エージェントモードのチャット履歴は会話形式（バブル）で表示し、ユーザーは右寄せ、アシスタントは左寄せとしなければならない
- **FR-027**: IME変換中（composition中）のEnter入力では送信せず、Enter送信/Shift+Enter改行を維持しなければならない
- **FR-028**: リクエスト送信中は送信ボタンにスピナーを表示し、再送信を抑止しなければならない
- **FR-029**: 新規メッセージ追加時はチャット履歴が自動的に最下部へスクロールしなければならない
- **FR-030**: システムはAgentビューにセッション内のタスク一覧（状態付き）を表示しなければならない
- **FR-031**: ユーザーがAgentビューでタスクを選択したとき、当該タスクに割り当てられたサブエージェント一覧を下部ペインに表示しなければならない
- **FR-032**: サブエージェント一覧には、各エージェントのworktree情報（パスまたは識別子）を表示しなければならない
- **FR-033**: マスターエージェントがタスク再分割・再割当を行った場合、Agentビューのタスク一覧およびサブエージェント一覧は同一セッション内で即時同期しなければならない
- **FR-034**: worktree情報はリポジトリ相対パスをデフォルト表示とし、ホバーまたは詳細表示で絶対パスを参照できなければならない
- **FR-035**: マスターエージェントはSpec Kitワークフローの成果物として`spec.md`/`plan.md`/`tasks.md`に加えて`tdd.md`を生成しなければならない
- **FR-036**: マスターエージェントは`spec.md`/`plan.md`/`tasks.md`/`tdd.md`の4点が揃うまでサブエージェント実行フェーズへ遷移してはならない

### テスト要件

- **TR-001**: エージェントモードUIのIME送信抑止、送信中スピナー、チャット表示は自動テストで検証する（TDD前提）
- **TR-002**: Agentビューでタスク選択時に、対応するサブエージェント一覧とworktree情報が下部に表示されることを自動テストで検証する（TDD前提）
- **TR-003**: MA実行開始前に`spec.md`/`plan.md`/`tasks.md`/`tdd.md`の存在チェックが行われ、不足時は実行がブロックされることを自動テストで検証する
...
- **SubAgent**: サブエージェント（Claude Code等）のインスタンス。GUI内蔵ターミナルペイン、worktree、状態を持つ
...
- **SC-007**: 新規メッセージ受信時に最新メッセージが画面下部に表示される

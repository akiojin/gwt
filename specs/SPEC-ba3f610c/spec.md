...
**この優先度の理由**: GitHub Issueベースの仕様管理がプロジェクトモードの核心機能であり、Coordinator/Developer実行の前提条件となる。
...
**この優先度の理由**: Developerへの指示がタスク実行の実体であり、プロジェクトモードの価値を実現する核心機能。
...
- これらのツールは`agent_tools.rs`から**完全移行**し、Codex/Claude Code/Geminiへ自動登録されるスキルとして一本化する。
...
- **issue_specツールはCodex/Claude Code/Geminiへ自動登録されるSkillとして公開**し、ブランチモードの各エージェントからも利用可能にする。
...
- **Developerのエージェント種別はプロジェクトモード起動時にユーザーが指定する**。指定されたエージェント種別が全Developerに適用される。
...
- **FR-018**: issue_specツール群はgwt内蔵（agent_tools.rs）と自動登録Skill（Codex/Claude Code/Gemini）の両方で提供しなければならない
- **FR-019**: issue_specツールはCodex/Claude Code/Geminiへ自動登録されるSkillとして公開し、プロジェクトモード・ブランチモード両方から利用可能でなければならない

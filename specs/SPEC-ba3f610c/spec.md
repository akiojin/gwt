...
- Claude Code向けプラグイン（`worktree-protection-hooks`）にはHook転送を同梱し、`UserPromptSubmit`/`PreToolUse`/`PostToolUse`/`Notification`/`Stop`を`gwt-tauri hook <Event>`へ転送する。
...
- **FR-074**: Claude Codeプラグイン（`worktree-protection-hooks`）のHook定義に、`UserPromptSubmit`/`PreToolUse`/`PostToolUse`/`Notification`/`Stop`の5イベントを`gwt-tauri hook <Event>`へ転送する設定を含めなければならない

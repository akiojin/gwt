...
| 層 | 内部名 | UI表示名 | 役割 | 実行場所 |
|---|---|---|---|---|
| PM | PM | Lead | プロジェクト統括・要件定義・Spec Kit実行・進捗管理・段階的委譲 | 選択可能（gwt内蔵AI / Claude Code等） |
| Orchestrator | Orchestrator | Coordinator | 実装単位のタスク管理・CI監視・修正ループ・Developer管理 | GUI内蔵ターミナルペイン |
...
3. Leadが要件定義を行いspec.mdを生成する（specify）
4. Leadが実装計画を策定しplan.md / tasks.md / tdd.mdを生成する
5. LeadがGitHub Issueを作成し、GitHub Projectに登録する
6. Leadがユーザーに計画全体を提示し承認を得る
7. 承認後、LeadがCoordinatorを内蔵ターミナルペインで起動し、**完成したSpec付きタスク**を渡す
8. CoordinatorがWorktreeを作成し、Developerを起動して実装を実行する
9. CoordinatorがDeveloper完了検出 → テスト検証 → PR作成を行う
10. CoordinatorがCI結果を監視し、失敗時はDeveloperに修正を指示する（自律修正ループ）
11. Leadが全体進捗を把握し、ユーザーに定期報告する
12. 全タスク完了・CI全パス後、LeadがGitHub Issueをクローズし、Projectステータスを更新する
...
- Leadが選択可能（内蔵AI / Claude Code等）な場合、切り替え時のセッション継続性は？
...
- Leadの実行基盤は**ユーザーが選択可能**。
- 選択肢: gwt内蔵AI（現在のMaster Agent相当）/ Claude Code / その他外部エージェント
- どちらの基盤でも同一のツールセット（スキル化されたPTY通信等）を使用する。
- デフォルトはgwt内蔵AI。

### Coordinator起動と管理

- CoordinatorはGUI内蔵ターミナルペインで起動する（ユーザーが出力を見られる）。
- Coordinatorの粒度はLeadが動的に判断する:
  - Spec規模に応じて1 Spec = 1 Coordinatorまたは1 Coordinator = N Specsを選択
  - 小規模なSpecは1つのCoordinatorで複数管理
  - 大規模なSpecは専用のCoordinatorを割り当て
- CoordinatorはClaude Code Agent Team等のチームセッションとして動作可能。
- Coordinatorの起動コマンドと引数はLead（またはgwt）が生成する。
...
- Coordinatorは生成したtasks.mdの各タスクをDeveloper単位に割り当てる。
- 割り当て時の判断:
  - 独立したタスクは別々のWorktree+Developerに割り当てる
  - 依存関係のあるタスクは同一Worktreeで順次実行、またはGit merge経由で連携する
- worktree_strategyはCoordinatorがLLMで判断する（`new`または`shared`）。
- Coordinatorはタスク分割時に各タスクが**1 Developerで完遂可能な粒度**であることを保証する。
...
- 完了済みセッションに関連する追加依頼があった場合、LeadがLLMで「前回の続き」か「新規セッション」かを判断する。
...
- プロジェクトチームは**機能実装単位**で存在する。1つの機能要求に対して1つのセッションが対応する。
- 複数の独立した機能を依頼した場合、それぞれ独立したセッションとして管理する。
- 複数セッションは**キュー方式**で処理する。実行は1セッションずつ行い、待機中はキューに入る。
...
- プロジェクトチームは、**Leadチャット（メイン）**、**サイドバータスク一覧**、**下部Coordinatorパネル**で構成する。
- タブ名: `Project Team`
...
|  Lead: "Plan approved. Starting Coordinator..."           |
|                                                           |
|  [Input area                                         Send]|
+-----------------------------------------------------------+
...
- **FR-042**: Leadの実行基盤はユーザーが選択可能でなければならない（gwt内蔵AI / Claude Code等）
...
- **FR-053**: Coordinatorの粒度（1:1 or 1:N）はLeadが動的に判断しなければならない
...
- **Session**: プロジェクトチームのセッション全体を表す。全層の状態、会話履歴、タスク一覧、進捗を含む
- **Task**: Coordinatorが分割した個別のタスク。状態（pending/running/completed/failed）、依存関係、割り当てworktreeを持つ
- **Coordinator**: Orchestrator層のインスタンス。GUI内蔵ターミナルペイン、管理するDeveloper一覧、状態を持つ
- **Developer**: Worker層のインスタンス。GUI内蔵ターミナルペイン、worktree、状態を持つ
...
- Coordinator用のエージェント（Claude Code等）が利用可能

...
- スクロールバックが未フラッシュのタイミングでも、Diagnostics が直近の出力を可能な限り反映し、誤って「色SGRなし」と判定しないこと
- プロジェクトを跨いで同名ブランチが存在しても、branch->paneId の保存先が衝突しないこと
- `paneId` にパスセパレータ等の不正文字が含まれる場合、バックエンドはエラーを返し、意図しないファイルを参照してはならない
...
- **FR-454**: システムは、Diagnostics 実行時に対象ペインが稼働中の場合、スクロールバックを可能な限りフラッシュしてからログ末尾を解析しなければ**ならない**
- **FR-455**: システムは、`paneId` をファイルパスへ埋め込む前に検証し、不正な `paneId` を拒否しなければ**ならない**

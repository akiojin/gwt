...
- 仕様検索は `gwt-project-index`、仕様作成/更新は `gwt-issue-spec-ops` を使う。

### Target agents and embed locations

- Claude Code: `./.claude/settings.json`, `./.claude/skills`, `./.claude/commands`, `./.claude/hooks` を project-local に使う
- Codex: `./.codex/skills`
...
- Claude Code が実際に読む hook 設定は `./.claude/settings.json` の `hooks` セクションとする。
...
| FR-REG-002 | Claude Code は project-local な `.claude/settings.json` を使うこと |
...
| FR-REG-005 | Codex/Gemini の bundle は `gwt-project-index` と `gwt-issue-spec-ops` を含む |
...
| US2 | Codex セッションで `$gwt` を実行 | `gwt-project-index`、`gwt-issue-spec-ops`、`gwt-pr`、`gwt-fix-pr` などの `gwt-*` skill が候補表示される |

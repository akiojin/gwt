...
### ユーザーストーリー 3 - モデル一覧の順序を最新優先にする (優先度: P2)

開発者がモデル一覧を見たとき、最新の gpt-5.2-codex が先頭付近に配置され、推奨モデルが迷わず選べる。

**この優先度の理由**: モデル数が増えるほど誤選択が起きやすく、並び順がユーザビリティに直結する。

**独立したテスト**: モデル一覧の並び順をユニットテストで確認できる。

**受け入れシナリオ**:

1. **前提条件** Codex のモデル一覧を取得、**操作** 並び順を確認、**期待結果** gpt-5.2-codex が先頭付近に並ぶ
...
- **FR-004**: Codex のモデル一覧は最新モデル（gpt-5.2-codex）を優先した並び順を維持しなければならない
...
- **SC-003**: UI のモデル選択テストが gpt-5.2-codex の並び順を満たした状態で成功する
...
- 既存の gpt-5.1 系モデルは互換性確保のため保持される

## 範囲外 *(必須)*

- gpt-5.1 系モデルの削除や非表示化

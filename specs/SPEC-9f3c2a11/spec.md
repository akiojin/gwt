...
| Engine | `qwen3-asr` |
| デフォルト品質 | `balanced` |
| デフォルトモデル | `Qwen/Qwen3-ASR-1.7B` |
| 品質別モデル | `fast -> Qwen/Qwen3-ASR-0.6B`, `balanced -> Qwen/Qwen3-ASR-1.7B`, `accurate -> Qwen/Qwen3-ASR-1.7B` |

### 非採用（現時点）

- `whisper-rs` ベース実行は互換入力（`engine=whisper`）のみ受け付け、保存時に `qwen3-asr` へ正規化する。
- `Qwen/Qwen3-ASR-0.6B` は `fast` 専用で、既定モデルにはしない。
- `microsoft/VibeVoice-ASR` など他系統モデルは比較評価対象とし、Voice Input の実行モデルには採用しない。
...
- **FR-021**: システムは Voice Input 既定値として `engine=qwen3-asr`、`quality=balanced`、`model=Qwen/Qwen3-ASR-1.7B` を適用しなければならない。
- **FR-022**: システムは Voice Input 実行モデルとして本仕様の品質別マップ以外を選択しない（比較評価用途を除く）。

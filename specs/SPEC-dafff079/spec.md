...
2. **前提条件** プロファイルエディター画面でプロファイル「production」が選択されている、**操作** Enterキーを押す、**期待結果** 「production」がアクティブプロファイルになり、環境変数編集画面が表示される
...
2. **前提条件** 環境変数`MY_VAR`が選択されている、**操作** `Enter`または`e`キーを押して値を`new-value`に変更する、**期待結果** 環境変数`MY_VAR`の値が`new-value`に更新される
...
6. **前提条件** 環境変数一覧が表示されている、**操作** `v`キーで表示切替を行う、**期待結果** 値が表示/マスクで切り替わる（hidden時は値がマスク表示される）
...
2. **前提条件** 環境変数の値入力中である、**操作** `e`や`v`を入力する、**期待結果** 文字入力として値に反映され、編集/表示切替などのショートカットは発生しない
...
- **FR-018**: システムは環境変数一覧の表示切替で、hidden時に全ての値をマスク表示し**なければならない**

...
開発者として、プロファイルの環境変数がOS環境変数をどのように上書きするかを確認したい。環境変数編集画面からOS環境変数一覧へ遷移でき、プロファイルで上書きされるキーは変数名の色が変わるようにしたい。
...
1. **前提条件** OS環境変数に`ANTHROPIC_API_KEY`が設定されており、プロファイルにも`ANTHROPIC_API_KEY`が設定されている、**操作** OS環境変数一覧を表示する、**期待結果** OS環境変数の`ANTHROPIC_API_KEY`の変数名が黄色で表示される
2. **前提条件** OS環境変数に`HOME`が設定されており、プロファイルには`HOME`が設定されていない、**操作** OS環境変数一覧を表示する、**期待結果** OS環境変数の`HOME`の変数名は通常色で表示される
3. **前提条件** OS環境変数一覧が表示されている、**操作** 矢印キーやPageUp/PageDownでスクロールする、**期待結果** 大量のOS環境変数をスクロールして閲覧できる
...
1. **前提条件** プロファイル「development」の環境変数編集画面が表示されている、**操作** `a`キーを押して新しい環境変数`MY_VAR=test`を追加する、**期待結果** 環境変数`MY_VAR`がプロファイルに追加される

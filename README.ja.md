...
ください。完全な URL は process-local capability なので、fragment を含む値全体を
private に扱ってください。gwt の再起動ごとに更新されます。frontend は fragment を
HttpOnly / SameSite=Strict の session cookie に交換し、直後に browser history の
表示 URL から fragment を除去します。
...
handoff file には同じ private capability が同一 directory 内の crash-consistent な
置換で書かれます。Unix では gwt が mode `0600` を強制します。Windows では current
user だけが読める ACL の directory を指定してください。CI artifact として公開しないで
ください。

信頼境界は **認証済み LAN のみ** (VPN-extended LAN を含む) です。すべての
WebSocket は process-local browser session cookie（または用途を限定した managed-pane
bearer）を要求し、Origin がない、または異なる接続は fail closed になります。一方で
TLS 終端と rate limiting は組み込まれていないため、信頼できない network 上では
capability と session traffic を安全に扱えません。`--bind` は opt-in で、既定の
`127.0.0.1` が最も安全です。別端末から利用する場合は、ポートを公開 Internet に
晒さず、信頼できる VPN (Tailscale、WireGuard など) と完全な capability URL を使って
ください。
...
ヘルスチェックでログが埋まりません。URL fragment、cookie、Authorization 値、
recovery handle は access log に記録しません。
...
gwt は復元対象の Intake または Execution Agent を起動する前に、プロジェクト単位の
復元レコードを作成します。アプリやマシンがクラッシュした後、検証済みの provider
conversation は元の worktree で自動再開されます。provider が同じ conversation は
もう存在しないと確定的に返した場合、gwt は最新の bounded semantic checkpoint を
渡して新しい conversation を開始します。空の prompt で暗黙に開始することは
ありません。認証、通信、protocol の再試行可能な失敗は、operator の判断が必要な
状態として表示されます。

Command Palette の `Recovery Center` から現在の復元候補をすべて開けます。起動時に自動で
開くのは判断が必要な候補がある場合だけなので、正常な exact restore は通常作業を
遮りません。候補には Intake/Execution 種別、provider、worktree、checkpoint coverage、
capture health、Board delivery、exact-resume の根拠が表示されます。利用できる操作は
以下です。

- `Focus` — すでに動いている replacement を前面に出す
- `Confirm & Resume` — authoritative な exact provider conversation を確認して再開する
- `Continue Checkpoint` — durable checkpoint を渡して新しい conversation を始める
- `Start Fresh` — 元の initial request から明示的にやり直す。旧形式からの移行で
  initial request が残らなかった場合、この操作は表示しない
- `Open Board` / `Details` — 公開 milestone または復元根拠を確認する
- `Discard` — 2 段階の確認後に gwt の復元内容を削除する

upgrade 後の最初の起動では、復元可能な旧 Session file と paused window projection も
棚卸しします。保存された command を再実行したり provider root を推測したりせず、
distinct な session ごとに候補として import します。旧 Session に provider root が
ない場合は、provider message を取り込まず、bounded な native metadata の root 種別、
canonical worktree、launch 時刻だけを照合します。唯一の強い一致は exact resume でき、
同じ Intake に複数の provider conversation が対応し得る場合は、Recovery Center が
記録済みの根拠を表示して明示的な選択を求めます。exact の根拠も以前の semantic
checkpoint も残っていない候補は、argv や Board から private discussion を再構成せず
Attention に保ちます。

未解決の ephemeral Intake worktree 自体が消えている場合、gwt は記録済みの
`.intake` / `.intake-N` path だけを immutable な pinned base commit で復元してから
再開します。Execution や任意の user path は再作成せず、pin がない、または不一致なら
候補を Attention に保ちます。

Intake の discussion checkpoint は、簡潔で idempotent な milestone として Board に
投稿されます。confirmed decision、open question、関連 SPEC、次の action は含みますが、
private transcript、tool output、hidden reasoning、credential は含みません。既存の旧
transcript は自動で backfill せず、現在時点の user-confirmed summary だけを投稿できます。
復元用にコピーする attachment は content-addressed です。Recovery Store は 1 件あたり
32 MiB まで受け入れますが、managed Codex capture はより厳しい 24 MiB の safety bound を
使います。未解決の復元内容に時間による期限はありません。`Discard` または復元完了時には
checkpoint とコピー済み attachment を直ちに削除し、最小 tombstone だけを 30 日
保持します。provider 自身の conversation history は削除しません。

現在の managed Intake では、structured `discussion.update` 自体が semantic checkpoint と
deterministic Board outbox の永続化境界です。同じ milestone の再実行では checkpoint
revision も Board entry も増えません。memo と Recovery checkpoint は同じ deterministic
operation marker を保持するため、crash や concurrency による不一致は retry で収束するまで
Stop が拒否します。`intake.checkpoint.current/update` は、allowlist
済みの完了 visible item または保持・新規 attachment を追加するときだけ使用します。
upgrade 前の current Intake に `GWT_RECOVERY_ID` がない場合は、正確な
`GWT_SESSION_ID` ledger を解決し、bounded な metadata-only import を on-demand で
行います。root がない、または曖昧な候補は Recovery Center の Attention に残し、
private conversation や Board から推測しません。Docker Claude の attachment capture は
既知の制限です。structured discussion fields は永続化されますが、managed runtime が
allowlist 済み copy を取得するまでは attachment fallback を利用できません。

Board の `Intake` filter は、記録済みの Intake origin または明示的な `intake` topic で
これらの milestone を表示し、title から推測しません。Local Board の再送は元の
milestone ID を使うため、crash 後も 1 entry に収束します。Slack / Teams は現在
caller 指定の idempotency ID を保持できないため、gwt は重複投稿や誤った ack を避けて
remote milestone を pending のまま残します。復元 checkpoint 自体は commit され、
安全に配送できるまで Recovery Center に bounded な配送エラーが表示されます。

Codex の root binding と checkpoint 復元は、Host では browser server と分離した専用
loopback-only bridge を使います。Docker / Podman の durability path は mounted `gwtd`
sidecar と、選択した Compose service 内の loopback を使うため、published port、host
gateway、token を含む URL は不要です。container 復元には通常の gwt Linux bundle
mount と、その service 内で利用できる Codex runner が必要です。

これとは別に、全 agent が使う managed hook / pane control channel は専用 agent-only
EmbeddedServer listener を使います。Host agent は `127.0.0.1`、Docker agent は
`host.docker.internal`（gwt 管理の Linux override が user override を変更せず
host-gateway mapping を追加）、Podman agent は予約済みの
`host.containers.internal` alias へ接続します。この listener が公開するのは hook と
pane-control endpoint だけです。opaque capability は server 側で canonical project と
Session に束縛し、argv や URL ではなく環境変数だけで渡します。runtime、bridge、
project、Session、control channel のいずれかが一致しない場合は fail closed で
Recovery Center に移り、観測されない direct Codex 起動へは戻りません。

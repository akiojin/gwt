...
Codex の Host 復元は browser server とは別の専用 loopback-only bridge を使います。
Docker / Podman は mounted `gwtd` sidecar と、選択した Compose service 内の loopback
だけを使い、published port、host gateway、token を含む URL は不要です。このため
container 復元には通常の gwt Linux bundle mount と、その service 内で利用できる
Codex runner が必要です。runner、bridge、control channel が一致しない場合は fail
closed で Recovery Center に移り、観測されない direct Codex 起動へは戻りません。

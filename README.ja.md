...
した内容が、別インスタンスにも遅延なく届く）。最初の `gwt` GUI
起動時には endpoint メタデータが `~/.gwt/...` に記録されるため、
後続のプロセスは live な daemon があるかを確認できます。Ctrl-C /
SIGTERM で daemon を停止するまでバックグラウンドで動き続け、
クラッシュ等で残った stale エントリは次の `gwtd daemon start` で
クリーンアップされます。診断用に `gwtd daemon status` で現在の
endpoint を確認できます。`gwtd daemon start` を実行していない
場合は multi-instance fan-out は無効ですが、ローカルのファイル
ベース state とファイル watcher は従来どおり動作します。

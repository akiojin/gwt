...
3. Project Bar または Command Palette の `Start Work` から、ブランチを選択・命名せずに
   launch settings を開く
4. `Agent` を起動する。起動確定時に gwt が内部で
   `work/YYYYMMDD-HHMM[-n]` ブランチ / worktree を作成する
5. ブランチ確認・フィルタ・cleanup・Git detail が必要な場合に `Branches` を開く
6. 追加ボタンから `File Tree` などの補助ウィンドウを開く

主なウィンドウ:

- `Agent` (Start Work / Launch Agent から作成)
- `Branches` (ブランチ確認と cleanup)
- `File Tree`
- `Settings`
- `Memo`
- `Profile`

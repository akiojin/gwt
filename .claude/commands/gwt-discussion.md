...
7. When autonomous follow-up should start after approval, record the Action Bundle goal condition with JSON operation `discuss.goal_pending` so the next approval turn can start `/goal` and then mark `goal_started`, `goal_failed`, or `goal_skipped`.
